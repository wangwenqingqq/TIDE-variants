...
// Buffer-only C1 is an explicit runtime mode.  It intentionally bypasses the
// uncertified direct/leaf-insertion fast path and uses the existing global
// insert_list as the only live Delta.  The default remains legacy behavior so
// old experiments are not silently reinterpreted.
static inline bool c1BufferOnlyMode()
{
	const char *value = std::getenv("TIDE_C1_BUFFER_ONLY");
					...
					char *&data_s, int *&size_s, FILE *fcost, float &time_update_s, float &time_update_u, int &count_update_s, int &count_update_u)
{
	...
	if (MAX_IN_SIZE <= 0)
	{
		fprintf(stderr, "[C1] invalid MAX_IN_SIZE=%d\n", MAX_IN_SIZE);
		return;
	}
	printf("[C1] update mode=%s; direct insertion=%s; C2(update)=off\n",
		c1_buffer_only ? "buffer-only" : "legacy-mixed",
		c1_buffer_only ? "disabled" : "enabled");
	cudaDeviceSynchronize();
	...
	for (int i = 0; i < update_num; i++)
	{
		if (update_list[i].update_flag == 0)
		{
			...
			int ins_result = 1;  // 1 means route to exact global Delta.
			if (!c1_buffer_only)
			{
				ins_result = incrementalInsert(data_d, node_list, empty_list,
			...
			if (ins_result == 0) {
				...
				{
					fprintf(stderr, "[C1] live Delta capacity invariant violated before append\n");
					return;
				}
			...
			{
				fprintf(stderr, "[C1] buffer-only invariant violated: direct overflow pool is non-empty\n");
				return;
			}
