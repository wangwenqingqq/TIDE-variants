#!/usr/bin/env python3
"""CPU-only preflight for the strict v4 stable-ID seeded G2 rebuild witness."""
...
def validate_bundle(root: Path, bundle: Path) -> tuple[dict[str, Any], list[str]]:
    ...
    try:
        if root.resolve() != ROOT.resolve():
            errors.append('root_must_equal_canonical_v4_root')
        try:
            bundle.resolve().relative_to((ROOT / 'bundles').resolve())
        except ValueError:
            errors.append('bundle_must_be_under_v4_bundles_root')
        ...
        for forbidden in ('g1b_capacity_witness_v3.json', 'candidate_selection_v2.json', 'engine_results.jsonl'):
            if (bundle / forbidden).exists():
                errors.append(f'external_result_copied_into_v4_bundle_forbidden:{forbidden}')
        manifest = read_json(bundle / 'manifest.json')
        if manifest.get('schema') != 'safe-c1-g2-rebuild-witness-v4-bundle-manifest':
        ...
        if manifest.get('stable_id_layout') != CURRENT_RUNNER_STABLE_ID_LAYOUT_MODE:
            errors.append('wrong_v4_stable_id_layout')
        ...
        if not isinstance(ext_g1b, dict) or ext_g1b.get('path') != str(V3_RESULT) or \
                ext_g1b.get('sha256') != sha256(V3_RESULT) or ext_g1b.get('copied_into_v4_bundle') is not False:
        ...
        if not isinstance(ext_selection, dict) or ext_selection.get('path') != str(V2_SELECTION) or \
                ext_selection.get('sha256') != sha256(V2_SELECTION) or ext_selection.get('copied_into_v4_bundle') is not False:
        ...
        if any(sha256(bundle / name) != digest for name, digest in v3_hashes.items()):
            errors.append('local_v4_immutable_payload_not_exact_v3_copy')
    ...
    report = {
        'schema': 'safe-c1-g2-v4-rebuild-witness-preflight',
