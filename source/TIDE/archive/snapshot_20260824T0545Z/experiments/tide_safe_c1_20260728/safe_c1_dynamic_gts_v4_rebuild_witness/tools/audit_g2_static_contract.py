#!/usr/bin/env python3
"""CPU-only static audit for the isolated v4 G2 rebuild witness."""
...
def main() -> int:
    ...
    if root != ROOT.resolve(): errors.append('root_must_equal_canonical_v4_root')
    ...
    # v3 source baseline is checked against v3, not v4 (which is intentionally changed).
    ...
    report={'schema':'safe-c1-g2-v4-static-audit','status':'PASS_CPU_ONLY_G2_STATIC_AUDIT' if not errors else 'FAIL_CPU_ONLY_G2_STATIC_AUDIT','gpu_used':False,'cuda_binary_executed':False,'root':str(root),'source_sha256':sha(src) if src.is_file() else None,'checks':checks,'errors':errors,'scope':'static source/freeze audit only; no CUDA binary execution or performance claim'}
