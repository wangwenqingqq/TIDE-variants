#!/usr/bin/env python3
"""Independent CPU finalizer for a future G2 v5 guarded engine run.
...
from artifact_pinning_v5 import VERIFY_PASS, verify_execution_artifact_pin
...
def validate_engine(bundle:Path, preflight:dict[str,Any], rows:list[dict[str,Any],], summary:dict[str,Any]) -> list[str]:
    ...
    if by_record.get('meta'):
        ...
        require(meta.get('schema')=='safe-c1-g2-rebuild-witness-v5',errors,'meta_schema')
        require(meta.get('legacy_incremental_updater_used') is False,errors,'meta_legacy_updater')
        require(meta.get('stable_id_layout')=='identity_full_immutable_pool_seeded_stable_ids_v5',errors,'meta_layout')
    ...
    require(summary.get('schema')=='safe-c1-g2-rebuild-witness-v5',errors,'summary_schema')
...
def validate_pinning(bundle: Path, binary: Path, artifact_pin: Path, pin_verification: Path, run_card: Path) -> tuple[dict[str, Any], list[str]]:
    """Recompute all pinned bytes, then bind the guard receipt and run card."""
    errors: list[str] = []
    fresh = verify_execution_artifact_pin(ROOT, bundle, binary, artifact_pin)
    if fresh.get('status') != VERIFY_PASS:
        errors.append('artifact_pin_mismatch')
        for item in fresh.get('errors', []):
            errors.append('artifact_pin_detail:' + str(item))
        return fresh, errors
    try:
        snapshot = load_json(pin_verification)
    except Exception as exc:
        return fresh, ['artifact_pin_mismatch', 'artifact_pin_verification_receipt_invalid:' + str(exc)]
    if snapshot.get('status') != VERIFY_PASS:
        errors.append('artifact_pin_mismatch')
        errors.append('artifact_pin_verification_receipt_not_pass')
    if snapshot.get('pin_sha256') != fresh.get('pin_sha256'):
        errors.append('artifact_pin_mismatch')
        errors.append('artifact_pin_verification_pin_sha_mismatch')
    if snapshot.get('actual_hashes') != fresh.get('actual_hashes'):
        errors.append('artifact_pin_mismatch')
        errors.append('artifact_pin_verification_actual_hash_mismatch')
    if snapshot.get('root') != str(ROOT) or snapshot.get('bundle') != str(bundle.resolve()) or snapshot.get('binary') != str(binary.resolve()):
        errors.append('artifact_pin_mismatch')
        errors.append('artifact_pin_verification_path_mismatch')
    try:
        card = load_json(run_card)
    except Exception as exc:
        return fresh, errors + ['artifact_pin_mismatch', 'run_card_invalid:' + str(exc)]
    receipt = card.get('artifact_pinning')
    if not isinstance(receipt, dict):
        return fresh, errors + ['artifact_pin_mismatch', 'run_card_artifact_pinning_missing']
    if receipt.get('pin_path') != str(artifact_pin.resolve()) or receipt.get('pin_sha256') != fresh.get('pin_sha256'):
        errors.extend(['artifact_pin_mismatch', 'run_card_artifact_pin_identity_mismatch'])
    if receipt.get('verification_path') != str(pin_verification.resolve()):
        errors.extend(['artifact_pin_mismatch', 'run_card_verification_path_mismatch'])
    if receipt.get('actual_hashes') != fresh.get('actual_hashes'):
        errors.extend(['artifact_pin_mismatch', 'run_card_actual_hash_mismatch'])
    inputs = card.get('inputs')
    if not isinstance(inputs, dict) or inputs.get('binary') != str(binary.resolve()) or inputs.get('bundle') != str(bundle.resolve()):
        errors.extend(['artifact_pin_mismatch', 'run_card_input_path_mismatch'])
    return fresh, errors
...
def main() -> int:
    ...
    p.add_argument('--engine-results', type=Path, required=True)
    p.add_argument('--engine-summary', type=Path, required=True)
    p.add_argument('--artifact-pin', type=Path, required=True)
    p.add_argument('--pin-verification', type=Path, required=True)
    p.add_argument('--binary', type=Path, required=True)
    p.add_argument('--run-card', type=Path, required=True)
    p.add_argument('--out', type=Path, required=True)
    a = p.parse_args()
    errors: list[str] = []
    pin_report: dict[str, Any] = {'status': 'NOT_VERIFIED', 'actual_hashes': {}, 'pin_sha256': None}
    report: dict[str, Any]
    try:
        root = a.root.resolve(); bundle = a.bundle.resolve(); binary = a.binary.resolve()
        if root != ROOT.resolve():
            errors.append('root_not_canonical')
        pin_report, pin_errors = validate_pinning(bundle, binary, a.artifact_pin, a.pin_verification, a.run_card)
        errors.extend(pin_errors)
        cpu_report, cpu_errors = validate_bundle(root, bundle)
        if cpu_errors:
            errors.append('bundle_revalidation_failed')
        pre = load_json(a.preflight)
        rows = load_jsonl(a.engine_results)
        summ = load_json(a.engine_summary)
        errors.extend(validate_engine(bundle, pre, rows, summ))
        report = {
            'schema': 'safe-c1-g2-v5-independent-finalizer',
            'status': 'PASS_G2_REBUILD_WITNESS_INDEPENDENTLY_VALIDATED' if not errors else 'FAIL_G2_REBUILD_WITNESS_INDEPENDENT_VALIDATION',
            'gpu_used_by_finalizer': False,
            'cuda_binary_executed_by_finalizer': False,
            'scope': 'validates one strict witness artifact; does not establish performance/general rebuild/range/C2/all-input tie claims',
            'root': str(root), 'bundle': str(bundle), 'binary': str(binary),
            'engine_results_sha256': hashlib.sha256(a.engine_results.read_bytes()).hexdigest(),
            'engine_summary_sha256': hashlib.sha256(a.engine_summary.read_bytes()).hexdigest(),
            'execution_artifact_pin_sha256': pin_report.get('pin_sha256'),
            'artifact_pin_verified': pin_report.get('status') == VERIFY_PASS and not any(x == 'artifact_pin_mismatch' for x in errors),
            'actual_hashes': pin_report.get('actual_hashes', {}),
            'errors': errors,
        }
    except Exception as exc:
        errors.append('finalizer_exception:' + str(exc))
        report = {
            'schema': 'safe-c1-g2-v5-independent-finalizer',
            'status': 'FAIL_G2_REBUILD_WITNESS_INDEPENDENT_VALIDATION',
            'gpu_used_by_finalizer': False,
            'cuda_binary_executed_by_finalizer': False,
            'execution_artifact_pin_sha256': pin_report.get('pin_sha256'),
