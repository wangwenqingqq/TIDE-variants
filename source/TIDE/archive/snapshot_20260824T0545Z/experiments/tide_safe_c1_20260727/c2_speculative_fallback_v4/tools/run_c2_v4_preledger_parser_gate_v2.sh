...
BINDING="$ROOT/provenance/c2_v4_final_binary_manifest_binding_v2.json"
GATE_PROTOCOL="$ROOT/protocols/c2_v4_preledger_parser_compatibility_gate_v1.json"
LAUNCHER="$ROOT/tools/run_c2_v4_preledger_parser_gate_v2.sh"
RECEIPTS="$ROOT/preledger_receipts"
PARENT="$RECEIPTS/c2_v4_parser_gate_v2"
...
x["status"]="FAILED_VALIDATE_WORKLOAD_ONLY_PRELEDGER_V2"
...
[[ "$(find "$RECEIPTS" -mindepth 1 -maxdepth 1 -printf '%f\n' | LC_ALL=C sort)" == "c2_v4_parser_gate_v1" ]] || {
  echo "unexpected prior receipt roots" >&2; exit 65;
}
[[ "$(sha256sum "$MANIFEST" | awk '{print $1}')" == "7b624940523955d6011fe55da1ca2736f9ac19d79c70e8b013c28f17ad00d7ae" ]] || {
...
if binding.get('status')!='PINNED_READY_FOR_FRESH_PRELEDGER_GATE_V2':
 raise SystemExit('binary/manifest binding is not v2 PASS')
...
print('PASS_PRELEDGER_GATE_V2_LAUNCH_PREFLIGHT')
PY

mkdir "$PARENT"
cat >"$CARD" <<EOF
{
  "schema": "safe-c2-v4-preledger-gate-launcher-card-v2",
...
x=json.load(open(card))
x["status"]="COMPLETE_VALIDATE_WORKLOAD_ONLY_RECEIPT_UNVERIFIED_V2"
