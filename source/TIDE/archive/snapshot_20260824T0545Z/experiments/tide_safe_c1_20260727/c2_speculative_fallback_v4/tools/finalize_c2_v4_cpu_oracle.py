...
FINAL_DIR = ROOT / "inputs/final_workload_v1"
...
def assert_no_c2_execution_state() -> None:
    forbidden = ("bin", "runs", "preledger_receipts", ".locks")
    present = [str(ROOT / name) for name in forbidden if (ROOT / name).exists() or (ROOT / name).is_symlink()]
    if present:
        raise FinalizeError(f"v4 C2 execution state existed before CPU admission: {present}")
...
def main() -> int:
    ...
    if selection.get("schema") != "safe-c2-v4-fresh-selection-pre-gt-v1" or selection.get("status") != "FRESH_IDS_FROZEN_PRE_GT_NO_C2_NO_GPU":
