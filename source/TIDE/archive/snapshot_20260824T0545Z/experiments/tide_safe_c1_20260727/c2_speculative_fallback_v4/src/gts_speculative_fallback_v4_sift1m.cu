...
BoundFile bind_manifest_file(const JsonValue& root, const std::string& label,
  ...
  if (strict_child_path(path, kV2Root)) fail("v2 root is forbidden for " + label);
  if (must_live_under_v4 && !strict_child_path(path, kV4Root)) {
...
WorkloadBinding validate_workload_binding(const Args& args) {
  if (!strict_child_path(args.workload_manifest, kV4Root)) {
  ...
  if (!args.gamma_vector_file.empty() && !strict_child_path(args.gamma_vector_file, kV4Root)) {
    fail("evaluate gamma vector must reside under isolated v4 root");
  }
  if (!args.validate_workload_only && !strict_child_path(args.output_dir, kV4Root)) {
...
void write_summary(const std::string& path, const Args& args, const BuildTiming& build, const Runtime& runtime,
  ...
  out << "{\n"
      ...
      << "\", \"schema\": \"gts-v4-compact-learn-workload-v1\", \"sha256_verified_in_runner\": true},\n"
      << "  \"mode\": \"" << args.mode << "\", \"stage\": \"" << args.stage << "\",\n"
