#!/usr/bin/env python3
"""Create the v8 source-only integrity root without compiling or launching CUDA."""
...
HELPER_PATHS = {
    ...
    "runtime_pin_verifier": "verify_v8_pins.py",
    "runtime_pin_generator": "create_v8_pins.py",
    ...
    "canonical_inspector_source": "tools/canonical_inspect_v8.cpp",
...
FIXTURE_PATHS = {
    ...
    "profile_manifest_execution": "tools/test_c1_v8_profile_manifest_fixtures.py",
    "profile_terminal_execution": "tools/test_c1_v8_profile_terminal_fixture.py",
    "guard_session_execution": "tools/test_c1_v8_guard_session_fixtures.py",
    "profile_child_sessions_execution": "tools/test_c1_v8_profile_child_sessions_fixture.py",
}
...
def main() -> int:
    ...
    if root != ROOT_DEFAULT:
        raise SystemExit("source pins must bind the canonical v8 root")
    ...
    payload = {
        "schema": "gtspp-c1-v8-source-pins-v1",
        "created_utc": datetime.now(timezone.utc).isoformat(),
        "root": str(root),
        "scope": "Source-only v8 child-session-contract integrity root. It pins the executable CPU-only artifact/terminal/real-session fixtures. No NVCC build, CUDA binary, nsys, nvidia-smi, GPU query, or workload is represented or authorized by this pin set.",
        "source_only": True,
    ...
    # The source pin root refuses a profile plan that merely permits child_sessions; it pins the exact strict evidence contract.
    if json.loads((root / "profile_execution_plan_v8.json").read_text()).get("child_session_contract") != expected_child_session_contract():
        raise SystemExit("profile child-session contract")
    out = args.out or root / "hardened_source_pins_v8.json"
    if not out.is_absolute() or out.is_symlink() or out.resolve(strict=False) != out or out.parent != root:
        raise SystemExit("source pin output must be a direct canonical file under the v8 root")
