#!/usr/bin/env python3
"""Fail-closed CPU-only integrity and source-policy verifier for C1 v8."""
...
def verify_profile_execution_plan(pins:dict)->None:
    ...
    if plan.get("schema")!="gtspp-c1-v8-profile-execution-plan-v1": raise SystemExit("profile execution plan schema")
    ...
    if plan.get("child_session_contract")!=expected_child_session_contract(): raise SystemExit("profile execution child-session contract")
...
def verify_trace_provenance(pins:dict,root:pathlib.Path)->None:
    """Fail before GPU launch if the v8-local trace provenance is inconsistent."""
...
def static_source_policy(root:pathlib.Path)->None:
    ...
    for token in (
        ...
        if token not in c1: raise SystemExit(f"missing C1 v8 policy token {token!r}")
    ...
    measured_verifier=(root/"verify_c1_semantics_v8.py").read_text()
    profile_guard=(root/"run_c1_profile_guard_v8.sh").read_text()
    profile_manifest=(root/"c1_v8_profile_manifest.py").read_text()
    profile_verifier=(root/"verify_c1_profile_artifacts_v8.py").read_text()
    profile_manifest_fixture=root/"tools/test_c1_v8_profile_manifest_fixtures.py"
    profile_terminal_fixture=root/"tools/test_c1_v8_profile_terminal_fixture.py"
    guard_session_fixture=root/"tools/test_c1_v8_guard_session_fixtures.py"
    profile_child_sessions_fixture=root/"tools/test_c1_v8_profile_child_sessions_fixture.py"
    finalizer=(root/"finalize_c1_evidence_v8.py").read_text()
    aggregator=(root/"aggregate_c1_formal_evidence_v8.py").read_text()
    for guard_name, guard_text, bootstrap in (("measured",measured_guard,"C1-V8-MEASURED-OWNED-BOOTSTRAP"),("profile",profile_guard,"C1-V8-PROFILE-OWNED-BOOTSTRAP")):
    ...
    for token in ("C1_V8_TRUST_ROOT=''","C1_V8_LAUNCHER=''","C1_V8_TRUST_GUARD_SHA=''","C1_V8_TRUST_PINS_SHA=''","C1_V8_TRUST_PIN_VERIFY_SHA=''","C1_ALLOW_GPU0=''","C1_RUN_OUT=''"):
        if token in measured_guard: raise SystemExit(f"measured guard erases inherited trust environment {token!r}")
    for token in ("C1_V8_PROFILE_TRUST_ROOT=''","C1_V8_PROFILE_LAUNCHER=''","C1_V8_PROFILE_TRUST_GUARD_SHA=''","C1_V8_PROFILE_TRUST_PINS_SHA=''","C1_V8_PROFILE_TRUST_PIN_VERIFY_SHA=''","C1_ALLOW_GPU0=''","C1_PROFILE_OUT=''"):
        if token in profile_guard: raise SystemExit(f"profile guard erases inherited trust environment {token!r}")
    for token in ("visible_cuda_device_uuid","CUDA UUID must exactly match manifest/session UUID","telemetry PCI discontinuity","capacity_snapshot_violations","capacity_snapshot_contract_v8.py"):
        if token not in measured_verifier: raise SystemExit(f"missing measured verifier binding token {token!r}")
    for token in ("--trace=cuda","--cuda-memory-usage=true","--profile-only","profile_nsys_stats_complete","um_sum","C1_V8_PROFILE_TRUST_GUARD_SHA","profile_execution_plan_v8.json","CUDA UUID/session","--repfile","--stats-stdout","PROFILE_MANIFEST_FIXTURE","PROFILE_TERMINAL_FIXTURE","PROFILE_CHILD_SESSIONS_FIXTURE","--fixture-terminal-failure","MANIFEST_READY","FINAL_STATUS='FAILED'","never allocation evidence"):
        if token not in profile_guard: raise SystemExit(f"missing profile guard policy token {token!r}")
    for token in ("profile_run_manifest_v8.json","nsys_argv","profile_artifacts","profile.sqlite","um_sum","args.repfile","--repfile","--stats-stdout","no_um_events_observed","UM_NO_PAGE_FAULT_MARKER"):
    ...
    for token in ("profile_run_manifest_v8.json","outer guarded nsys invocation manifest","profile_nsys_stats_complete","SQLite format 3","um_sum","no_um_events_observed","nsys_stats_stdout","EMPTY RESULTS","not allocation evidence","CUDA UUID differs from manifest/session UUID","profile telemetry PCI differs from outer prelaunch","capacity_snapshot_violations","capacity snapshot","verify_child_sessions_contract","CHILD_SESSION_DIRECTORY","SESSION_READY_PATTERN","owned_session_cleanup_v8.json","profile root directory set"):
        if token not in profile_verifier: raise SystemExit(f"missing profile verifier policy token {token!r}")
    for token in ("gtspp-c1-v8-measured-evidence-v2","gpu_pci_bus_id_normalized"):
        if token not in finalizer: raise SystemExit(f"missing measured finalizer binding token {token!r}")
    for token in ("gtspp-c1-v8-profile-verification-v5","cross-root GPU PCI disagreement","gtspp-c1-v8-formal-evidence-aggregate-v4","aggregate self pin/provenance mismatch","aggregate_script_sha256","SELF_PATH","um_zero_byte_interpretation"):
    ...
    for label,fixture_path in (("profile-manifest",profile_manifest_fixture),("profile-terminal",profile_terminal_fixture),("guard-session-ownership-and-manifest-race",guard_session_fixture),("profile-child-sessions-contract",profile_child_sessions_fixture)):
...
def main()->int:
    ...
    if p.get("schema")!="gtspp-c1-v8-pins-v1" or p.get("root")!=str(root): raise SystemExit("pins schema/root mismatch")
