...
int main(int argc,char**argv) {
 ...
 try {
  ...
  std::cerr<<"FAIL_STATIC_AABB_TOPK_CANARY: "<<e.what()<<std::endl;if(out_d)cudaFree(out_d);if(q_d)cudaFree(q_d);release_res();release_boxes(db);release_runtime(r);return 2;
 }
}
