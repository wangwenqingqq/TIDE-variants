/*
 * Safe-C2 static-AABB top-K canary v1.
 * New standard-SIFT smoke input only. Static raw-float32 L2 tree only.
 * It compares archived radial traversal and projected-AABB traversal against
 * an independent CPU exact oracle. It records no timing and supports no update.
 */
#include <algorithm>
#include <array>
#include <cmath>
...
#include "tree.cuh"
#include "search_static_aabb_v1.cuh"
...
struct Args {
 std::string base,queries,groundtruth,ids,outdir;
...
Args parse_args(int argc,char** argv) {
 ...
 if(a.base.empty()||a.queries.empty()||a.groundtruth.empty()||a.ids.empty()||a.outdir.empty()) fail("required argument missing");
 if(a.projection_dims<=0||a.projection_dims>kDim||a.warmups<0||a.reps<3) fail("invalid probe parameter");
 return a;
}
...
void run_aabb(const Runtime&r,float*q_d,int*out_d,int nq,StaticProjectedAabbDeviceView v){searchIndexKnnStaticAabbV1(r.data_d,r.node_list,r.id_list,r.max_node_num,q_d,out_d,nq,kK,r.height,r.data_info,r.empty_list,nullptr,nullptr,v);}
...
void write_perf_result(const std::string& out,const Args&a,const Boxes& b,const HostTree& tree,const Eval& base,const Eval& cand,bool same,const std::vector<double>& bt,const std::vector<double>& at,const std::vector<int>& ids) {
 ...
 f<<std::setprecision(12)<<"{\"schema\":\"safe-c2-static-aabb-paired-perf-probe-v1\",\"status\":\""<<(ok?"PASS_EXPLORATORY_PAIRED_PROBE":"FAIL_EXPLORATORY_PAIRED_PROBE")<<"\",\"scope\":\"exploratory paired GPU timing after an unmeasured GT correctness gate; no heldout/sealed/formal performance claim\",\"gpu_executed\":true,\"formal_claim_eligible\":false,";
...
int main(int argc,char**argv) {
 ...
 try {
  ...
  for(int i=0;i<a.warmups;++i){timed_baseline_probe(r,q_d,out_d,(int)ids.size());timed_aabb_probe(r,q_d,out_d,(int)ids.size(),view);}same_snapshot(frozen,capture_snapshot(r,db,boxes));
  std::vector<double> bt,at;bt.reserve(a.reps);at.reserve(a.reps);
  for(int rep=0;rep<a.reps;++rep){if((rep&1)==0){bt.push_back(timed_baseline_probe(r,q_d,out_d,(int)ids.size()));at.push_back(timed_aabb_probe(r,q_d,out_d,(int)ids.size(),view));}else{at.push_back(timed_aabb_probe(r,q_d,out_d,(int)ids.size(),view));bt.push_back(timed_baseline_probe(r,q_d,out_d,(int)ids.size()));}}
 ...
 } catch(const std::exception&e){std::cerr<<"FAIL_STATIC_AABB_PERF_PROBE: "<<e.what()<<std::endl;if(out_d)cudaFree(out_d);if(q_d)cudaFree(q_d);release_res();release_boxes(db);release_runtime(r);return 2;}
}
