...
PREF="$ROOT/tools/preflight_diskguard_v2.py";BIN="$ROOT/bin/static_aabb_topk_canary_v2_diskguard"
BASE=/workspace/legacy_workspace/GTS/Datasets/sift1m/sift_base.fvecs;QRY=/workspace/legacy_workspace/GTS/Datasets/sift1m/sift_query.fvecs;IDS="$ROOT/inputs/standard_sift_tiefree_smoke32_v1.ids"
...
RUN="$(mktemp -d "$ROOT/runs/static_aabb_v2_diskguard_topk_m128_$(date -u +%Y%m%dT%H%M%SZ)_XXXXXX")";chmod 0700 "$RUN";C=run
trap 'rc=$?;python3 - "$RUN/terminal.json" "$rc" "$C" <<PY
import json,sys
from pathlib import Path
Path(sys.argv[1]).write_text(json.dumps({"schema":"safe-c2-static-aabb-diskguard-v2-terminal","status":"FAILED","exit_code":int(sys.argv[2]),"context":sys.argv[3],"formal_claim_eligible":False},sort_keys=True,indent=2)+"\n")
PY
exit $rc' ERR
...
import json,sys
x=json.load(open(sys.argv[1]));assert x['status']=='PASS_STATIC_AABB_TOPK_CANARY_V2_DISKGUARD' and x['projection']['dimensions']==128 and x['snapshot_pre_post_byte_equal'] is True
...
o.write_text(json.dumps({'schema':'safe-c2-static-aabb-diskguard-v2-topk-manifest','status':'COMPLETE_CANARY_ONLY','scope':'SIFT-integer 128d disk-guard exact top-K canary; no timing/formal claim','binary':{'path':str(b),'sha256':h(b)},'artifacts':{n:{'sha256':h(r/n),'bytes':(r/n).stat().st_size} for n in ns},'gpu_binary_executed':True,'formal_claim_eligible':False},sort_keys=True,indent=2)+'\n')
PY
trap - ERR
echo "$RUN"
