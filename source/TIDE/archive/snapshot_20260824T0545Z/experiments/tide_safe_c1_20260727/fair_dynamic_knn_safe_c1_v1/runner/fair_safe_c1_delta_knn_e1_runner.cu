// Independent E1 frozen-base KNN projection runner.
//
// Correctness scope only.  The frozen base is constructed once through the
// byte-identical v24 compact GTS core, and every trace insertion is retained
// only in DeltaOnlyState::global_delta_.  This wrapper never calls the core's
// insert(), erase_mutable(), or rebuild_from_current_live() APIs: no old
// routing certificate, sidecar, direct C3 path, or native tree mutation is
// part of the trace semantics.
//
// A diagnostic gate established that the archived GTS receipt-selected KNN
// candidate path is not a full-base exact top-k oracle.  Therefore this
// strict-correctness wrapper deliberately uses the core's explicitly typed
// full-immutable-base range fallback for each
// projected KNN, selects its exact base top-k, then merges the exact global
// delta.  This is a correctness fallback, not a native-GTS-KNN or performance
// result.  The one private KNN/range bootstrap only satisfies v24's internal
// post-rebuild gate; it is not a projected trace operation.
//
// --mode correctness is the only implemented mode.  --mode timing fails
...
[[noreturn]] void fail(const std::string& message) {
  throw std::runtime_error("fair_delta_knn fail-stop: " + message);
}
...
Args parse_args(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (flag == "--bundle") args.bundle = value();
      ...
      std::cout << "usage: fair_safe_c1_delta_knn_e1_runner"
  ...
  require(args.mode == "correctness",
          "timing is not implemented; refuse GPU work without a separately admitted hot path");
  return args;
}
...
int run(const Args& args) {
  ...
  for (const TraceEvent& event : bundle.events) {
    ...
    // The receipt-selected GTS KNN path is intentionally not used here: the
    // diagnostic trace found a non-exact base candidate at op=5/query=113.  This typed fallback
    // scans only the immutable base and makes no traversal/performance claim.
    IssuedQuery exact_base_range = engine.query_range(
        event.argument, query, std::numeric_limits<DistanceSq>::max());
    require(!exact_base_range.post_rebuild_ticket.has_value(),
            "unexpected post-rebuild ticket during frozen-base replay");
    require(exact_base_range.export_data.kind == "range" &&
                exact_base_range.export_data.exact_full_immutable_base_range_fallback &&
                exact_base_range.export_data.full_immutable_base_candidate_count ==
                    static_cast<int>(bundle.header.base_n) &&
                exact_base_range.export_data.base_path ==
                    "exact_full_immutable_base_range_fallback_no_gts_receipt",
            "strict correctness replay did not use the typed immutable-base fallback");

    const auto expected_base_all = exact_topk(
        bundle.pool, state.immutable_base(), query, static_cast<int>(bundle.header.base_n));
    require_equal(exact_base_range.export_data.results, expected_base_all,
                  "exact immutable-base fallback at op=" + std::to_string(event.op_index) +
                      " query=" + std::to_string(event.argument));
    std::vector<StableDistance> exact_base_topk = exact_base_range.export_data.results;
    exact_base_topk.resize(static_cast<std::size_t>(bundle.header.k));
    const auto expected_base =
        exact_topk(bundle.pool, state.immutable_base(), query, static_cast<int>(bundle.header.k));
    require_equal(exact_base_topk, expected_base, "exact immutable-base top-k");
    ...
    const auto merged = merge_topk(exact_base_topk, exact_delta,
                                   static_cast<int>(bundle.header.k));
    const auto expected_full =
        exact_topk(bundle.pool, state.active(), query, static_cast<int>(bundle.header.k));
    require_equal(merged, expected_full, "base-plus-global-delta KNN");
    ...
    output << "{\"record\":\"knn\",\"op_index\":" << event.op_index
           ...
           << ",\"k\":" << bundle.header.k
           << ",\"base_path\":\"" << exact_base_range.export_data.base_path << "\""
           << ",\"base_query_mode\":\"exact_full_immutable_base_range_fallback\""
           << ",\"base_exact_full_immutable_fallback\":true"
           << ",\"full_immutable_base_candidate_count\":"
           << exact_base_range.export_data.full_immutable_base_candidate_count
           << ",\"native_gts_knn_used_for_trace\":false"
           << ",\"base_immutable\":true,\"direct_sidecar_used\":false"
           << ",\"global_delta_live\":" << state.global_delta().size()
           << ",\"base_exact_topk\":";
    write_results(output, exact_base_topk);
    output << ",\"merged\":";
  ...
  }
  {
    ...
    summary << "{\"schema\":\"fair-safe-c1-delta-knn-correctness-v3\""
            << ",\"mode\":\"correctness\""
...
int main(int argc, char** argv) {
  try {
    return fair_delta_knn::run(fair_delta_knn::parse_args(argc, argv));
