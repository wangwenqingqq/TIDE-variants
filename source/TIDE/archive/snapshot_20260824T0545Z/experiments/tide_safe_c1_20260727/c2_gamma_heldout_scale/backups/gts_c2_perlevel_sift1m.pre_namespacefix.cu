...
ReferenceCounters evaluate_results(const Fvecs& base, const Fvecs& all_queries, const std::vector<int>& gt,
  ...
  for (std::size_t local = 0; local < selected_ids.size(); ++local) {
    ...
    for (int rank = 0; rank < kK; ++rank) {
      ...
      if (first_occurrence && gt_topk.count(id) != 0) ++result.correct;
    }
  }
  return result;
}
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    if (args.mode == "calibrate") {
      ...
      upload_gamma(all_ones);
      paired_baseline_result = run_stage(runtime, frozen, selected_queries_d, selected_count, base, all_queries, gt,
                                         selected_ids, args.warmup_reps, args.timed_reps);
      has_paired_baseline = true;
      if (paired_baseline_result.repetitions.empty()) fail("missing paired baseline result");
      baseline_target = paired_baseline_result.repetitions.front().correct;
      for (const auto& counters : paired_baseline_result.repetitions) {
        if (!counters.valid() || counters.correct != baseline_target) {
          fail("paired baseline is invalid or nondeterministic; C2 holdout stage is blocked");
        }
      }
      upload_gamma(gamma);
      stage_result = run_stage(runtime, frozen, selected_queries_d, selected_count, base, all_queries, gt,
    ...
    write_summary(args.output_dir + "/summary.json", args, build, runtime, metric, query_h2d, gamma, stage_result,
