...
if protocol.get("candidate", {}).get("stable_id") != 4099:
    fail("protocol candidate binding")
if protocol.get("candidate", {}).get("expected_route") != [0, 7, 78, 790]:
    fail("protocol route binding")
if protocol.get("mutation_whitelist", {}).get("tn_size_increment_nodes") != [0, 7, 78, 790]:
    fail("protocol size whitelist")
if protocol.get("mutation_whitelist", {}).get("max_dis_update", {}).get("node") != 78:
...
required = [
    ...
    "kSafeCandidateStableId = 4099",
    "kSafeExpectedLeaf = 790",
    "kSafeExpectedRoute{{0, 7, 78, 790}}",
...
report = {
    "schema": "safe-c3-static-audit-v1",
