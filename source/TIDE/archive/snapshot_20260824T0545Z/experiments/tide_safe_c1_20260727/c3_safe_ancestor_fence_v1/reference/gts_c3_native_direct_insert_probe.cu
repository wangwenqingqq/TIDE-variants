...
int main(int argc, char** argv) {
  ...
  try {
    ...
    // Make the actual static API probe a no-residual baseline query.  This is
    // checked after the device upload rather than assumed from startup state.
    ...
    output << "{\"record\":\"meta\",\"schema\":\"c3-native-certified-direct-insert-runner-v1\","
           ...
           << "\"archived_incremental_updater_used\":false,\"residual_pruning_mode\":0,"
           << "\"residual_pruning_runtime_verified\":"
           << (residual_mode_verified ? "true" : "false")
    ...
    if (accepted == 0) {
      write_summary(args.summary, "INCONCLUSIVE_NO_CERTIFIED_INSERT", "no candidate passed strict certificate/capacity gate",
                    &header, candidates, accepted, rejected, &plan, residual_mode_verified);
    ...
    if (!args.summary.empty()) write_summary(args.summary, "FAIL_RUNNER_EXCEPTION", error.what(),
                                             ...
                                             plan.infi_dis ? &plan : nullptr, residual_mode_verified);
    std::cerr << error.what() << '\n';
    release_runtime(runtime);
    return 2;
  }
