#!/usr/bin/env python3
"""Strict CPU-only verifier for the separately outer-guarded v7 Nsight profile.
...
if _contract_spec is None or _contract_spec.loader is None:
    raise RuntimeError("cannot load v7 capacity snapshot contract")
...
def expected_plan(pins: dict[str, Any]) -> dict[str, Any]:
    ...
    need(plan.get("schema") == "gtspp-c1-v7-profile-execution-plan-v1", "profile plan schema")
...
def verify_child(root: pathlib.Path, pins: dict[str, Any], uuid: str, outer_pci: str, child: dict[str, Any], artifact_record: dict[str, Any], expected_variant: str) -> dict[str, Any]:
    ...
    need(card.get("schema") == "gtspp-c1-microbench-run-card-v7" and done.get("schema") == "gtspp-c1-microbench-completion-v7", variant + ": schemas")
...
def main() -> int:
    ...
    need(root.parent == PROFILES_ROOT and root.name.startswith("c1_v7_profile_"), "profile root must be direct c1_v7_profile_* child of v7 profiles")
    pins_path = raw_file(str(args.pins), "pins")
    need(pins_path == PINS_PATH, "pins path must be canonical v7 hardened pin file")
    pins = load(pins_path, "pins")
    need(pins.get("schema") == "gtspp-c1-v7-pins-v1", "pins schema")
    run_pin_verifier(pathlib.Path(ROOT_DEFAULT), pins_path, pins)
    plan = expected_plan(pins)
    manifest = load(root / "profile_run_manifest_v7.json", "profile run manifest")
    guard = load(root / "profile_guard_card_v7.json", "profile guard card")
    need(manifest.get("schema") == "gtspp-c1-v7-profile-run-manifest-v1" and manifest.get("execution_mode") == "PROFILE" and manifest.get("status") == "COMPLETE", "profile manifest mode/status")
    ...
    need(launcher == pathlib.Path(ROOT_DEFAULT) / "launch_c1_profile_v7.sh" and sha(launcher) == manifest.get("trust_root_launcher", {}).get("sha256"), "profile launcher provenance")
    ...
    need(guard.get("schema") == "gtspp-c1-v7-profile-guard-card-v1" and guard.get("status") == "COMPLETE" and guard.get("execution_mode") == "PROFILE" and guard.get("physical_gpu_index") == 0 and guard.get("physical_gpu_uuid") == uuid, "profile guard card provenance")
    ...
    root_dirs = {entry.name for entry in root.iterdir() if entry.is_dir()}
    need(root_dirs == set(PRIMARY) | {"gpu_snapshots"}, "profile root directory set")
    ...
    output = {
        "schema": "gtspp-c1-v7-profile-verification-v4",
        "verified_utc": datetime.now(timezone.utc).isoformat(),
        "profile_root": str(root),
        "pins_sha256": sha(pins_path),
        "profile_run_manifest_sha256": sha(root / "profile_run_manifest_v7.json"),
        "profile_guard_card_sha256": sha(root / "profile_guard_card_v7.json"),
        "gpu_uuid": uuid,
    ...
    target = root / "profile_verification_v7.json"
