#!/usr/bin/env python3
"""Fail-closed CPU-only source/pin verifier for the C1-v7 profile repair."""
...
PINS_DEFAULT = ROOT_DEFAULT / "hardened_source_pins_v7.json"
EXTERNALS = {
...
PROTOCOL_SCHEMAS = {
    "formal_supersession": "gtspp-c1-v7-capacity-snapshot-supersession-v1",
    "frozen_protocol": "gtspp-c1-v7-workspace-onequery-microbenchmark-protocol-v1",
    "gpu_uuid_safety": "gtspp-c1-v7-gpu-uuid-safety-v1",
    "idle_poll_safety": "gtspp-c1-v7-gpu-idle-poll-safety-v1",
    "idle_poll_logdir": "gtspp-c1-v7-gpu-idle-poll-logdir-v1",
    "implementation_correction": "gtspp-c1-v7-capacity-snapshot-implementation-correction-v1",
    "execution_plan": "gtspp-c1-v7-measured-execution-plan-v1",
    "profile_execution_plan": "gtspp-c1-v7-profile-execution-plan-v1",
}
HELPERS = {
...
FIXTURES = {
    ...
    "profile_guard_cli_contract_execution",
}
...
def entry_file(entry: Any, label: str, root: pathlib.Path, allowed: set[pathlib.Path]) -> pathlib.Path:
    ...
    if not under(path, root) and path not in allowed:
        raise ValueError(f"{label}: path outside v7 root / external allowlist: {path}")
...
def verify_protocols(pins: dict[str, Any], root: pathlib.Path) -> None:
    ...
    for key, schema in PROTOCOL_SCHEMAS.items():
        ...
        if obj.get("schema") != schema:
            raise ValueError(f"protocol/{key}: v7 schema mismatch")
        if key not in {"execution_plan", "profile_execution_plan"} and obj.get("v7_local_root", obj.get("root")) != str(root):
            raise ValueError(f"protocol/{key}: v7 root binding mismatch")
    capacity = {
...
def verify_trace_provenance(pins: dict[str, Any], root: pathlib.Path) -> None:
    ...
    if provenance.get("schema") != "gtspp-c1-query-only-trace-provenance-v2" or provenance.get("v7_local_root") != str(root):
        raise ValueError("derived trace provenance v7 binding/schema mismatch")
    ...
    correction = provenance.get("v7_capacity_snapshot_correction")
...
def forbid_stale_text(paths: list[pathlib.Path]) -> None:
    stale = ("v" + "5", "V" + "5", "v" + "6", "V" + "6", "c1_workspace_onequery_microbenchmark_v7_" + "capacity_snapshot")
    for path in paths:
        if path.suffix not in {".py", ".sh", ".json", ".cu", ".cuh", ".cpp", ".txt"}:
            continue
        text = path.read_text(errors="strict")
        if any(token in text for token in stale):
            raise ValueError(f"stale version/root identity in v7 artifact: {path}")
...
def run_review_fail_closed_launcher(path: pathlib.Path) -> None:
    ...
    if result.returncode == 0 or "review-required v7 launcher" not in combined:
...
def static_policy(root: pathlib.Path, pins: dict[str, Any]) -> None:
    ...
    semantic = (root / "verify_c1_semantics_v7.py").read_text()
    profile = (root / "verify_c1_profile_artifacts_v7.py").read_text()
    analyzer = (root / "analyze_c1_microbenchmark_v7.py").read_text()
    contract = (root / "capacity_snapshot_contract_v7.py").read_text()
    runtime = (root / "verify_v7_pins.py").read_text()
    profile_manifest = (root / "c1_v7_profile_manifest.py").read_text()
    profile_guard = (root / "run_c1_profile_guard_v7.sh").read_text()
    manifest_fixture = root / "tools" / "test_c1_v7_profile_manifest_fixtures.py"
    terminal_fixture = root / "tools" / "test_c1_v7_profile_terminal_fixture.py"
    session_fixture = root / "tools" / "test_c1_v7_guard_session_fixtures.py"
    ...
    for name, text in (("semantic", semantic), ("profile", profile), ("analyzer", analyzer)):
        if "capacity_snapshot_violations" not in text or "capacity_snapshot_contract_v7.py" not in text:
    ...
    for token in ("assert_capacity_snapshot_source_policy", "global update_result_ws_cap read after release", "capacity_snapshot_stage", "profile_manifest_fixture", "profile_terminal_fixture", "profile_guard_cli_fixture"):
    ...
    for token in ("--repfile", "--stats-stdout", "args.repfile", "no_um_events_observed", "UM_NO_PAGE_FAULT_MARKER", "_require_empty_um_witness"):
        if token not in profile_manifest:
            raise ValueError(f"profile manifest lacks strict v7 artifact token: {token}")
    ...
    for guard_name, guard_text, bootstrap in (
        ("measured", measured_guard, "C1-V7-MEASURED-OWNED-BOOTSTRAP"),
        ("profile", profile_guard, "C1-V7-PROFILE-OWNED-BOOTSTRAP"),
    ):
        for token in (
    ...
    for token in ("--repfile", "--stats-stdout", "PROFILE_MANIFEST_FIXTURE", "PROFILE_TERMINAL_FIXTURE", "--fixture-terminal-failure", "MANIFEST_READY", "FINAL_STATUS='FAILED'", "never allocation evidence"):
        if token not in profile_guard:
            raise ValueError(f"profile guard lacks v7 failure/UM policy token: {token}")
    for token in ("nsys_stats_stdout", "no_um_events_observed", "EMPTY RESULTS", "not allocation evidence", "profile-verification-v4"):
        if token not in profile:
            raise ValueError(f"profile verifier lacks v7 UM contract token: {token}")
    for name, text in (
        ("capacity fixture", (root / "tools" / "test_c1_v7_capacity_snapshot_fixtures.py").read_text()),
        ...
        ("profile guard CLI fixture", guard_cli_fixture.read_text()),
        ("semantic", semantic), ("profile", profile), ("analyzer", analyzer),
        ("source verifier", (root / "verify_v7_source_pins.py").read_text()),
        ("source generator", (root / "create_v7_source_pins.py").read_text()),
    ):
    ...
    build_script = (root / "build_v7_variants.sh").read_text()
    if 'python3 -B "$ROOT/create_v7_build_manifest.py"' not in build_script:
        raise ValueError("build manifest creator lacks explicit -B")
    for guard in ((root / "run_c1_workspace_microbenchmark_guard_v7.sh").read_text(), profile_guard):
        if '"$PYTHON" -I' in guard or '"$PYTHON" -B -I' not in guard:
            raise ValueError("guard has an unprotected static Python invocation")
    for launcher_name in ("launch_c1_v7.sh", "launch_c1_profile_v7.sh"):
        launcher = (root / launcher_name).read_text()
        if "REVIEW_REQUIRED_V7" not in launcher or "review-required v7 launcher" not in launcher:
    ...
    run_cpu_fixture(guard_cli_fixture, root, "profile-guard-cli-contract")
    run_review_fail_closed_launcher(root / "launch_c1_v7.sh")
    run_review_fail_closed_launcher(root / "launch_c1_profile_v7.sh")
...
def main() -> int:
    ...
    try:
        ...
        if root != ROOT_DEFAULT or pins_path != PINS_DEFAULT:
            raise ValueError("canonical v7 source root/pins path required")
        pins = load(pins_path, "pins")
        if pins.get("schema") != "gtspp-c1-v7-source-pins-v1" or pins.get("root") != str(root) or pins.get("source_only") is not True:
        ...
        raise SystemExit(f"C1-V7-SOURCE-PINS BLOCKED: {type(exc).__name__}: {exc}")
