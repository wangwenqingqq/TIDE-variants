#!/usr/bin/env python3
"""Fail-closed CPU-only integrity and source-policy verifier for C1 v7."""
from __future__ import annotations
import sys
sys.dont_write_bytecode = True
import argparse, hashlib, json, pathlib, re, subprocess

ROOT_DEFAULT="/workspace/experiments/tide_safe_c1_20260727/c1_workspace_onequery_microbenchmark_v7_profile_cpu_preflight"
VARIANTS={
...
def verify_profile_execution_plan(pins:dict)->None:
    ...
    if plan.get("schema")!="gtspp-c1-v7-profile-execution-plan-v1": raise SystemExit("profile execution plan schema")
    ...
    if nsys.get("artifact_layout")!=expected_layout: raise SystemExit("profile execution artifact layout")
    if plan.get("um_zero_byte_contract")!=expected_um: raise SystemExit("profile execution UM zero-byte contract")
...
def verify_trace_provenance(pins:dict,root:pathlib.Path)->None:
    """Fail before GPU launch if the v7-local trace provenance is inconsistent."""
...
def static_source_policy(root:pathlib.Path)->None:
    ...
    for token in (
        ...
        if token not in c1: raise SystemExit(f"missing C1 v7 policy token {token!r}")
    ...
    measured_guard=(root/"run_c1_workspace_microbenchmark_guard_v7.sh").read_text()
    measured_verifier=(root/"verify_c1_semantics_v7.py").read_text()
    profile_guard=(root/"run_c1_profile_guard_v7.sh").read_text()
    profile_manifest=(root/"c1_v7_profile_manifest.py").read_text()
    profile_verifier=(root/"verify_c1_profile_artifacts_v7.py").read_text()
    ...
    guard_session_fixture=root/"tools/test_c1_v7_guard_session_fixtures.py"
    guard_cli_fixture=root/"tools/test_c1_v7_profile_guard_cli_contract.py"
    finalizer=(root/"finalize_c1_evidence_v7.py").read_text()
    aggregator=(root/"aggregate_c1_formal_evidence_v7.py").read_text()
    for guard_name, guard_text, bootstrap in (("measured",measured_guard,"C1-V7-MEASURED-OWNED-BOOTSTRAP"),("profile",profile_guard,"C1-V7-PROFILE-OWNED-BOOTSTRAP")):
        for token in ('"$SETSID" --wait /bin/bash -c',bootstrap,"adopt_own_child_session_from_ready","allow_and_verify_owned_child_exec","verify_own_child_session","CHILD_SESSION_PID","CHILD_PGID","CHILD_SID","TERM_OWN_VERIFIED_PROCESS_GROUP","REFUSED_UNVERIFIED_PROCESS_GROUP_HOLDING_LOCK","HOLD_LOCK_UNTIL_SESSION_EXIT","ensure_manifest_ready_from_disk","--fixture-first-variant-midfailure","CPU_ONLY_FIXTURE_ACTIVE"):
            if token not in guard_text: raise SystemExit(f"{guard_name} guard lacks owned-session/manifest-race policy token {token!r}")
        if 'CHILD_PID=' in guard_text or 'kill -TERM -- "-$CHILD_PID"' in guard_text:
            raise SystemExit(f"{guard_name} guard retains unsafe background-PID process-group termination")
    ...
    for token in ("C1_V7_TRUST_ROOT=''","C1_V7_LAUNCHER=''","C1_V7_TRUST_GUARD_SHA=''","C1_V7_TRUST_PINS_SHA=''","C1_V7_TRUST_PIN_VERIFY_SHA=''","C1_ALLOW_GPU0=''","C1_RUN_OUT=''"):
        if token in measured_guard: raise SystemExit(f"measured guard erases inherited trust environment {token!r}")
    for token in ("C1_V7_PROFILE_TRUST_ROOT=''","C1_V7_PROFILE_LAUNCHER=''","C1_V7_PROFILE_TRUST_GUARD_SHA=''","C1_V7_PROFILE_TRUST_PINS_SHA=''","C1_V7_PROFILE_TRUST_PIN_VERIFY_SHA=''","C1_ALLOW_GPU0=''","C1_PROFILE_OUT=''"):
        if token in profile_guard: raise SystemExit(f"profile guard erases inherited trust environment {token!r}")
    for token in ("visible_cuda_device_uuid","CUDA UUID must exactly match manifest/session UUID","telemetry PCI discontinuity","capacity_snapshot_violations","capacity_snapshot_contract_v7.py"):
        if token not in measured_verifier: raise SystemExit(f"missing measured verifier binding token {token!r}")
    for token in ("--trace=cuda","--cuda-memory-usage=true","--profile-only","profile_nsys_stats_complete","um_sum","C1_V7_PROFILE_TRUST_GUARD_SHA","profile_execution_plan_v7.json","CUDA UUID/session","--repfile","--stats-stdout","PROFILE_MANIFEST_FIXTURE","PROFILE_TERMINAL_FIXTURE","--fixture-terminal-failure","MANIFEST_READY","FINAL_STATUS='FAILED'","never allocation evidence"):
        if token not in profile_guard: raise SystemExit(f"missing profile guard policy token {token!r}")
    for token in ("profile_run_manifest_v7.json","nsys_argv","profile_artifacts","profile.sqlite","um_sum","args.repfile","--repfile","--stats-stdout","no_um_events_observed","UM_NO_PAGE_FAULT_MARKER"):
        if token not in profile_manifest: raise SystemExit(f"missing profile manifest policy token {token!r}")
    if "pathlib.Path(a.rep)" in profile_manifest: raise SystemExit("predecessor int-to-Path profile bug remains")
    for token in ("profile_run_manifest_v7.json","outer guarded nsys invocation manifest","profile_nsys_stats_complete","SQLite format 3","um_sum","no_um_events_observed","nsys_stats_stdout","EMPTY RESULTS","not allocation evidence","CUDA UUID differs from manifest/session UUID","profile telemetry PCI differs from outer prelaunch","capacity_snapshot_violations","capacity snapshot"):
        if token not in profile_verifier: raise SystemExit(f"missing profile verifier policy token {token!r}")
    for token in ("gtspp-c1-v7-measured-evidence-v2","gpu_pci_bus_id_normalized"):
        if token not in finalizer: raise SystemExit(f"missing measured finalizer binding token {token!r}")
    for token in ("gtspp-c1-v7-profile-verification-v4","cross-root GPU PCI disagreement","gtspp-c1-v7-formal-evidence-aggregate-v3","aggregate self pin/provenance mismatch","aggregate_script_sha256","SELF_PATH","um_zero_byte_interpretation"):
    ...
    # Execute real CPU-only fixture paths before a guard can touch GPU telemetry.
    for label,fixture_path in (("profile-manifest",profile_manifest_fixture),("profile-terminal",profile_terminal_fixture),("guard-session-ownership-and-manifest-race",guard_session_fixture),("profile-guard-cli-contract",guard_cli_fixture)):
        result=subprocess.run([sys.executable,"-B","-I",str(fixture_path),"--root",str(root)],text=True,capture_output=True,check=False)
        if result.returncode!=0 or '"pass": true' not in result.stdout:
            raise SystemExit(f"{label} CPU-only fixture failed: {result.stderr[-800:]}")

def verify_review_required_launcher_trust(root:pathlib.Path,pins_path:pathlib.Path)->None:
    """Bind inert launchers to this exact reviewed build closure without approving them.

    Launchers intentionally remain outside ``hardened_static_pins_v7.json`` to
    avoid a PINS<->launcher self-hash cycle.  This verifier is itself hash-bound
    by PINS and checks the two review-required launchers against the current
    PINS/guard/verifier hashes before any launcher can later be considered for
    independent human approval.
    """
    pin_hash=sha(pins_path)
    verifier=root/"verify_v7_pins.py"
    cases=(
        ("launch_c1_v7.sh", "run_c1_workspace_microbenchmark_guard_v7.sh", "GUARD_SHA", "C1-V7-LAUNCHER BLOCKED:"),
        ("launch_c1_profile_v7.sh", "run_c1_profile_guard_v7.sh", "PROFILE_GUARD_SHA", "C1-V7-PROFILE-LAUNCHER BLOCKED:"),
    )
    for launcher_name,guard_name,guard_hash_key,blocked_prefix in cases:
        launcher=safe_file(str(root/launcher_name),"launcher "+launcher_name)
        guard=safe_file(str(root/guard_name),"launcher guard "+launcher_name)
        text=launcher.read_text(errors="strict")
        expected_lines=(
            f"ROOT='{root}'",
            f'LAUNCHER="$ROOT/{launcher_name}"',
            f'GUARD="$ROOT/{guard_name}"',
            'PINS="$ROOT/hardened_static_pins_v7.json"',
            'PIN_VERIFY="$ROOT/verify_v7_pins.py"',
            "REVIEW_STATE='REVIEW_REQUIRED_V7'",
            f"{guard_hash_key}='{sha(guard)}'",
            f"PINS_SHA='{pin_hash}'",
            f"PIN_VERIFY_SHA='{sha(verifier)}'",
        )
        for line in expected_lines:
            if text.count(line) != 1:
                raise SystemExit(f"{launcher_name}: review-required trust binding mismatch {line}")
        if "REVIEW_STATE='REVIEW_APPROVED_V7'" in text:
            raise SystemExit(f"{launcher_name}: launcher must remain REVIEW_REQUIRED during CPU-only closure")
        gate="[[ \"$REVIEW_STATE\" == 'REVIEW_APPROVED_V7' ]] ||"
        if gate not in text or blocked_prefix not in text or "review-required v7 launcher" not in text:
            raise SystemExit(f"{launcher_name}: review-required fail-closed gate missing")
        gate_pos=text.index(gate)
        regular_pos=text.index('regular "$LAUNCHER"')
        exec_pos=text.index('exec /usr/bin/env -i')
        if not gate_pos < regular_pos < exec_pos:
            raise SystemExit(f"{launcher_name}: review gate is not before runtime trust/exec")
...
def main()->int:
    ...
    if p.get("schema")!="gtspp-c1-v7-pins-v1" or p.get("root")!=str(root): raise SystemExit("pins schema/root mismatch")
    ...
    if bm.get("schema")!="gtspp-c1-v7-build-manifest-v1": raise SystemExit("build manifest schema")
    ...
    verify_review_required_launcher_trust(root,pins_path)
    if a.static_source: static_source_policy(root)
    print(json.dumps({"pass":True,"pins_sha256":sha(pins_path),"checked_unique":len(seen),"variant":a.variant,"static_source":a.static_source,"launcher_trust_state":"REVIEW_REQUIRED_V7"},sort_keys=True))
    return 0
