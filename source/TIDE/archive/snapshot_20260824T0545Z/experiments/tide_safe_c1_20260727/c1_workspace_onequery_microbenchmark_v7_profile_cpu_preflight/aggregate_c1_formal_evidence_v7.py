...
SELF_PATH=pathlib.Path(ROOT_DEFAULT)/"aggregate_c1_formal_evidence_v7.py"
...
def main()->int:
    ...
    if pins_path!=PINS_PATH:raise SystemExit("pins must be canonical v7 hardened pin file")
    self_path=raw_file(str(pathlib.Path(__file__)),"aggregate self")
    if self_path!=SELF_PATH:raise SystemExit("aggregate must execute from canonical v7 self path")
    mroot=raw_dir(str(a.measured_root),"measured root")
    proot=raw_dir(str(a.profile_root),"profile root")
    if mroot.parent!=RUNS_ROOT or not mroot.name.startswith("c1_v7_measured_"):raise SystemExit("measured root boundary")
    if proot.parent!=PROFILES_ROOT or not proot.name.startswith("c1_v7_profile_"):raise SystemExit("profile root boundary")
    pins=load(pins_path,"pins")
    if pins.get("schema")!="gtspp-c1-v7-pins-v1":raise SystemExit("pins schema")
    ...
    m=load(mroot/"final_evidence_v7.json","measured final evidence")
    p=load(proot/"profile_verification_v7.json","profile verification")
    ps=sha(pins_path)
    if m.get("schema")!="gtspp-c1-v7-measured-evidence-v2" or m.get("pass") is not True or m.get("formal_claim_eligible") is not False:raise SystemExit("measured evidence not in required pre-formal state")
    if m.get("run_root")!=str(mroot):raise SystemExit("measured evidence root mismatch")
    if p.get("schema")!="gtspp-c1-v7-profile-verification-v4" or p.get("pass") is not True or p.get("formal_profile_eligible") is not True:raise SystemExit("profile verification not pass")
    ...
    out={
        "schema":"gtspp-c1-v7-formal-evidence-aggregate-v3","created_utc":datetime.now(timezone.utc).isoformat(),"pins_sha256":ps,
        "aggregate_script":{"path":str(self_path),"sha256":sha(self_path)},"aggregate_script_sha256":sha(self_path),
        "measured_evidence":{"path":str(mroot/"final_evidence_v7.json"),"sha256":sha(mroot/"final_evidence_v7.json")},
        "profile_verification":{"path":str(proot/"profile_verification_v7.json"),"sha256":sha(proot/"profile_verification_v7.json")},
    ...
    target=proot/"formal_evidence_aggregate_v7.json"
