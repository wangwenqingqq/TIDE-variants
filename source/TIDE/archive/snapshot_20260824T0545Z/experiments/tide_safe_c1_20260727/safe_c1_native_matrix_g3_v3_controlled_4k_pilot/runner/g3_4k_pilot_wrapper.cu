...
constexpr char kReleaseRoot[] =
    ...
    "safe_c1_native_matrix_g3_v3_controlled_4k_pilot";
...
FixtureData load_and_validate_binary_fixture(const fs::path& fixture) {
  ...
  require(safe_c1_g3::stable_set_sha256(data.initial_base) == kInitialLiveSha,
...
std::vector<StableDistance> json_distance_pairs_field(const std::string& json,
  ...
  safe_c1_g3::require_canonical_stable_distances(output, label.c_str());
  return output;
}
...
std::vector<StableDistance> exact_oracle(const FixtureData& data,
  ...
  safe_c1_g3::sort_and_require_unique(&all, "independent signed-int64 L2 oracle");
  if (is_knn) {
  ...
  safe_c1_g3::require_canonical_stable_distances(all,
                                                  "independent signed-int64 L2 oracle output");
  return all;
}
...
void require_equal_results(const std::vector<StableDistance>& actual,
                           ...
                           const std::string& label) {
  if (actual != expected) {
    const std::string actual_sha = safe_c1_g3::stable_distance_vector_sha256(
        actual, "controlled-4k-actual-result");
    const std::string expected_sha = safe_c1_g3::stable_distance_vector_sha256(
        expected, "controlled-4k-independent-result");
...
std::string result_sha_for_attestation(const std::vector<StableDistance>& values) {
  return safe_c1_g3::stable_distance_vector_sha256(
      values, "safe-c1-g3-controlled-4k-independent-result-v1");
}
...
std::string mutation_attestation_json(const TraceOp& event, const Placement& placement,
                                      const std::set<StableId>& active) {
  // Keep update records compatible with validate_g3_fixture.py --engine-jsonl.
  // fixture_role is diagnostic only: the archived selector's historical
  // direct labels are never used as v3 placement evidence.
  const bool is_insert = event.op == "insert";
  require(is_insert || event.op == "delete",
          "mutation serializer received a non-mutation trace event");
  require(placement.kind == PlacementKind::kDelta,
          "closed-direct runtime must serialize delta placement only");
  require(placement.sidecar_leaf_id == -1,
          "closed-direct delta must not carry a sidecar leaf");
  std::ostringstream out;
  out << "{\"record\":\"update\",\"op\":\"" << event.op
      ...
      << ",\"placement\":\"delta\""
      << ",\"actual_placement\":\"delta\""
      << ",\"sidecar_leaf_id\":-1"
      << ",\"certificate_ok\":" << (placement.certificate.ok ? "true" : "false")
      << ",\"fallback_reason\":\"" << json_escape(placement.fallback_reason) << "\""
      << ",\"safe_policy_requires_delta\":true"
      << ",\"direct_runtime_guard_open\":false"
      << ",\"active_ids_sha256\":\""
      << safe_c1_g3::stable_set_sha256(
             std::vector<StableId>(active.begin(), active.end()))
      << "\"";
  if (!is_insert) {
    // The pre-delete Placement is the only authoritative prior placement.  It
    // is delta under the closed direct gate; this is checked above.
    out << ",\"prior_placement\":\"delta\"";
  }
  out << "}";
  return out.str();
}
...
fs::path create_run_directory_or_fail(int argc, char** argv) {
  require(argc == 3 && std::string_view(argv[1]) == "--run-dir",
          "usage is exactly: g3_4k_pilot_wrapper --run-dir <absolute-v3-runs-child>");
  ...
  const fs::path requested = fs::absolute(fs::path(argv[2])).lexically_normal();
  require(requested.parent_path() == runs_root,
          "run directory must be one new direct child of the fixed v3 runs directory");
  require(!requested.filename().empty() && requested.filename() != "." &&
              requested.filename() != "..",
          "run directory name is invalid");
  require(!fs::exists(requested), "run directory already exists; no reuse/retry is allowed");
  require(fs::create_directory(requested), "cannot create a new exclusive run directory");
  return requested;
}
...
void require_topology_receipt(const RebuildReceipt& receipt, const std::string& phase,
  ...
  require(receipt.node_count > 0, phase + " receipt has no native nodes");
  ...
  require(receipt.live_ids_sha256 ==
              safe_c1_g3::stable_set_sha256(std::vector<StableId>(active.begin(), active.end())),
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    run_dir = g3_4k_pilot::create_run_directory_or_fail(argc, argv);
    const std::filesystem::path engine_tmp = run_dir / ".engine.jsonl.tmp";
    const std::filesystem::path bootstrap_tmp = run_dir / ".bootstrap_attestation.json.tmp";
    const std::filesystem::path tickets_tmp = run_dir / ".ticket_attestation.jsonl.tmp";
    ...
    g3_4k_pilot::require(safe_c1_g3::stable_set_sha256(
    ...
    std::filesystem::rename(engine_tmp, run_dir / "engine.jsonl");
    std::filesystem::rename(bootstrap_tmp, run_dir / "bootstrap_attestation.json");
    std::filesystem::rename(tickets_tmp, run_dir / "ticket_attestation.jsonl");
    return 0;
  } catch (const std::exception& error) {
    // Fail closed: do not publish a partial engine/ticket/bootstrap artifact
    // and never transform a failed correctness gate into a performance result.
    if (!run_dir.empty()) {
      std::error_code ignored;
      std::filesystem::remove(run_dir / ".engine.jsonl.tmp", ignored);
      std::filesystem::remove(run_dir / ".bootstrap_attestation.json.tmp", ignored);
      std::filesystem::remove(run_dir / ".ticket_attestation.jsonl.tmp", ignored);
      std::filesystem::remove(run_dir, ignored);
    }
    std::cerr << error.what() << '\n';
    return 70;
  }
