...
MANIFEST_NAME="profile_run_manifest_v6.json"
...
def root_and_out(a:argparse.Namespace)->tuple[pathlib.Path,pathlib.Path]:
    root=secure_dir(a.root,"v6 root");out=secure_dir(a.out,"profile output")
    if root!=pathlib.Path(ROOT_DEFAULT) or out.parent!=root/"profiles" or not out.name.startswith("c1_v6_profile_"):
...
def init(a:argparse.Namespace)->None:
    ...
    if launcher!=root/"launch_c1_profile_v6.sh" or guard!=root/"run_c1_profile_guard_v6.sh":raise SystemExit("canonical profile trust path required")
    pin=load(pins,"pins")
    entries=[];flatten(pin,entries)
    if pin.get("schema")!="gtspp-c1-v6-pins-v1" or not entries:raise SystemExit("invalid pins")
    artifacts={}
    ...
    obj={
        "schema":"gtspp-c1-v6-profile-run-manifest-v1","created_utc":utc(),"updated_utc":utc(),"status":"PREPARED","execution_mode":"PROFILE",
