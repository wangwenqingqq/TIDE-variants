#!/usr/bin/env python3
"""CPU-only measured-only analysis after strict C1 v6 semantic verification."""
...
_contract_spec=importlib.util.spec_from_file_location("gtspp_c1_v6_capacity_snapshot_contract",ROOT_DEFAULT/"capacity_snapshot_contract_v6.py")
...
def main()->int:
    ...
    semantic=root/"semantic_verification_v6.json"
    ...
    if s.get("schema")!="gtspp-c1-v6-semantic-verification-v1" or s.get("pass") is not True:raise SystemExit("refusing statistics: strict semantic gate did not pass")
    if s.get("pins_sha256")!=sha(pins):raise SystemExit("refusing statistics: pins drifted after semantic gate")
    manifest=json.loads((root/"run_manifest_v6.json").read_text())
    ...
    result={"schema":"gtspp-c1-v6-analysis-v1","generated_utc":datetime.now(timezone.utc).isoformat(),
    ...
    out=root/"analysis_summary_v6.json";tmp=out.with_suffix(".tmp");tmp.write_text(json.dumps(result,indent=2,sort_keys=True)+"\n");tmp.replace(out)
