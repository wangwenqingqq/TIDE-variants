#!/usr/bin/env bash
# V6 outer guard: only this process launches C1Microbench.  There are no
...
PLAN="$ROOT/run_c1_four_variants_v6.sh"
PLAN_JSON="$ROOT/measured_execution_plan_v6.json"
PINS="$ROOT/hardened_static_pins_v6.json"
PIN_VERIFY="$ROOT/verify_v6_pins.py"
MANIFEST="$ROOT/c1_v6_manifest.py"
SEMANTIC="$ROOT/verify_c1_semantics_v6.py"
ANALYZER="$ROOT/analyze_c1_microbenchmark_v6.py"
FINALIZER="$ROOT/finalize_c1_evidence_v6.py"
...
die() { printf 'C1-V6-GUARD BLOCKED: %s\n' "$*" >&2; exit 69; }
...
write_guard_card() {
  ...
  status="$status" reason="$reason" GPU_UUID="$SESSION_UUID" "$PYTHON" -B -I - "$C1_RUN_OUT/guard_run_card_v6.json" <<'PY'
...
old.update({"schema":"gtspp-c1-v6-guard-card-v2",
 ...
 "manifest":str(p.parent/"run_manifest_v6.json")})
...
need(card.get("schema")=="gtspp-c1-microbench-run-card-v6","run-card schema")
need(done.get("schema")=="gtspp-c1-microbench-completion-v6","completion schema")
...
cleanup() {
  ...
  if [[ "$SESSION_READY" == 1 ]]; then
    ...
    if ! rm -f -- "$C1_RUN_OUT/.c1_guard_session_v6.json"; then rc=69; FINAL_STATUS='SESSION_CLEANUP_INCOMPLETE'; FINAL_REASON='session removal failed'; fi
    ...
    if [[ "$rc" -eq 0 && "$FINAL_STATUS" == COMPLETE ]]; then
      ...
      elif ! "$PYTHON" -B -I "$FINALIZER" --run-root "$C1_RUN_OUT" --pins "$PINS"; then rc=69; FINAL_STATUS='FINAL_EVIDENCE_FAILED'; FINAL_REASON='final evidence attestation failed'
      fi
...
C1_V6_TRUST_ROOT="$(read_env C1_V6_TRUST_ROOT)"
C1_V6_LAUNCHER="$(read_env C1_V6_LAUNCHER)"
C1_V6_TRUST_GUARD_SHA="$(read_env C1_V6_TRUST_GUARD_SHA)"
C1_V6_TRUST_PINS_SHA="$(read_env C1_V6_TRUST_PINS_SHA)"
C1_V6_TRUST_PIN_VERIFY_SHA="$(read_env C1_V6_TRUST_PIN_VERIFY_SHA)"
C1_ALLOW_GPU0="$(read_env C1_ALLOW_GPU0)"
RUN_OUT="$(read_env C1_RUN_OUT)"
[[ "$0" == "$GUARD" && "$C1_V6_TRUST_ROOT" == YES && "$C1_V6_LAUNCHER" == "$LAUNCHER" ]] || die 'reviewed trust-root required'
...
[[ "$(sha "$GUARD")" == "$C1_V6_TRUST_GUARD_SHA" && "$(sha "$PINS")" == "$C1_V6_TRUST_PINS_SHA" && "$(sha "$PIN_VERIFY")" == "$C1_V6_TRUST_PIN_VERIFY_SHA" ]] || die 'trust-root bootstrap hash mismatch'
...
is_under "$OUT" "$BASE" && [[ "$(basename "$OUT")" == c1_v6_measured_* && ! -e "$OUT" && ! -L "$OUT" ]] || die 'unsafe output'
...
"$PYTHON" -B -I - "$C1_RUN_OUT/.c1_guard_session_v6.json" "$SESSION_UUID" <<'PY'
import json,pathlib,sys
p=pathlib.Path(sys.argv[1]); p.write_text(json.dumps({"schema":"gtspp-c1-v6-session-v2","physical_gpu_index":0,"physical_gpu_uuid":sys.argv[2]})+"\n")
PY
chmod 0600 "$C1_RUN_OUT/.c1_guard_session_v6.json" || die 'session chmod'
SESSION_READY=1
