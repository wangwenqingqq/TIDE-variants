#!/usr/bin/env python3
"""Create the v6 source-only integrity root without compiling or launching CUDA."""
...
PROTOCOL_PATHS = {
    "formal_supersession": "protocol_c1_v6_formal_supersession_v1.json",
    "frozen_protocol": "protocol_c1_workspace_onequery_sift1m_v1_v6_local.json",
    "gpu_uuid_safety": "protocol_c1_gpu_uuid_safety_v6_local.json",
    "idle_poll_safety": "protocol_c1_gpu_idle_poll_safety_v6_local.json",
    "idle_poll_logdir": "protocol_c1_gpu_idle_poll_logdir_v6_local.json",
    "implementation_correction": "implementation_correction_capacity_snapshot_v1.json",
    "execution_plan": "measured_execution_plan_v6.json",
    "profile_execution_plan": "profile_execution_plan_v6.json",
}
HELPER_PATHS = {
    "guard": "run_c1_workspace_microbenchmark_guard_v6.sh",
    "measured_engine": "run_c1_four_variants_v6.sh",
    "profile_engine": "run_c1_profile_primary_v6.sh",
    "profile_guard": "run_c1_profile_guard_v6.sh",
    "profile_manifest_utility": "c1_v6_profile_manifest.py",
    "strict_semantic_verifier": "verify_c1_semantics_v6.py",
    "measured_analyzer": "analyze_c1_microbenchmark_v6.py",
    "profile_verifier": "verify_c1_profile_artifacts_v6.py",
    "measured_finalizer": "finalize_c1_evidence_v6.py",
    "formal_aggregator": "aggregate_c1_formal_evidence_v6.py",
    "manifest_utility": "c1_v6_manifest.py",
    "runtime_pin_verifier": "verify_v6_pins.py",
    "runtime_pin_generator": "create_v6_pins.py",
    "build_script": "build_v6_variants.sh",
    "build_manifest_creator": "create_v6_build_manifest.py",
    "capacity_snapshot_contract": "capacity_snapshot_contract_v6.py",
    "capacity_snapshot_fixture": "tools/test_c1_v6_capacity_snapshot_fixtures.py",
    "source_pin_verifier": "verify_v6_source_pins.py",
    "source_pin_generator": "create_v6_source_pins.py",
    "canonical_inspector_source": "tools/canonical_inspect_v6.cpp",
}
FIXTURE_PATHS = {
...
def main() -> int:
    ...
    if root != ROOT_DEFAULT:
        raise SystemExit("source pins must bind the canonical v6 root")
    ...
    payload = {
        "schema": "gtspp-c1-v6-source-pins-v1",
        "created_utc": datetime.now(timezone.utc).isoformat(),
        "root": str(root),
        "scope": "Source-only v6 capacity-snapshot integrity root. No NVCC build, CUDA binary, nsys, nvidia-smi, GPU query, or workload is represented or authorized by this pin set.",
        "source_only": True,
    ...
    out = args.out or root / "hardened_source_pins_v6.json"
    if not out.is_absolute() or out.is_symlink() or out.resolve(strict=False) != out or out.parent != root:
        raise SystemExit("source pin output must be a direct canonical file under the v6 root")
    tmp = out.with_name(out.name + ".tmp")
    tmp.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n")
    tmp.replace(out)
    print(json.dumps({"pass": True, "mode": "SOURCE_PIN_GENERATION_CPU_ONLY", "pins": str(out), "sha256": sha(out), "worktree_sources": len(worktree_entries), "helpers": len(helpers)}, sort_keys=True))
    return 0
