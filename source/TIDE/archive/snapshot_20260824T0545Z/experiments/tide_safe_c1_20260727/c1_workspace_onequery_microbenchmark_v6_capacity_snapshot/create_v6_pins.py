#!/usr/bin/env python3
"""Create the complete CPU-only C1 v6 integrity pin set after a successful build."""
...
HELPER_PATHS={
    ...
    "pin_verifier":"verify_v6_pins.py",
    "pin_generator":"create_v6_pins.py",
...
def validate_v6_trace_provenance(inputs:dict,protocols:dict)->None:
...
def validate_profile_execution_plan(protocols:dict)->None:
    ...
    if plan.get("schema")!="gtspp-c1-v6-profile-execution-plan-v1": raise SystemExit("profile plan schema")
...
def main()->None:
    ...
    build=root/"worktree_build_manifest_v6.json"
    ...
    validate_v6_trace_provenance(inputs,protocols)
    ...
    out={
        "schema":"gtspp-c1-v6-pins-v1",
        "created_utc":datetime.now(timezone.utc).isoformat(),
        "root":str(root),
        "scope":"All v6 C1 artifacts pinned after CPU/NVCC compilation only; no CUDA binary, nsys, nvidia-smi, or GPU workload executed.",
    ...
    target=a.out or root/"hardened_static_pins_v6.json"
