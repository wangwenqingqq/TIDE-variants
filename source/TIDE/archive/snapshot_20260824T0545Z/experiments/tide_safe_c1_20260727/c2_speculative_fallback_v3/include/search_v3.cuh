// Search v3. Speculative gamma traversal with gamma-only fallback diagnostics.
...
// V3 diagnostic only: a gamma-only prune is a node that the certified
								   ...
								   int *gamma_only_pruned, GammaOnlyPruneTrace *gamma_only_trace)
{
	...
	for (int i = tid; i < nnum_l * qnum_l; i += total_num)
	{
		...
		if (needs_process)
		{
			...
			// V3: gamma is speculative only. Keep the exact Eq. (1) condition
...
void searchIndexKnnV2(short *data_d, TN *node_list, int *id_list, int *max_node_num, float *query_data,
				  int *res_ids, int qnum, int k, int tree_h, int *data_info, int *empty_list, char *data_s, int *size_s)
{
	// V3 fallback baseline path: every CUDA/thrust failure below is fatal.
	cout << "Searching..." << endl;
...
// V3 speculative vector+IDs traversal. It records gamma-only pruning but does
