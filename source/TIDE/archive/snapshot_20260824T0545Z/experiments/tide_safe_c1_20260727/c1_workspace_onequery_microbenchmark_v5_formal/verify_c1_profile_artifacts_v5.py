#!/usr/bin/env python3
"""Strict CPU-only verifier for the separately outer-guarded v5 Nsight profile."""
...
def run_pin_verifier(root:pathlib.Path,pins_path:pathlib.Path,pins:dict[str,Any])->None:
    ...
    r=subprocess.run([sys.executable,"-I",verifier["path"],"--root",str(root),"--pins",str(pins_path),"--static-source"],text=True,capture_output=True)
    need(r.returncode==0,"pinned static verifier failed: "+r.stderr[-500:])
    for v in PRIMARY:
        r=subprocess.run([sys.executable,"-I",verifier["path"],"--root",str(root),"--pins",str(pins_path),"--variant",v],text=True,capture_output=True)
...
def expected_plan(pins:dict[str,Any])->dict[str,Any]:
    ...
    need(plan.get("schema")=="gtspp-c1-v5-profile-execution-plan-v1","profile plan schema")
...
def verify_child(root:pathlib.Path,pins:dict[str,Any],manifest:dict[str,Any],uuid:str,outer_pci:str,plan:dict[str,Any],child:dict[str,Any],artifact_record:dict[str,Any],expected_variant:str)->dict[str,Any]:
    ...
    need(card.get("schema")=="gtspp-c1-microbench-run-card-v5" and done.get("schema")=="gtspp-c1-microbench-completion-v5",v+": schemas")
...
def main()->int:
    ...
    need(root.parent==PROFILES_ROOT and root.name.startswith("c1_v5_profile_"),"profile root must be direct c1_v5_profile_* child of v5 profiles")
    pins_path=raw_file(str(a.pins),"pins");need(pins_path==PINS_PATH,"pins path must be canonical v5 hardened pin file")
    pins=load(pins_path,"pins");need(pins.get("schema")=="gtspp-c1-v5-pins-v1","pins schema")
    run_pin_verifier(pathlib.Path(ROOT_DEFAULT),pins_path,pins)
    plan=expected_plan(pins)
    manifest=load(root/"profile_run_manifest_v5.json","profile run manifest");guard=load(root/"profile_guard_card_v5.json","profile guard card")
    need(manifest.get("schema")=="gtspp-c1-v5-profile-run-manifest-v1" and manifest.get("execution_mode")=="PROFILE" and manifest.get("status")=="COMPLETE","profile manifest mode/status")
    ...
    need(launcher==pathlib.Path(ROOT_DEFAULT)/"launch_c1_profile_v5.sh" and sha(launcher)==manifest.get("trust_root_launcher",{}).get("sha256"),"profile launcher provenance")
    ...
    need(guard.get("schema")=="gtspp-c1-v5-profile-guard-card-v1" and guard.get("status")=="COMPLETE" and guard.get("execution_mode")=="PROFILE" and guard.get("physical_gpu_index")==0 and guard.get("physical_gpu_uuid")==uuid,"profile guard card provenance")
    ...
    out={"schema":"gtspp-c1-v5-profile-verification-v3","verified_utc":datetime.now(timezone.utc).isoformat(),"profile_root":str(root),"pins_sha256":sha(pins_path),"profile_run_manifest_sha256":sha(root/"profile_run_manifest_v5.json"),"profile_guard_card_sha256":sha(root/"profile_guard_card_v5.json"),"gpu_uuid":uuid,"gpu_pci_bus_id_normalized":outer_pci,"pass":True,"formal_profile_eligible":True,"profiles":rows,"warning":"Explicitly unmeasured Nsight allocation evidence only; no profile value is timing data and this verifier never launches CUDA/nsys."}
    target=root/"profile_verification_v5.json"
