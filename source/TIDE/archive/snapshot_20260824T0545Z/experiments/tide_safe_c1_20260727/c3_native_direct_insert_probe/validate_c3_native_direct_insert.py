...
def main() -> int:
    ...
    try:
        ...
        if not rows or rows[0].get("record") != "meta":
            ...
            meta = rows[0]
            if meta.get("archived_incremental_updater_used") is not False:
                fail(errors, "legacy_updater_not_disabled")
            # The actual static API reaches rp_predict inside nodeProcessKnn.
            # C3 is intentionally baseline-only: the runner must explicitly
            # upload and read back c_rp_mode=0 before its first self query.
            if meta.get("residual_pruning_mode") != 0:
                fail(errors, "residual_pruning_mode", expected=0, observed=meta.get("residual_pruning_mode"))
            if meta.get("residual_pruning_runtime_verified") is not True:
                fail(errors, "residual_pruning_runtime_not_verified", observed=meta.get("residual_pruning_runtime_verified"))
            if meta.get("search_upper_invariant_ok") is not search_upper_ok:
                fail(errors, "runner_search_upper_audit", expected=search_upper_ok, observed=meta.get("search_upper_invariant_ok"))
