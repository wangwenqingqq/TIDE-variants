...
constexpr char kReleaseRoot[] =
    ...
    "safe_c1_native_matrix_g3_v13_frozen_snapshot_certificate_diag";
...
constexpr ExpectedFile kSourceClosure[] = {
    {"src/g3_safe_c1_native_matrix.cu",
     "18293057dbffab1dd068f9a5313b922948e0110058f5f143f5c065cb8be18b7b", 0},
...
std::string frozen_certificate_candidate_json(
    StableId stable, const FrozenCertificateDiagnostic& diagnostic) {
  const safe_c1_g3::StrictCertificate& certificate = diagnostic.certificate;
  std::ostringstream out;
  out << "{\"stable_id\":" << stable
      << ",\"candidate_role\":\"fixture_source_mirror_annotation_only\""
      << ",\"certificate_scope\":\"frozen_snapshot_host_replay_no_native_query_membership\""
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    constexpr std::array<StableId, 6> kCandidates = {4876, 4898, 5303, 4097, 4099, 4102};
    std::array<FrozenCertificateDiagnostic, kCandidates.size()> diagnostics;
    for (std::size_t i = 0; i < kCandidates.size(); ++i) {
      diagnostics[i] = matrix.inspect_initial_frozen_certificate(kCandidates[i]);
    }
    ...
    receipt << "{\"schema\":\"safe-c1-g3-native-snapshot-certificate-diagnostic-v1\""
            ...
            << ",\"node_count\":" << initial_build.node_capacity
