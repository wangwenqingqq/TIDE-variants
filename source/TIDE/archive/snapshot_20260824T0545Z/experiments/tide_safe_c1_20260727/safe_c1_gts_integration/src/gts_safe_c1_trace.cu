...
[[noreturn]] void die(const std::string& message) {
  throw std::runtime_error("E1-GI-B: " + message);
}
...
Args parse_args(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (key == "--bundle") args.bundle = value();
      ...
      std::cout << "usage: GTS_safe_c1_trace --bundle QUANTIZED_BUNDLE --out RESULTS.jsonl "
                   "--summary SUMMARY.json [--leaf-capacity 64]\n";
      std::exit(0);
    } else {
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    output << "{\"record\":\"meta\",\"schema\":\"e1gi-b-gts-safe-c1-v1\","
           << "\"scope\":\"frozen real GTS base + Safe-C1 state + exact CUDA dynamic exporter; not latency evidence\","
    ...
    summary << "{\n"
            << "  \"schema\": \"e1gi-b-gts-safe-c1-v1\",\n"
            << "  \"status\": \"PASS_GTS_BASE_INTEGRATED_REFERENCE\",\n"
            << "  \"scope\": \"frozen real GTS base tree plus Safe-C1 state and exact CUDA dynamic exporter; not a dynamic-GTS latency claim\",\n"
