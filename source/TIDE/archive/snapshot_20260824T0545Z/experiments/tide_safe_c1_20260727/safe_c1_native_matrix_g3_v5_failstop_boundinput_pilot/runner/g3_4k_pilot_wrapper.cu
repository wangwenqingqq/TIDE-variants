...
constexpr char kReleaseRoot[] =
    ...
    "safe_c1_native_matrix_g3_v5_failstop_boundinput_pilot";
...
bool decimal_gpu_ordinal(const std::string& value) {
  return !value.empty() &&
...
void require_controlled_environment_or_fail(bool require_explicit_gpu) {
  bool saw_path = false;
  bool saw_home = false;
  bool saw_gpu = false;
  for (char** entry = ::environ; entry != nullptr && *entry != nullptr; ++entry) {
    ...
    if (value == "PATH=/usr/bin:/bin") {
      ...
      saw_home = true;
               ...
               decimal_gpu_ordinal(value.substr(std::string("CUDA_VISIBLE_DEVICES=").size()))) {
      saw_gpu = true;
    } else {
  ...
  require(saw_path && saw_home,
          "controlled launcher must provide fixed PATH and HOME through env -i");
  require(!require_explicit_gpu || saw_gpu,
          "controlled run launcher must provide one explicit CUDA_VISIBLE_DEVICES ordinal");
  require(require_explicit_gpu || !saw_gpu,
          "pre-native control descriptor must not carry a CUDA device selection");
}

void require_clean_loader_environment_or_fail() {
  require_controlled_environment_or_fail(false);
}

void require_clean_run_environment_or_fail() {
  require_controlled_environment_or_fail(true);
}
...
LaunchArguments parse_launch_arguments_or_fail(int argc, char** argv) {
  require(argc == 5 && std::string_view(argv[1]) == "--run-dir" &&
          ...
          "usage is exactly: g3_4k_pilot_wrapper --run-dir <absolute-v5-runs-child> --admission <absolute-v5-admission>");
...
void validate_link_command_manifest_or_fail(
  ...
  constexpr const char* required[] = {
      ...
      "dynamic_loader_calls_in_controlled_closure", "build_launcher_path",
      "build_launcher_sha256", "build_body_path", "build_body_sha256",
  ...
  require(admission_value(fields, "schema") == "safe-c1-g3-link-command-manifest-v2" &&
              ...
              admission_value(fields, "dynamic_loader_calls_in_controlled_closure") == "absent" &&
...
LaunchAdmission validate_launch_admission_or_fail(
  ...
  require(canonical_admission_root == admission_root &&
          ...
          "admission root must be fixed real v5 preflight/admissions directory");
  ...
  require(admission.parent_path() == admission_root && admission.extension() == ".admission",
          "admission must be a fixed real child of v5 preflight/admissions");
  ...
  constexpr const char* required[] = {
      ...
      "link_command_sha256", "pre_native_host_loader_descriptor_sha256",
      "loader_snapshot_scope", "sanitized_exec_environment",
      "run_launcher_path", "run_launcher_sha256", "run_body_path", "run_body_sha256"};
  require(fields.size() == std::size(required),
          "launch admission has unrecognized/missing keys");
  for (const char* key : required) (void)admission_value(fields, key);
  require(admission_value(fields, "schema") == "safe-c1-g3-single-tu-admission-v2" &&
  ...
  require(fs::weakly_canonical(binary_root) == binary_root &&
          ...
          "executing binary does not equal admission-bound v5 preflight/bin path");
  ...
  require(admission_value(fields, "run_launcher_path") == run_launcher.string() &&
          ...
          "current controlled run launcher/body differs from admission");
  ...
  constexpr const char* report_required[] = {
      ...
      "dynamic_loader_calls_in_controlled_closure", "legacy_gts_dso",
  ...
  require(admission_value(report_fields, "schema") ==
                  "safe-c1-g3-link-image-report-v3" &&
              ...
              admission_value(report_fields, "raw_legacy_gts_rp_calls_in_wrapper") ==
                  "absent" &&
              admission_value(report_fields, "dynamic_loader_calls_in_controlled_closure") ==
                  "absent" &&
              ...
              admission_value(report_fields, "link_command_sha256") ==
                  admission_value(fields, "link_command_sha256"),
          "link-image report does not bind binary/source/command");
...
std::vector<TraceOp> parse_and_validate_fixed_trace(const std::vector<std::uint8_t>& trace_bytes) {
  ...
  for (std::size_t i = 0; i < lines.size(); ++i) {
    ...
    if (op == "insert" || op == "delete") {
      ...
      // v5 runtime policy.  v5 validates it for input provenance only and
...
RunPaths create_staging_directory_or_fail(const fs::path& final_dir) {
  ...
  require(final_dir.parent_path() == runs_root,
          "run directory must be one new direct child of the fixed v5 runs directory");
