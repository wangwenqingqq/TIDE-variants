#!/bin/bash
# Controlled one-shot GPU token issuer for the Safe-C3 4102 leaf-envelope gate.
set -euo pipefail
umask 077

ROOT="/workspace/experiments/tide_safe_c1_20260727/c3_safe_leaf_envelope_v1"
BIN="$ROOT/bin/GTS_safe_c3_leaf_envelope"
SRC="$ROOT/src/gts_safe_c3_leaf_envelope.cu"
PROTO="$ROOT/protocol/safe_c3_leaf_envelope_v1.json"
STATIC="$ROOT/preflight/static_audit_leaf.json"
HELPER="$ROOT/tools/.nvml_snapshot.py"
VALIDATOR="$ROOT/tools/validate_safe_c3_leaf.py"
...
usage() {
  echo "usage: issue_safe_c3_leaf_gpu_token.sh --gpu-ordinal 1 --run-name <name> --approval-id <id> --ttl-seconds <1..120>" >&2
  exit 69
}
...
TMP="$PENDING/.token-$NAME-$NONCE"
{
  ...
  printf 'purpose=safe_c3_4102_leaf_envelope_correctness_gate\n'
} > "$TMP"
chmod 600 "$TMP"
