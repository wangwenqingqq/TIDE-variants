#!/bin/bash
# One-shot controlled runner for the Safe-C3 4102 leaf-envelope gate.
set -euo pipefail
umask 077
...
usage() { echo "usage: run_safe_c3_leaf_gpu1_once.sh --run-name <authorized-name>" >&2; exit 69; }
field() {
  local key="$1" value
...
/usr/bin/env -i PATH=/usr/bin:/bin HOME=/nonexistent \
  ...
  --candidate-id 4102 --expected-leaf 604 \
