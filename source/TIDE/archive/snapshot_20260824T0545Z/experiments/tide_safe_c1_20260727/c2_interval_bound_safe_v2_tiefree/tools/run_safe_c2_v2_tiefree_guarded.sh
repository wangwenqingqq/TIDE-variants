...
assert p['resource_policy']['write_boundary'] == str(protocol.parent.parent)
split = p['split']
assert split['selection_audit_path'] == p['tie_free_v2']['selection_audit_path']
assert split['selection_audit_sha256'] == p['tie_free_v2']['selection_audit_sha256']
assert split['selected_evaluation_exhaustive_over_original_10000'] is False
assert split['selected_plus_excluded_partition_original_10000'] is True
assert q['schema'] == 'safe-c2-corrected-after-submit-tiefree-preflight-v2'
assert q['split']['selected_plus_excluded_partition_original_10000'] is True
assert q['tie_free_v2']['selection_audit_path'] == p['tie_free_v2']['selection_audit_path']
assert q['tie_free_v2']['selection_audit_sha256'] == p['tie_free_v2']['selection_audit_sha256']
assert q['tie_free_v2']['fp32_selected_checks'] == p['tie_free_v2']['fp32_selected_checks']
s = source.read_text()
...
for stage, expected_max_d2 in {'calibration':128166, 'validation':133413, 'test':150288}.items():
    c = j['fp32_selected_checks'][stage]
    assert c['fp32_boundary_ties'] == 0
    assert c['fp32_stored_top10_mismatches'] == 0
    assert c['fp32_equal_distance_distinct_d2_pairs'] == 0
    assert c['max_d2'] == expected_max_d2
lines = [
...
for stage, c in j['fp32_selected_checks'].items():
    lines.append(f'{stage}_fp32_boundary_ties={c["fp32_boundary_ties"]}')
    lines.append(f'{stage}_fp32_stored_top10_mismatches={c["fp32_stored_top10_mismatches"]}')
    lines.append(f'{stage}_fp32_equal_distance_distinct_d2_pairs={c["fp32_equal_distance_distinct_d2_pairs"]}')
    lines.append(f'{stage}_fp32_max_d2={c["max_d2"]}')
