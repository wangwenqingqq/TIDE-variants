#!/usr/bin/env bash
# CPU-only Safe-C2 v2 tie-free compilation: nvcc compiles but does not initialize or execute a GPU.
set -Eeuo pipefail
...
OUT="$ROOT/bin/GTS_safe_c2_v2_tiefree_sift1m"
LOG="$ROOT/logs/build_safe_c2_v2_tiefree_$(date -u +%Y%m%dT%H%M%SZ).log"
