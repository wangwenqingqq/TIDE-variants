...
void write_run_card(const fs::path& out, const Config& cfg) {
  std::ofstream f(out / "run_card.json");
  f << "{\n"
    << "  \"schema\": \"gtspp-c1-microbench-run-card-v1\",\n"
...
void setup_update_session_for_query_only() {
  // This follows the non-C1 session initialization from updateIndexRnn, so the
  // query path sees the same delete/overflow bookkeeping without permitting a mutation.
...
QueryResult run_query(int query_id, c1c::Phase phase, cudaEvent_t event_start, cudaEvent_t event_stop) {
  ...
  if (total_result_num < 0) die("negative merged result count");
  if (total_result_num > 0) {
    ...
    mergeTotalResult<<<(total_result_num - 1) / THREAD_NUM + 1, THREAD_NUM>>>(
        total_result_num, total_result_id, qresult_count, result_id, result_dis, obj_r,
        total_result_dis, is_delete_prefix, tree_size);
  }
...
void write_completion(const fs::path& out, const Config& cfg, const TreeFingerprint& initial,
  ...
  f << "{\n"
    << "  \"schema\": \"gtspp-c1-microbench-completion-v1\",\n"
    << "  \"run_mode\": \"" << run_mode(cfg) << "\",\n"
    << "  \"compiled_variant\": \"" << compiled_variant_name() << "\",\n"
...
int main(int argc, char** argv) {
  ...
  upload_c2_mode_zero();
  setup_update_session_for_query_only();
