#!/usr/bin/env bash
# Fable5 matched Safe-C1/buffer-only GPU1 guard (v1).
#
...
readonly STATIC_AUDIT="$ROOT/tools/audit_fable5_matched_gpu_static_v1.py"
readonly TRACE_PREFLIGHT="$ROOT/tools/preflight_fable5_matched_trace_v1.py"
readonly TRACE_PREFLIGHT_CONTRACT="$ROOT/manifests/fable5_matched_trace_preflight_v1.json"
...
fail() { printf 'FABLE5_GUARD_FAIL: %s\n' "$*" >&2; exit 2; }
...
[[ "$EXECUTE" == 1 ]] || { printf 'FABLE5_GUARD_REFUSED: add --execute after fresh approval; no action taken\n' >&2; exit 64; }
...
if ! TRACE_PREFLIGHT_JSON="$("$PYTHON_BIN" "$TRACE_PREFLIGHT" \
  ...
  fail 'trace preflight failed before run-directory creation/NVML'
fi
...
json.dump({
  'schema': 'fable5-matched-gpu1-guard-run-card-v1', 'status': 'PRE_NVML_TRACE_AND_STATIC_AUDITS_PASSED',
  ...
  'scope': 'matched correctness only; no timing/performance/C2/C3 claim'
...
p=sys.argv[1]
r=json.load(open(p))
r['status']='PASS_FABLE5_MATCHED_GUARDED_RUN_PENDING_MANUAL_REVIEW'
with open(p,'w') as f: json.dump(r,f,indent=2,sort_keys=True); f.write('\n')
PY
printf 'PASS_FABLE5_MATCHED_GUARDED_RUN_PENDING_MANUAL_REVIEW\n'
