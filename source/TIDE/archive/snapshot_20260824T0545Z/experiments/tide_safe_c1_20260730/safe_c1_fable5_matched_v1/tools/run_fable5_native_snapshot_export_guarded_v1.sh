...
# System python on 8p has no pynvml; use this existing local CPython 3.12
# environment only for the NVML binding. The guard still performs all static
# JSON/input checks with system python and never falls back to a CLI GPU-status tool.
...
# Validate the explicitly pinned Python NVML binding before any run directory,
# NVML initialization, CUDA visibility change, or exporter binary invocation.
