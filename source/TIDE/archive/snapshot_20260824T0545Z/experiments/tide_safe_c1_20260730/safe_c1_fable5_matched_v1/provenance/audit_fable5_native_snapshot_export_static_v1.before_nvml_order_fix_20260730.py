...
def main() -> int:
    ...
    report: dict[str, Any] = {
        ...
        'execution': {
            ...
            'compiler_invoked_by_this_audit': False,
    ...
    try:
        ...
        if selector.is_file():
            ...
            for name, token in {
                ...
                'snapshot_c2_false_gate': 'snapshot_must_record_c2_used_false',
                'snapshot_c3_false_gate': 'snapshot_must_record_c3_used_false',
                ...
                'same_leaf_three_gate': 'bounded_query_ranked_direct_pool_has_no_same_leaf_triple',
                ...
                'direct_observability_A': 'exact_visible_queries(ranker, (a,), a)',
                'direct_observability_C': 'exact_visible_queries(ranker, (r, c), c)',
                'fallback_observability_mid': 'b in topk and r in topk',
                'bounded_candidate_selector': 'bounded_prefix_cache_v1',
                'bounded_candidate_limit': 'MAX_TUPLE_EVALUATIONS',
                'bounded_no_cartesian_enumeration': 'never explores the reservoir/cartesian product exhaustively',
                ...
                'runtime_receipt_requirement': 'verify native traversal receipt visits the selected direct leaf',
                'e1_active_formula': 'ranker.static_epoch((r, c), base_delete)',
                ...
                'non_c2c3_scope': 'no native runtime certificate, traversal receipt, timing, performance, C2, C3, or deployment claim',
            }.items():
                require(checks, errors, text, name, token)
            for name, token in {
        ...
        if guard.is_file():
            ...
            for name, token in {
                ...
                'machine_c2c3_output_gate': 'snapshot_must_machine_record_c2_c3_unused',
                'pinned_nvml_python': "readonly NVML_PYTHON_BIN='/workspace/project/livsyn/.venv/bin/python'",
                'nvml_binding_import_preflight': "-c 'import pynvml'",
            ...
            nvml = text.find('import pynvml', binding + 1)
            checks['guard_gate_before_static_audit'] = gate >= 0 and audit >= 0 and gate < audit
            checks['guard_binding_before_run_dir'] = binding >= 0 and run_dir >= 0 and binding < run_dir
            ...
            if not checks['guard_binding_before_run_dir']:
                errors.append('guard_binding_run_dir_order')
        ...
        if manifest.is_file():
            ...
            checks['manifest_pre_gpu_boundary'] = (
                boundary.get('compiled') is True
                and boundary.get('cuda_binary_executed') is False
                and boundary.get('gpu_used') is False
                and boundary.get('nvml_queried') is False
            )
            checks['manifest_cpu_build_state'] = m.get('status') == 'CPU_BUILT_PRE_GPU'
            checks['manifest_mode_zero'] = m.get('residual_pruning', {}).get('mode') == 0
            checks['manifest_machine_no_c2c3'] = (
                m.get('feature_flags', {}).get('c2_used') is False
                and m.get('feature_flags', {}).get('c3_used') is False
            )
            checks['manifest_no_c2c3'] = all(item in m.get('non_claims', []) for item in ('C2', 'C3'))
            export = m.get('exporter', {})
            checks['manifest_exporter_source_hash'] = (
                exporter.is_file() and export.get('source_sha256') == sha(exporter)
            )
            checks['manifest_selector_source_hash'] = (
                selector.is_file() and export.get('cpu_selector_source_sha256') == sha(selector)
            )
            binary = root / export.get('binary', '') if isinstance(export.get('binary'), str) else root / '__invalid_binary__'
            receipt = root / export.get('cpu_build_receipt', '') if isinstance(export.get('cpu_build_receipt'), str) else root / '__invalid_receipt__'
            checks['manifest_binary_exists_and_hash'] = (
                binary.is_file() and export.get('binary_sha256') == sha(binary)
            )
            checks['manifest_receipt_exists_and_hash'] = (
                receipt.is_file() and export.get('cpu_build_receipt_sha256') == sha(receipt)
            )
            ...
            if receipt.is_file():
                try:
                    ...
                    errors.append('invalid:cpu_build_receipt_json:' + type(exc).__name__)
            checks['cpu_build_receipt_semantics'] = (
                receipt_doc.get('schema') == 'fable5-native-snapshot-exporter-v1-cpu-build-receipt'
                and receipt_doc.get('status') == 'PASS_CPU_BUILD_ONLY_NOT_EXECUTED'
                and receipt_doc.get('source_sha256') == sha(exporter)
                and receipt_doc.get('archived_core_sha256') == PINNED_CORE_SHA256
                and receipt_doc.get('matched_runner_sha256') == PINNED_MATCHED_RUNNER_SHA256
                ...
                and receipt_doc.get('nvidia_smi_called') is False
            )
            compile_log = Path(receipt_doc.get('compile_log', '__missing__'))
            checks['cpu_build_receipt_log_pin'] = (
                compile_log.is_file() and receipt_doc.get('compile_log_sha256') == sha(compile_log)
            )
            provenance = m.get('cpu_build_provenance', {})
            checks['manifest_build_provenance_binding'] = (
                provenance.get('receipt') == export.get('cpu_build_receipt')
                and provenance.get('receipt_sha256') == export.get('cpu_build_receipt_sha256')
                and provenance.get('binary') == export.get('binary')
                and provenance.get('binary_sha256') == export.get('binary_sha256')
                and provenance.get('status') == 'PASS_CPU_BUILD_ONLY_NOT_EXECUTED'
            )
            for name in (
                'manifest_schema', 'manifest_pre_gpu_boundary', 'manifest_cpu_build_state',
                'manifest_mode_zero', 'manifest_machine_no_c2c3', 'manifest_no_c2c3',
                'manifest_exporter_source_hash', 'manifest_selector_source_hash',
                'manifest_binary_exists_and_hash', 'manifest_receipt_exists_and_hash',
                'cpu_build_receipt_semantics', 'cpu_build_receipt_log_pin',
                'manifest_build_provenance_binding',
            ):
                if not checks[name]:
