// Isolated Safe-C1 G2 stable-ID seeded immediate-rebuild witness (v5).
//
...
// -----------------------------------------------------------------------------
// G2 seeded stable-ID rebuild witness executor (isolated v5)
// -----------------------------------------------------------------------------
...
[[noreturn]] void die(const std::string& message) {
  throw std::runtime_error("Safe-C1 G2 v5: " + message);
}
...
Args parse_args(int argc, char** argv) {
  ...
  for (int index = 1; index < argc; ++index) {
    ...
    if (key == "--bundle") args.bundle = value();
      ...
      std::cout << "usage: GTS_safe_c1_g2_v5 --bundle BUNDLE --out RESULTS.jsonl --summary SUMMARY.json "
...
G2RebuildWitnessContract read_g2_rebuild_witness_contract(const std::string& path) {
  ...
  if (values.at("schema") != "safe-c1-g2-rebuild-witness-contract-v5") die("wrong G2 rebuild contract schema");
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    out << "{\"record\":\"meta\",\"schema\":\"safe-c1-g2-rebuild-witness-v5\","
        << "\"scope\":\"strict stable-ID immediate rebuild witness only; no timing/performance/general rebuild/range/C2/all-input tie claim\","
        << "\"legacy_incremental_updater_used\":false,\"stable_id_layout\":\"identity_full_immutable_pool_seeded_stable_ids_v5\","
    ...
    summary << "{\n"
            << "  \"schema\": \"safe-c1-g2-rebuild-witness-v5\",\n"
