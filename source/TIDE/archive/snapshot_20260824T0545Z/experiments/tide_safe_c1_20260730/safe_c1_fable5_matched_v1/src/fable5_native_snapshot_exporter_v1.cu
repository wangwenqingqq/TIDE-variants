...
// Keep standard-library headers before the archived config macro (`short` ->
// `float`) becomes visible through the pinned core include.
#include <cctype>
#include <cstring>
...
std::uint64_t canonical_live_tree_hash(const safe_c1::FrozenTreeSnapshot& snapshot) {
  // Empty TN slots are not semantically initialized by the legacy constructor.
  // A reproducible identity hashes initialized live nodes plus the initialized
  // empty bitmap, never indeterminate TN bytes from empty slots.
  const int node_count = static_cast<int>(snapshot.nodes.size());
  std::uint64_t value = safe_c1::FrozenTreeSnapshot::fnv1a(
      &snapshot.tree_height, sizeof(snapshot.tree_height));
  value = safe_c1::FrozenTreeSnapshot::fnv1a(&snapshot.fanout, sizeof(snapshot.fanout), value);
  value = safe_c1::FrozenTreeSnapshot::fnv1a(&node_count, sizeof(node_count), value);
  for (int node_id = 0; node_id < node_count; ++node_id) {
    const int empty = snapshot.empty[static_cast<std::size_t>(node_id)];
    value = safe_c1::FrozenTreeSnapshot::fnv1a(&node_id, sizeof(node_id), value);
    value = safe_c1::FrozenTreeSnapshot::fnv1a(&empty, sizeof(empty), value);
    if (empty == 0) {
      value = safe_c1::FrozenTreeSnapshot::fnv1a(
          &snapshot.nodes[static_cast<std::size_t>(node_id)], sizeof(TN), value);
      value = safe_c1::FrozenTreeSnapshot::fnv1a(
          &snapshot.max_distance[static_cast<std::size_t>(node_id)], sizeof(float), value);
    }
  }
  value = safe_c1::FrozenTreeSnapshot::fnv1a(
      &snapshot.logical_leaf_id_count, sizeof(snapshot.logical_leaf_id_count), value);
  value = safe_c1::FrozenTreeSnapshot::fnv1a(
      &snapshot.logical_leaf_id_hash, sizeof(snapshot.logical_leaf_id_hash), value);
  return value;
}
...
void write_snapshot(const Args& args, const TraceHeader& header,
  ...
  const std::uint64_t canonical_hash = canonical_live_tree_hash(snapshot);
  ...
  output << "{\n"
         ...
         << "  \"tn_abi\": \"live-only:little-endian:i32,f32bits,i32,i32,i32;empty-nodes=null\",\n"
         << "  \"tn_size_bytes\": " << sizeof(TN) << ",\n"
         << "  \"metric_encoding\": {\"infi_dis\":" << INFI_DIS << ",\"dis_code\":" << DIS_CODE << "},\n"
         << "  \"native_hashes\": {\"canonical_live_tree_fnv1a64\":" << canonical_hash
  ...
  for (std::size_t index = 0; index < snapshot.max_distance.size(); ++index) {
    if (index) output << ',';
    if (snapshot.empty[index] != 0) output << "null";
    else output << float_bits(snapshot.max_distance[index]);
  }
