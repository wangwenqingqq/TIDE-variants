...
class NativeBatch:
    def __init__(self, root, data):
        self.path=root/'build/bridges_v2/libp2_tide.so'; self.lib=C.CDLL(str(self.path))
        ptr=np.ctypeslib.ndpointer(dtype=np.uint64,flags='C_CONTIGUOUS')
        self.lib.p2_create.argtypes=[C.c_char_p]; self.lib.p2_create.restype=C.c_void_p
        self.lib.p2_destroy.argtypes=[C.c_void_p]; self.lib.p2_destroy.restype=None
        self.lib.p2_error.restype=C.c_char_p
        self.lib.p2_run.argtypes=[C.c_void_p,ptr,C.c_uint,C.c_uint,C.c_uint,ptr,ptr];self.lib.p2_run.restype=C.c_int
        self.handle=self.lib.p2_create(str(data).encode())
        if not self.handle:raise RuntimeError(self.lib.p2_error().decode())
        self.output=np.empty((64,65536),dtype=np.uint64);self.counts=np.empty(64,dtype=np.uint64)
        self.info={'adapter':'ordinary native queued batch; unchanged kernel; one stream; full tuple sort/hash',
                   'library':str(self.path),'library_sha256':digest(self.path)}
    def run(self, queries, indices, p, d):
        start=time.perf_counter_ns()
        packed=np.ascontiguousarray(queries[indices])
        code=self.lib.p2_run(self.handle,packed,len(indices),p,d,self.output,self.counts)
        if code:raise RuntimeError(self.lib.p2_error().decode())
        result=[self.output[i,:self.counts[i]].copy() for i in range(len(indices))]
        return result,(time.perf_counter_ns()-start)/1e6,None
    def close(self):
        if self.handle:self.lib.p2_destroy(self.handle);self.handle=None
...
def main():
    ...
    ap.add_argument('--backend',choices=['nv_row','nv_column','gpusim','tide_bounded','tide_unbounded','tide_batch'],required=True)
    ...
    def create():
        if args.backend=='tide_batch': return NativeBatch(root,data)
