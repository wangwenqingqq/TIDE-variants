...
if (($# < 1)); then
  echo "usage: $0 <TIDE benchmark arguments...>" >&2
  exit 2
fi

readonly scratch=/workspace/TIDE/active/tide_surechembl_gate6_20260828/scratch/nvmolkit060_tide_paired/nvmolkit060_tide_paired_20260904_v1
readonly binary="$scratch/artifacts/gate6_query_campaign_external_v2"
readonly expected_binary_sha=73ce0bb3c5aaa2bacfb74ae8837edfd91051cd16e44dc77aec0faefe4d909f1e
...
[[ $(sha256sum "$binary" | awk '{print $1}') == "$expected_binary_sha" ]] || {
  echo "TIDE binary hash mismatch" >&2
  exit 30
}
exec 9>"$global_lock"
