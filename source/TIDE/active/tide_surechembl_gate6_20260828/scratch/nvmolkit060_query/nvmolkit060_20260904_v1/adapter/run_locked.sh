...
if (($# < 1)); then
  echo "usage: $0 <benchmark arguments...>" >&2
  exit 2
fi
...
exec taskset -c 127 "$python" "$scratch/adapter/exact_complete_benchmark.py" "$@"
