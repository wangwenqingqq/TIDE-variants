...
// Private shadow ownership: no query runtime, snapshot publication or mutation.
namespace svcq {
struct Shadow {
  HostRun host;
  DeviceRun<4> device;
  explicit Shadow(const std::string& root) : host(root), device(0, host.host) {}
};
}
extern "C" void* svcq_shadow_create(const char* root) {
  try { return new svcq::Shadow(root); }
  catch (const std::exception& e) { svcq::error = e.what(); return nullptr; }
}
extern "C" void svcq_shadow_destroy(void* shadow) {
  delete static_cast<svcq::Shadow*>(shadow);
}
