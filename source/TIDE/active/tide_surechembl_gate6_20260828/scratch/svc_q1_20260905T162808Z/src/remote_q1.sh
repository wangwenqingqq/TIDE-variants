...
[[ "$WORK" == "$ROOT"/scratch/svc_q1_* && "$ATTEMPT" =~ ^a[0-9][0-9]$ ]]
cd "$WORK"
mkdir -p raw results
...
if [[ "$MODE" == build ]]; then
  ...
  run build nvcc -O3 -std=c++17 -shared -Xcompiler=-fPIC -arch=sm_120 \
    --ptxas-options=-v src/svc_q1_bridge.cu -o build/libsvcq.so
  run sass cuobjdump --dump-sass build/libsvcq.so
  run cpu_input_audit "$PY" -B src/run_q1.py --mode audit --output results/cpu_input_audit
...
for check in normal memcheck synccheck; do
  label="${ATTEMPT}_${check}"
  if [[ "$check" == normal ]]; then
    run "$label" "$PY" -B src/run_q1.py --mode validate --output "results/$label"
  else
    run "$label" compute-sanitizer --tool "$check" --error-exitcode 86 --target-processes all \
      "$PY" -B src/run_q1.py --mode validate --output "results/$label"
...
for rep in 0 1 2 3; do
  arms='B0 B1 U0 U1'
  thresholds='7/10 4/5'
  if [[ "$rep" == 1 || "$rep" == 2 ]]; then arms='U1 U0 B1 B0'; fi
  if [[ "$rep" == 2 || "$rep" == 3 ]]; then thresholds='4/5 7/10'; fi
  for threshold in $thresholds; do
    for arm in $arms; do
      label="${ATTEMPT}_r${rep}_${arm}_${threshold//\//-}"
      [[ -z $(nvidia-smi -i 2 --query-compute-apps=pid --format=csv,noheader) ]]
      run "$label" "$PY" -B src/run_q1.py --mode measure --arm "$arm" --threshold "$threshold" \
        --repetition "$rep" --output "results/$label"
      [[ ! -f raw/CONTAMINATION.json ]]
