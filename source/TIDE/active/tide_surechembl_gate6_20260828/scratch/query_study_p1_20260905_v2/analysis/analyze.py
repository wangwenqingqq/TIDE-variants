...
def read_process(stage, item):
    ...
    if (guard['status'] != 'complete' or guard['exit_code'] != 0 or
            guard['before']['apps'] or guard['after']['apps']):
...
def main():
    ...
    if not completion['complete'] or completion['processes'] != 48:
        raise ValueError('formal campaign incomplete')
    gates = json.loads((stage.parent / 'validate/GATES.json').read_text())
    if not gates['complete'] or len(gates['invocations']) != 25:
    ...
    for peer in ['nv_row', 'nv_column', 'gpusim', 'tide_unbounded']:
        ...
        for pair in range(1, 7):
            ...
            b = process[peer, pair, 'tide_bounded']
    ...
    report = {'scope': '1M-row 256-bit complete adapter service; not kernel-only or full-scale',
              ...
              'ratio_direction': 'peer / tide_bounded; above one favors TIDE',
    ...
    lines = ['# P1 paired service screen', '', 'Ratio = peer / bounded TIDE. All six process pairs retained.',
