...
"$CUDA/bin/nvcc" -std=c++17 -O3 -lineinfo -arch=sm_120a \
  -Xcompiler=-pthread,-Wall,-Wextra,-Wpedantic \
...
OMP_NUM_THREADS=32 /usr/bin/time -v "$ORACLE_BIN" \
  "$FIXTURE" "$FIXTURE_ORACLE" 512 \
  > "$WORK/oracle/fixture.stdout.txt" 2> "$WORK/oracle/fixture.stderr.txt"
OMP_NUM_THREADS=32 /usr/bin/time -v "$ORACLE_BIN" \
  "$DATA" "$FORMAL_ORACLE" 512 \
...
run_one real_smoke_overlap "$DATA" "$FORMAL_ORACLE" overlap 2048 1 0
