...
def main() -> int:
    ...
    # Keep the real canonical future-arrival queries inside the sampled delta so
    # the base-only versus base-plus-delta visibility check includes their exact
    # self matches. Fill the remaining rows with deterministic stratified ranks.
    delta_ids_all = delta_all[:, 0].view("<i8")
    required_delta_indices: list[int] = []
    for query_id, query in zip(query_ids, queries, strict=True):
        matches = np.flatnonzero(delta_ids_all == query_id)
        if len(matches) != 1:
            raise RuntimeError(f"query ID {int(query_id)} occurs {len(matches)} times in delta")
        position = int(matches[0])
        if not np.array_equal(delta_all[position], query):
            raise RuntimeError(f"query ID {int(query_id)} differs from its delta record")
        required_delta_indices.append(position)
    selected = set(required_delta_indices)
    for position in stratified_indices(len(delta_all), args.delta_rows):
        selected.add(int(position))
        if len(selected) == args.delta_rows:
            break
    if len(selected) != args.delta_rows:
        raise RuntimeError("could not construct the fixed-size delta sample")
    delta_index = np.array(sorted(selected), dtype=np.int64)
    ...
    result = {
        ...
        "queries_contained_in_delta": bool(
            set(int(value) for value in query_ids)
            <= set(int(value) for value in delta_ids)
        ),
        "selected_indices": {
