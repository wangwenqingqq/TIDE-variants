...
void init_context(DeviceContext &c, int gpu, std::uint64_t capacity,
  ...
  c.output.resize(static_cast<std::size_t>(max_batch) * K);
...
BatchResult run_batch(std::vector<DeviceContext> &ctx, const QueryStore &qs,
  ...
  for (auto &c : ctx) {
    CUDA_CHECK(cudaSetDevice(c.gpu));
    CUDA_CHECK(cudaMemcpyAsync(c.d_qfp, qfp.data(), qfp.size() * sizeof(std::uint64_t), cudaMemcpyHostToDevice, c.stream));
    CUDA_CHECK(cudaMemcpyAsync(c.d_qids, qids.data(), qids.size() * sizeof(std::int64_t), cudaMemcpyHostToDevice, c.stream));
    CUDA_CHECK(cudaMemcpyAsync(c.d_qpc, qpc.data(), qpc.size() * sizeof(std::uint16_t), cudaMemcpyHostToDevice, c.stream));
    ...
    CUDA_CHECK(cudaMemcpyAsync(c.output.data(), c.d_out,
  ...
  for (int q = 0; q < batch; ++q) {
    ...
    for (const auto &c : ctx)
      for (int k = 0; k < K; ++k) top.insert(c.output[static_cast<std::size_t>(q) * K + k]);
...
int main(int argc, char **argv) {
  try {
    ...
    auto ctx = load_base(o, bfp, bids, bpc, o.limit_rows ? 0 : delta_rows, max_batch);
    HostSoA delta = load_delta(dfile, true);
    if (!o.limit_rows) append_delta(ctx, delta);
    QueryStore qs(qfile);
