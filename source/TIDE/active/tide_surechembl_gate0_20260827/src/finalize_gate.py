...
def main() -> None:
    ...
    decision = "PASS_TO_GATE1" if all(gates.values()) else "STOP_BEFORE_GRAPH_KERNEL"
    ...
    (results / "FINAL_VERDICT.json").write_text(json.dumps(final, indent=2) + "\n")
    ...
    results_md = f"""# TIDE SureChEMBL Gate-0 results
...
"""
    ...
    memo = f"""# TIDE SureChEMBL Gate-0 decision memo
...
"""
    (args.root / "DECISION_MEMO.md").write_text(memo)
