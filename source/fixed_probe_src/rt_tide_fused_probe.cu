...
// Query-tiled exact scan: load each database vector once per block, then
...
struct Stats { double mean; float minv; float p50; float p95; };
...
int main(int argc, char** argv) {
  try {
    ...
    if (const char* fp = std::getenv("RTSPATIAL_FIXED_PARALLELISM"))
      cfg.fixed_parallelism = std::stoi(fp);
    if (const char* sf = std::getenv("RTSPATIAL_SKIP_FORWARD"))
      cfg.skip_forward = std::stoi(sf) != 0;
    ...
    // Experiment-only stage split using LibRTS built-in profiler.
    if (const char* sp = std::getenv("RTSPATIAL_STAGE_PROFILE")) {
      const int profile_parallelism = std::stoi(sp);
      result.Clear(stream.cuda_stream());
      stream.Sync();
      index.IntersectsWhatQueryProfiling(
          rtspatial::ArrayView<envelope_t>(dq), d_result.data(),
          stream.cuda_stream(), profile_parallelism);
      result.Clear(stream.cuda_stream());
      stream.Sync();
    }
    ...
    const bool skip_barrier = [] {
      if (const char* sb = std::getenv("RTSPATIAL_SKIP_BARRIER"))
        return std::stoi(sb) != 0;
      return false;
    }();
    ...
    auto fused_launch = [&] {
      ...
      // Same-stream ordering already supplies the candidate-count dependency.
      // Retain the legacy host barrier unless the experiment explicitly disables it.
      if (!skip_barrier) stream.Sync();
    ...
    std::cout << std::setprecision(10)
      ...
      << ",\"tiled_threshold_counts_equal\":"<<(tiled_counts_equal?"true":"false")
      << ",\"full_true_within_tau\":"<<full_true
      << ",\"candidate_true_within_tau\":"<<cand_true<<"}\n";
    return (counts_equal && tiled_counts_equal) ? 0 : 3;
