...
__global__ void exact_topk_partition_kernel(
    const std::uint64_t *__restrict__ targets,
    const std::int64_t *__restrict__ target_ids,
    const std::uint16_t *__restrict__ target_pc,
    std::uint64_t shard_rows, std::uint64_t global_row_offset,
    const std::uint64_t *__restrict__ queries,
    const std::int64_t *__restrict__ query_ids,
    const std::uint16_t *__restrict__ query_pc,
    int query_count, int partitions, Candidate *__restrict__ partials) {
  const int q = blockIdx.x / partitions;
  const int part = blockIdx.x - q * partitions;
  if (q >= query_count) return;
  const std::uint64_t q0 = queries[static_cast<std::uint64_t>(q) * 4 + 0];
  const std::uint64_t q1 = queries[static_cast<std::uint64_t>(q) * 4 + 1];
  const std::uint64_t q2 = queries[static_cast<std::uint64_t>(q) * 4 + 2];
  ...
  Top10 top;
  top.init();
  const std::uint64_t first = static_cast<std::uint64_t>(part) * blockDim.x + threadIdx.x;
  const std::uint64_t stride = static_cast<std::uint64_t>(partitions) * blockDim.x;
  for (std::uint64_t row = first; row < shard_rows; row += stride) {
    const std::int64_t id = target_ids[row];
    if (id == qid) continue;
    const std::uint64_t off = row * 4;
    const unsigned inter =
...
#pragma unroll
    ...
    merged.store(partials + (static_cast<std::size_t>(q) * partitions + part) * K);
  }
}

__global__ void reduce_partition_topk_kernel(
    const Candidate *__restrict__ partials, int query_count, int partitions,
    Candidate *__restrict__ output) {
  const int q = blockIdx.x;
  if (q >= query_count) return;
  Top10 top;
  top.init();
  const std::size_t base = static_cast<std::size_t>(q) * partitions * K;
  for (int i = threadIdx.x; i < partitions * K; i += blockDim.x)
    top.insert(partials[base + i]);
  extern __shared__ Candidate shared[];
  top.store(shared + threadIdx.x * K);
  __syncthreads();
  if (threadIdx.x == 0) {
...
#pragma unroll
    ...
    merged.store(output + static_cast<std::size_t>(q) * K);
  }
}
...
Options parse_args(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    else if (a == "--partitions") o.partitions = std::stoi(need());
    else if (a == "--query-start") o.query_start = std::stoi(need());
    else if (a == "--query-span") o.query_span = std::stoi(need());
    else if (a == "--force-qparallel") o.force_qparallel = true;
  ...
  if (o.partitions < 1 || o.partitions > 256)
    throw std::runtime_error("partitions must be in [1,256]");
  if (o.query_start < 0)
    throw std::runtime_error("query-start must be nonnegative");
  if (o.query_span < 1)
    throw std::runtime_error("query-span must be positive");
  return o;
}
...
struct DeviceContext {
  ...
  Candidate *d_out = nullptr, *d_partial = nullptr;
...
void cleanup(DeviceContext &c) {
  ...
  if (c.d_partial) cudaFree(c.d_partial);
...
void init_context(DeviceContext &c, int gpu, std::uint64_t capacity,
                  std::uint64_t global_offset, int max_batch, int partitions) {
  ...
  CUDA_CHECK(cudaMalloc(&c.d_partial, static_cast<std::size_t>(max_batch) *
                                         partitions * K * sizeof(Candidate)));
...
std::vector<DeviceContext> load_base(const Options &o, const MappedFile &fp_file,
  ...
  for (std::size_t gi = 0; gi < o.gpus.size(); ++gi) {
    ...
    auto &c = ctx[gi];
    init_context(c, o.gpus[gi], (end - begin) + (dend - dbegin), begin,
                 max_batch, o.partitions);
...
BatchResult run_batch(std::vector<DeviceContext> &ctx, const QueryStore &qs,
                      const std::vector<int> &qidx, bool qparallel,
                      int partitions) {
  ...
  for (auto &c : ctx) {
    ...
    if (qparallel) {
      exact_topk_partition_kernel<<<batch * partitions, BLOCK, smem, c.stream>>>(
          c.d_fp, c.d_ids, c.d_pc, c.visible_rows, c.global_offset,
          c.d_qfp, c.d_qids, c.d_qpc, batch, partitions, c.d_partial);
      CUDA_CHECK(cudaGetLastError());
      reduce_partition_topk_kernel<<<batch, BLOCK, smem, c.stream>>>(
          c.d_partial, batch, partitions, c.d_out);
      CUDA_CHECK(cudaGetLastError());
    } else {
...
std::vector<int> rotating_indices(int batch, int iter, int query_start,
                                  int query_span) {
  std::vector<int> q(batch);
  const int start = (iter * 17) % query_span;
  for (int i = 0; i < batch; ++i)
    q[i] = query_start + ((start + i) % query_span);
  return q;
}
...
void write_correctness(const Options &o, std::vector<DeviceContext> &ctx,
  ...
  const BatchResult r = run_batch(ctx, qs, qidx, true, o.partitions);
...
void write_bench(const Options &o, std::vector<DeviceContext> &ctx,
  ...
  for (int batch : o.batches) {
    const bool qparallel = o.force_qparallel || batch == 1 || batch == 8;
    for (int i = -o.warmup; i < o.iterations; ++i) {
      const int logical = i < 0 ? i + o.warmup : i + o.warmup;
      const BatchResult r = run_batch(
          ctx, qs, rotating_indices(batch, logical, o.query_start,
                                    o.query_span),
          qparallel, o.partitions);
      if (i >= 0)
...
void write_stress(const Options &o, std::vector<DeviceContext> &ctx,
  ...
  for (int i = 0; i < o.stress_cycles; ++i) {
    ...
    BatchResult r = run_batch(ctx, qs, std::vector<int>{512 + slot}, true,
                              o.partitions);
...
int main(int argc, char **argv) {
  try {
    ...
    QueryStore qs(qfile);
    if (o.mode == "bench" &&
        static_cast<std::size_t>(o.query_start + o.query_span) > qs.size())
      throw std::runtime_error("query-start window exceeds query store");
