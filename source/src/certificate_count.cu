...
__global__ void certificate_count_kernel(
  ...
  for (std::uint64_t b = threadIdx.x; b < blocks; b += blockDim.x) {
    ...
    const unsigned b_low = max(static_cast<unsigned>(block_minpc[b]), cmax);
    unsigned ub_den = qcount + b_low - cmax;
    if (ub_den == 0) ub_den = 1;
    const bool survives = cmax * den >= num * ub_den;
    if (survives) {
      ++surviving_blocks;
...
struct DeviceContext {
  ...
  std::uint16_t *d_minpc = nullptr, *d_qpc = nullptr, *d_num = nullptr,
                *d_den = nullptr;
  ...
  try {
    ...
    for (auto &x : ctx) {
      ...
      certificate_count_kernel<<<x.q_count, BLOCK_THREADS, 0, x.stream>>>(
          x.d_or, x.d_minpc, x.d_len, blocks, x.d_qfp, x.d_qpc, x.d_num,
