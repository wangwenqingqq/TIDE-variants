...
BIN=$WORK/build/gate6_overlap_async_campaign_v1
...
"$CUDA/bin/nvcc" -std=c++17 -O3 -lineinfo -arch=sm_120a \
  ...
  "$WORK/src/gate6_overlap_async_campaign_v1.cu" -o "$BIN" \
...
run_one fixture_overlap "$FIXTURE" "$FIXTURE_ORACLE" overlap 64 0 0
