...
void write_result(const std::string& out,const Boxes& b,const HostTree& tree,const Eval& base,const Eval& cand,bool same,const std::vector<int>& qids) {
 ...
 f<<"{\"schema\":\"safe-c2-static-aabb-topk-canary-v2-diskguard\",\"status\":\""<<(ok?"PASS_STATIC_AABB_TOPK_CANARY_V2_DISKGUARD":"FAIL_STATIC_AABB_TOPK_CANARY_V2_DISKGUARD")<<"\",\"scope\":\"new standard-SIFT static-tree top-K correctness canary; no timing, update, calibration, heldout, validation, sealed, or formal claim\",\"gpu_executed\":true,\"formal_claim_eligible\":false,";
...
int main(int argc,char**argv) {
 ...
 try {
  ...
  std::cerr<<"FAIL_STATIC_AABB_TOPK_CANARY_V2_DISKGUARD: "<<e.what()<<std::endl;if(out_d)cudaFree(out_d);if(q_d)cudaFree(q_d);release_res();release_boxes(db);release_runtime(r);return 2;
 }
}
