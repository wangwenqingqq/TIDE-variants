...
def snapshot():
    gpu=command(['nvidia-smi','-i','3','--query-gpu=uuid,name,memory.used,utilization.gpu,clocks.current.sm,clocks.current.memory,power.limit','--format=csv,noheader'])
...
def main():
    ...
    start=time.monotonic(); proc=subprocess.Popen(child,env=env)
    rec['child_pid']=proc.pid; args.record.write_text(json.dumps(rec,indent=2)+'\n')
    code=proc.wait(); after=snapshot()
