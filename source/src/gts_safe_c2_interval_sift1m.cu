// Safe-C2 corrected-after-submit full-SIFT1M interval-bound gate.
//
// Scope boundary:
...
void write_summary(const std::string& path, const Args& args, const BuildTiming& build, const Runtime& runtime,
  ...
  out << "{\n"
      << "  \"schema\": \"safe-c2-corrected-after-submit-run-v1\",\n"
      << "  \"status\": \"" << status << "\",\n"
      << "  \"scope\": \"Safe-C2 corrected-after-submit static SIFT-1M vector-KNN interval-bound experiment; empirical recall evaluation only\",\n"
      << "  \"implementation_identity\": \"corrected-after-submit; not the submitted legacy C2 kernel\",\n"
