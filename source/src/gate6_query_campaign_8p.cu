...
Options parse_options(int argc, char **argv) {
  ...
  for (int index = 1; index < argc; ++index) {
    ...
    else if (key == "--gpus") options.gpus = std::stoi(value);
  ...
  if (options.gpus != 1 && options.gpus != 2 && options.gpus != 4 && options.gpus != 8)
    throw std::runtime_error("--gpus must be 1, 2, 4, or 8");
...
template <int WORDS>
HostDBView<WORDS> shard_view(const HostDBView<WORDS> &source, int shard,
                             int shard_count) {
  const std::uint64_t begin = source.rows * shard / shard_count;
  const std::uint64_t end = source.rows * (shard + 1) / shard_count;
  HostDBView<WORDS> view;
  view.fp = source.fp + begin * WORDS;
  view.ids = source.ids + begin;
  view.pc = source.pc + begin;
  view.rows = end - begin;
  std::array<std::uint64_t, WORDS * 64 + 1> counts{};
  for (std::uint64_t row = 0; row < view.rows; ++row) counts[view.pc[row]]++;
  for (int value = 0; value <= WORDS * 64; ++value)
    view.cumulative[value + 1] = view.cumulative[value] + counts[value];
  return view;
}
...
template <int WORDS>
struct ShardedLogicalIndex {
  std::vector<OwnedHostDB<WORDS>> delta_owned;
  std::vector<HostDBView<WORDS>> host_runs;
  std::vector<std::vector<std::shared_ptr<DeviceRun<WORDS>>>> devices;
  std::vector<Snapshot<WORDS>> snapshots;
};

template <int WORDS>
ShardedLogicalIndex<WORDS> build_sharded_logical(
    int gpus, const HostDBView<WORDS> &base, const HostDBView<WORDS> &delta) {
  ShardedLogicalIndex<WORDS> result;
  result.delta_owned = split_runs<WORDS>(delta, 64);
  result.host_runs.push_back(base);
  for (const auto &owned : result.delta_owned) result.host_runs.push_back(owned.view());
  result.devices.resize(gpus);
  result.snapshots.resize(gpus);
  for (const auto &run : result.host_runs) {
    for (int gpu = 0; gpu < gpus; ++gpu) {
      const auto shard = shard_view<WORDS>(run, gpu, gpus);
      auto device = std::make_shared<DeviceRun<WORDS>>(gpu, shard);
      result.devices[gpu].push_back(device);
      result.snapshots[gpu].runs.push_back({device, shard});
    }
  }
  return result;
}
...
class MultiQueryRuntime {
 public:
  explicit MultiQueryRuntime(int gpus) {
    for (int gpu = 0; gpu < gpus; ++gpu)
      runtimes_.push_back(std::make_unique<QueryRuntime<WORDS>>(gpu));
    partial_.resize(gpus);
    errors_.resize(gpus);
    if (pthread_barrier_init(&start_barrier_, nullptr, gpus + 1) != 0 ||
        pthread_barrier_init(&done_barrier_, nullptr, gpus + 1) != 0)
      throw std::runtime_error("pthread_barrier_init failed");
    workers_.reserve(gpus);
    for (int gpu = 0; gpu < gpus; ++gpu)
      workers_.emplace_back([this, gpu] { worker_loop(gpu); });
  }
  ...
  ~MultiQueryRuntime() {
    ...
    pthread_barrier_destroy(&done_barrier_);
    pthread_barrier_destroy(&start_barrier_);
  }

  MultiQueryRuntime(const MultiQueryRuntime &) = delete;
  MultiQueryRuntime &operator=(const MultiQueryRuntime &) = delete;

  MultiResult run(const std::vector<Snapshot<WORDS>> &snapshots,
                  const Query<WORDS> &query, int num, int den) {
    const auto begin = std::chrono::steady_clock::now();
    ...
    pthread_barrier_wait(&start_barrier_);
    pthread_barrier_wait(&done_barrier_);
    for (const auto &error : errors_)
      if (error) std::rethrow_exception(error);

    MultiResult result;
    for (const auto &item : partial_) {
      result.candidate_rows += item.candidate_rows;
      result.observed_hits += item.observed_hits;
      result.overflow = result.overflow || item.overflow;
      result.maximum_kernel_ms = std::max(result.maximum_kernel_ms, item.kernel_ms);
      result.hits.insert(result.hits.end(), item.hits.begin(), item.hits.end());
    }
    result.overflow = result.overflow || result.observed_hits > OUTPUT_CAPACITY;
    sort_hits(result.hits);
    for (std::size_t index = 1; index < result.hits.size(); ++index)
      result.duplicate_id =
          result.duplicate_id || result.hits[index - 1].id == result.hits[index].id;
    result.result_hash = hash_hits(result.hits);
 ...
 private:
  void worker_loop(int gpu) {
    while (true) {
      pthread_barrier_wait(&start_barrier_);
      if (stop_) break;
      try {
        partial_[gpu] = runtimes_[gpu]->run(
            (*snapshots_)[gpu], *query_, numerator_, denominator_, true);
      } catch (...) {
        errors_[gpu] = std::current_exception();
      }
      pthread_barrier_wait(&done_barrier_);
    }
  }

  std::vector<std::unique_ptr<QueryRuntime<WORDS>>> runtimes_;
  std::vector<RunResult> partial_;
  std::vector<std::exception_ptr> errors_;
  std::vector<std::thread> workers_;
  pthread_barrier_t start_barrier_{};
  pthread_barrier_t done_barrier_{};
  const std::vector<Snapshot<WORDS>> *snapshots_ = nullptr;
  const Query<WORDS> *query_ = nullptr;
  int numerator_ = 0;
  int denominator_ = 0;
  bool stop_ = false;
};

void write_setup(const Options &options, double setup_s,
                 const std::vector<std::uint64_t> &free_before,
                 const std::vector<std::uint64_t> &free_after,
                 const std::vector<std::uint64_t> &total) {
  std::ofstream output(options.output + ".setup.json");
  output << "{\n  \"gpus\": " << options.gpus
         << ",\n  \"setup_s\": " << std::setprecision(17) << setup_s
         << ",\n  \"gpu_memory\": [\n";
  for (int gpu = 0; gpu < options.gpus; ++gpu) {
    output << "    {\"gpu\": " << gpu << ", \"free_before\": "
           << free_before[gpu] << ", \"free_after\": " << free_after[gpu]
           << ", \"used_delta\": " << free_before[gpu] - free_after[gpu]
           << ", \"total\": " << total[gpu] << "}"
           << (gpu + 1 == options.gpus ? "\n" : ",\n");
  }
  output << "  ]\n}\n";
...
template <int WORDS>
std::pair<ShardedLogicalIndex<WORDS>, double> prepare_index(
    const Options &options, Inputs<WORDS> &inputs,
    std::vector<std::uint64_t> *free_before,
    std::vector<std::uint64_t> *free_after,
    std::vector<std::uint64_t> *total) {
  free_before->resize(options.gpus);
  free_after->resize(options.gpus);
  total->resize(options.gpus);
  for (int gpu = 0; gpu < options.gpus; ++gpu) {
    CUDA_CHECK(cudaSetDevice(gpu));
    std::size_t free_bytes = 0;
    std::size_t total_bytes = 0;
    CUDA_CHECK(cudaMemGetInfo(&free_bytes, &total_bytes));
    (*free_before)[gpu] = free_bytes;
    (*total)[gpu] = total_bytes;
  }
  const auto begin = std::chrono::steady_clock::now();
  auto index = build_sharded_logical<WORDS>(options.gpus, inputs.base, inputs.delta);
  ...
  for (int gpu = 0; gpu < options.gpus; ++gpu) {
    ...
    std::size_t total_bytes = 0;
    CUDA_CHECK(cudaMemGetInfo(&free_bytes, &total_bytes));
    (*free_after)[gpu] = free_bytes;
  }
  return {std::move(index), seconds};
}
...
void run_correctness(const Options &options, Inputs<WORDS> &inputs) {
  ...
  std::vector<std::uint64_t> free_before, free_after, total;
  auto prepared = prepare_index<WORDS>(options, inputs, &free_before, &free_after, &total);
  auto &index = prepared.first;
  write_setup(options, prepared.second, free_before, free_after, total);
  MultiQueryRuntime<WORDS> runtime(options.gpus);
  ...
  samples << "gpus,query,threshold_num,threshold_den,candidates,hits,hash,"
             "oracle_hits,oracle_hash,overflow,duplicate_id,match\n";
  ...
  for (int query_index = 0; query_index < query_count; ++query_index) {
    ...
    for (const auto [num, den] :
      ...
      const auto candidate_range = bounds<WORDS>(inputs.fresh, query, num, den);
      const std::uint64_t expected_candidates =
          candidate_range.second - candidate_range.first;
      const auto result = runtime.run(index.snapshots, query, num, den);
      const bool match = !result.overflow && !result.duplicate_id &&
                         result.hits.size() == oracle.size() &&
                         result.result_hash == oracle_hash;
      checks++;
      mismatches += !match;
      candidate_mismatches += result.candidate_rows != expected_candidates;
      duplicate_ids += result.duplicate_id;
      samples << options.gpus << ',' << query_index << ',' << num << ',' << den
              << ',' << result.candidate_rows << ',' << result.hits.size() << ','
              << result.result_hash << ',' << oracle.size() << ',' << oracle_hash
              << ',' << result.overflow << ',' << result.duplicate_id << ','
              << match << '\n';
    }
  }
  const bool pass = mismatches == 0 && candidate_mismatches == 0 &&
                    duplicate_ids == 0 &&
                    checks == static_cast<std::uint64_t>(query_count) * 2;
  std::ofstream output(options.output);
  output << "{\n"
         << "  \"experiment_id\": \"tide_20260829_gate6_8p_exact\",\n"
         << "  \"gpus\": " << options.gpus << ",\n"
         << "  \"word_count\": " << WORDS << ",\n"
         ...
         << ",\n"
         << "  \"duplicate_id_requests\": " << duplicate_ids << ",\n"
         << "  \"G6_8P_EXACT_COMPONENT\": " << (pass ? "true" : "false")
         << "\n}\n";
  if (!pass) throw std::runtime_error("Gate-6 8P exact component failed");
}
...
void run_benchmark(const Options &options, Inputs<WORDS> &inputs) {
  ...
  samples << "gpus,variant,threshold_num,threshold_den,query,service_ms,"
             "maximum_kernel_ms,candidate_rows,hits,result_hash,overflow,"
             "duplicate_id\n";
  for (const auto [num, den] : thresholds(options.threshold_order)) {
    for (int warmup = 0; warmup < 16; ++warmup)
      runtime.run(index.snapshots, inputs.queries[warmup % query_count], num, den);
    for (int query_index = 0; query_index < query_count; ++query_index) {
      const auto result =
          runtime.run(index.snapshots, inputs.queries[query_index], num, den);
      if (result.overflow || result.duplicate_id)
        throw std::runtime_error("8P benchmark output contract failed");
      samples << options.gpus << ",logical64," << num << ',' << den << ','
              << query_index << ',' << std::setprecision(12) << result.service_ms
              << ',' << result.maximum_kernel_ms << ',' << result.candidate_rows
