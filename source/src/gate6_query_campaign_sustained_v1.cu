...
Options parse_options(int argc, char **argv) {
  ...
  if (options.mode != "correctness" && options.mode != "benchmark" &&
      options.mode != "sustained")
    throw std::runtime_error(
        "--mode must be correctness, benchmark, or sustained");
...
void run_sustained(
  ...
  if (variants.size() != 1 ||
    ...
    throw std::runtime_error(
        "sustained mode requires exactly one of compacted or logical64");
        ...
        for (const auto [num, den] : threshold_order) {
          for (int warmup = 0; warmup < 16; ++warmup) {
            ...
            const auto result = runtimes[reader]->run(
                *snapshot, inputs.queries[query], num, den, true);
            if (result.overflow)
