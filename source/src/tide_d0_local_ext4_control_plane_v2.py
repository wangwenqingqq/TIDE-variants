#!/usr/bin/env python3
"""Source-only D0 local-ext4 control plane v2: prepare a ledger or issue one claimed auth.
...
private ext4 authorization leaf only after a permanent nonsecret issuer claim, and never prints it.
"""
...
# A fixed nonsecret sibling reserves the global fixed-D0 issuance slot *before* any
# entropy draw. It is deliberately not a 32-hex authorization filename, so selector v6
# and runner v11 only ever open the separately generated <authorization_id>.json leaf.
ISSUER_CLAIM_LEAF = "d0_v4_pinned_preflight_issuer_claim.json"
...
CONTROL_PLANE_ROOT = "/workspace/tide_d0_control_plane_v2_issuerclaim_isolatedprobe_sourceonly_20260809T111500CST"
CONTROL_PLANE_CONTRACT_PATH = CONTROL_PLANE_ROOT + "/metadata/CONTROL_PLANE_CONTRACT.md"
CONTROL_PLANE_CONTRACT_SHA256 = "e3b650224a875f2b7385d4085f285df0027fa146cff849b0b551c147dbcc14db"
...
ISSUER_CLAIM_SCHEMA = "tide-d0-local-ext4-issuer-claim-v2-preentropy-permanent-burn"
ISSUER_CLAIM_STATUS = "PERMANENT_ISSUER_CLAIM_BEFORE_ENTROPY"
...
module_name = "_tide_d0_selector_v6_isolated_contract_probe"
module = types.ModuleType(module_name)
module.__file__ = "/workspace/sift_d0_selection_plan_generator_v6_halfopenauth_20260809T081000CST/src/sift_d0_selection_plan_generator_v6.py"
...
def issue_authorization(external_approval_id: str, intended_output_root: str) -> dict[str, Any]:
    """Claim the fixed slot before entropy, then issue one v6-compatible local authorization."""
    _require_linux_uid_1001()
    _validate_control_plane_contract()
    ...
    try:
        ...
        # This postclaim closed set is checked before every entropy draw. A concurrent
        # loser fails at O_EXCL above and never reaches getrandom().
        ...
        fds.clear()
        return {
            "status": "LOCAL_EXT4_V6_AUTHORIZATION_ISSUED_AFTER_PERMANENT_ISSUER_CLAIM",
