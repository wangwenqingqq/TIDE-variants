// Safe-C2 v4 speculative gamma traversal plus gamma-only fallback skeleton.
...
// historical provenance only and is explicitly forbidden for v4 tuning/runs.
// V4 binds a strict fresh-workload manifest before any executable run.
//
// Scope boundary:
...
//
// This file and headers live only under c2_speculative_fallback_v4. `search_v4.cuh`
...
struct Args {
  ...
  bool validate_workload_only = false;
...
Args parse_args(int argc, char** argv) {
  Args args;
  if (argc <= 0 || argv == nullptr || argv[0] == nullptr) fail("missing argv[0]");
  args.binary_path = argv[0];
  std::unordered_set<std::string> seen;
  auto set_once = [&](const std::string& key, std::string* target, const std::string& value) {
    if (!seen.insert(key).second) fail("duplicate argument " + key);
    *target = value;
  };
  for (int i = 1; i < argc; ++i) {
    ...
    if (key == "--mode") set_once(key, &args.mode, next());
    else if (key == "--base-fvecs") set_once(key, &args.base_fvecs, next());
    else if (key == "--query-fvecs") set_once(key, &args.query_fvecs, next());
    else if (key == "--groundtruth-ivecs") set_once(key, &args.groundtruth_ivecs, next());
    else if (key == "--query-ids") set_once(key, &args.query_ids, next());
    else if (key == "--out") set_once(key, &args.output_dir, next());
    else if (key == "--gamma-vector-file") set_once(key, &args.gamma_vector_file, next());
    else if (key == "--stage") set_once(key, &args.stage, next());
    else if (key == "--workload-manifest") set_once(key, &args.workload_manifest, next());
    else if (key == "--validation-receipt") set_once(key, &args.validation_receipt, next());
    else if (key == "--validate-workload-only") {
      if (!seen.insert(key).second) fail("duplicate argument " + key);
      args.validate_workload_only = true;
    } else if (key == "--warmup-reps") {
      if (!seen.insert(key).second) fail("duplicate argument " + key);
      args.saw_warmup = true; args.warmup_reps = std::stoi(next());
    } else if (key == "--timed-reps") {
      if (!seen.insert(key).second) fail("duplicate argument " + key);
      args.saw_timed = true; args.timed_reps = std::stoi(next());
    } else if (key == "--baseline-target-correct") {
      if (!seen.insert(key).second) fail("duplicate argument " + key);
      args.baseline_target_correct = std::stoll(next());
    } else if (key == "--help") {
      std::cout
          << "normal usage: GTS_safe_c2_v4 --mode calibrate|evaluate --base-fvecs BASE "
          << "--query-fvecs QUERY --groundtruth-ivecs GT --query-ids IDS --out DIR "
          << "--stage calibration|validation|test --workload-manifest MANIFEST "
          << "[--gamma-vector-file FILE] [--warmup-reps 1] [--timed-reps N]\n"
          << "pre-ledger parser gate: GTS_safe_c2_v4 --validate-workload-only --mode calibrate "
          << "--base-fvecs BASE --query-fvecs QUERY --groundtruth-ivecs GT --query-ids IDS "
          << "--stage calibration --workload-manifest MANIFEST --validation-receipt RECEIPT\n";
      std::exit(0);
    } else {
  ...
  if (args.base_fvecs.empty() || args.query_fvecs.empty() || args.groundtruth_ivecs.empty() ||
      args.query_ids.empty() || args.stage.empty() || args.workload_manifest.empty()) {
    fail("missing required workload input argument");
  }
  if (args.warmup_reps < 0 || args.timed_reps <= 0) fail("invalid repetition counts");
  if (args.validate_workload_only) {
    if (!args.output_dir.empty() || args.validation_receipt.empty() || args.saw_warmup || args.saw_timed ||
        !args.gamma_vector_file.empty() || args.baseline_target_correct != -1) {
      fail("--validate-workload-only accepts only workload binding arguments and --validation-receipt");
    }
    if (args.mode != "calibrate" || args.stage != "calibration") {
      fail("--validate-workload-only is deliberately limited to the formal calibration binding");
    }
    return args;
  }
  if (!args.validation_receipt.empty()) fail("--validation-receipt is only valid with --validate-workload-only");
  if (args.output_dir.empty()) fail("normal execution requires --out");
  if (args.mode == "calibrate") {
    ...
    if (args.gamma_vector_file.empty()) fail("evaluate mode requires --gamma-vector-file");
...
// strict manifest-bound workload.  Do not substitute a shell
...
// Strict JSON parsing and workload binding.  This intentionally does not use
// substring search: runner-visible fields are read only from the root JSON
// object, duplicate keys are rejected in every object scope, and every value
// type is checked before it is used.
...
class StrictJsonParser {
 public:
  ...
  JsonValue parse_document() {
    ...
    skip_whitespace();
    if (pos_ != text_.size()) parse_fail("trailing bytes after JSON document");
    return value;
  }
 ...
 private:
  ...
  [[noreturn]] void parse_fail(const std::string& detail) const {
    throw std::runtime_error("SAFE-C2: strict JSON parse failure at byte " +
  ...
  void skip_whitespace() {
    while (pos_ < text_.size() && whitespace(static_cast<unsigned char>(text_[pos_]))) ++pos_;
  }
  ...
  void require(char expected, const char* label) {
    if (!take(expected)) parse_fail(std::string("expected ") + label);
  }

  unsigned char peek_byte() const {
    if (pos_ >= text_.size()) parse_fail("unexpected end of document");
  ...
  std::uint32_t unicode_quad() {
    if (text_.size() - pos_ < 4) parse_fail("truncated unicode escape");
    std::uint32_t value = 0;
    for (int i = 0; i < 4; ++i) {
      const int digit = hex_digit(static_cast<unsigned char>(text_[pos_++]));
      if (digit < 0) parse_fail("non-hex unicode escape");
  ...
  void append_raw_utf8(std::string* out) {
    const std::size_t start = pos_;
    const unsigned char lead = peek_byte();
    int count = 0;
    if (lead >= 0xc2U && lead <= 0xdfU) count = 2;
    else if (lead >= 0xe0U && lead <= 0xefU) count = 3;
    else if (lead >= 0xf0U && lead <= 0xf4U) count = 4;
    else parse_fail("invalid UTF-8 lead byte in JSON string");
    if (text_.size() - start < static_cast<std::size_t>(count)) {
      parse_fail("truncated UTF-8 sequence in JSON string");
    }
    const unsigned char second = static_cast<unsigned char>(text_[start + 1]);
    if ((lead == 0xe0U && second < 0xa0U) || (lead == 0xedU && second > 0x9fU) ||
        (lead == 0xf0U && second < 0x90U) || (lead == 0xf4U && second > 0x8fU)) {
      parse_fail("overlong or surrogate UTF-8 sequence in JSON string");
    }
    for (int i = 1; i < count; ++i) {
      const unsigned char c = static_cast<unsigned char>(text_[start + static_cast<std::size_t>(i)]);
      if ((c & 0xc0U) != 0x80U) parse_fail("invalid UTF-8 continuation byte in JSON string");
    }
    out->append(text_, start, static_cast<std::size_t>(count));
    pos_ += static_cast<std::size_t>(count);
  }

  std::string parse_string() {
    require('"', "opening JSON string quote");
    std::string out;
    while (pos_ < text_.size()) {
      const unsigned char c = static_cast<unsigned char>(text_[pos_++]);
      if (c == '"') return out;
      if (c < 0x20U) parse_fail("unescaped control byte in JSON string");
      if (c == '\\') {
        if (pos_ >= text_.size()) parse_fail("truncated JSON escape");
        const unsigned char escape = static_cast<unsigned char>(text_[pos_++]);
        switch (escape) {
          ...
          case 'u': {
            std::uint32_t codepoint = unicode_quad();
            if (codepoint >= 0xd800U && codepoint <= 0xdbffU) {
              if (text_.size() - pos_ < 2 || text_[pos_] != '\\' || text_[pos_ + 1] != 'u') {
                parse_fail("high surrogate without a low-surrogate escape");
              }
              pos_ += 2;
              const std::uint32_t low = unicode_quad();
              if (low < 0xdc00U || low > 0xdfffU) parse_fail("invalid low surrogate");
              codepoint = 0x10000U + ((codepoint - 0xd800U) << 10U) + (low - 0xdc00U);
            } else if (codepoint >= 0xdc00U && codepoint <= 0xdfffU) {
              parse_fail("unpaired low surrogate");
            }
            append_utf8(&out, codepoint);
            break;
          }
          default: parse_fail("unknown JSON string escape");
        }
      } else if (c >= 0x80U) {
        --pos_;
        append_raw_utf8(&out);
    ...
    parse_fail("unterminated JSON string");
  }

  JsonValue parse_number() {
    const std::size_t start = pos_;
    take('-');
    if (pos_ >= text_.size()) parse_fail("truncated number");
    if (take('0')) {
      if (pos_ < text_.size() && std::isdigit(static_cast<unsigned char>(text_[pos_]))) {
        parse_fail("leading zero in JSON number");
      }
    } else {
      if (pos_ >= text_.size() || text_[pos_] < '1' || text_[pos_] > '9') {
        parse_fail("invalid JSON number integer component");
      }
      do { ++pos_; } while (pos_ < text_.size() && std::isdigit(static_cast<unsigned char>(text_[pos_])));
    }
    if (take('.')) {
      if (pos_ >= text_.size() || !std::isdigit(static_cast<unsigned char>(text_[pos_]))) {
        parse_fail("invalid JSON number fractional component");
      }
      do { ++pos_; } while (pos_ < text_.size() && std::isdigit(static_cast<unsigned char>(text_[pos_])));
    }
    if (pos_ < text_.size() && (text_[pos_] == 'e' || text_[pos_] == 'E')) {
      ...
      if (pos_ >= text_.size() || !std::isdigit(static_cast<unsigned char>(text_[pos_]))) {
        parse_fail("invalid JSON number exponent");
      }
      do { ++pos_; } while (pos_ < text_.size() && std::isdigit(static_cast<unsigned char>(text_[pos_])));
    }
    JsonValue value;
  ...
  JsonValue parse_array() {
    require('[', "opening JSON array bracket");
    JsonValue value;
    ...
    while (true) {
      ...
      skip_whitespace();
      if (take(']')) return value;
      require(',', "JSON array comma");
      skip_whitespace();
    }
  }

  JsonValue parse_object() {
    require('{', "opening JSON object brace");
    JsonValue value;
    ...
    while (true) {
      skip_whitespace();
      if (pos_ >= text_.size() || text_[pos_] != '"') parse_fail("JSON object key is not a string");
      const std::string key = parse_string();
      if (!keys.insert(key).second) parse_fail("duplicate JSON object key");
      skip_whitespace();
      ...
      skip_whitespace();
      if (take('}')) return value;
      require(',', "JSON object comma");
      skip_whitespace();
    }
  }

  JsonValue parse_value() {
    if (++depth_ > 128) parse_fail("JSON nesting depth exceeds 128");
    skip_whitespace();
    if (pos_ >= text_.size()) parse_fail("missing JSON value");
    JsonValue value;
    const char c = text_[pos_];
    if (c == '{') value = parse_object();
    else if (c == '[') value = parse_array();
    else if (c == '"') { value.type = JsonType::kString; value.text = parse_string(); }
    else if (c == 't') {
      if (text_.compare(pos_, 4, "true") != 0) parse_fail("invalid JSON literal");
      pos_ += 4; value.type = JsonType::kBoolean; value.boolean = true;
    } else if (c == 'f') {
      if (text_.compare(pos_, 5, "false") != 0) parse_fail("invalid JSON literal");
      pos_ += 5; value.type = JsonType::kBoolean; value.boolean = false;
    } else if (c == 'n') {
      if (text_.compare(pos_, 4, "null") != 0) parse_fail("invalid JSON literal");
      pos_ += 4; value.type = JsonType::kNull;
    } else if (c == '-' || (c >= '0' && c <= '9')) {
      value = parse_number();
    } else {
      parse_fail("unexpected JSON value byte");
    }
    --depth_;
    return value;
  }
...
const JsonValue& require_top_field(const JsonValue& root, const std::string& key) {
  if (root.type != JsonType::kObject) fail("workload manifest root must be a JSON object");
  for (const auto& entry : root.object) {
    if (entry.first == key) return entry.second;
  }
  fail("workload manifest lacks required top-level field " + key);
}

void require_non_nul(const std::string& value, const std::string& label) {
  for (unsigned char c : value) {
    if (c == 0U) fail(label + " must not contain NUL");
  }
}

std::string require_top_string(const JsonValue& root, const std::string& key) {
  const JsonValue& value = require_top_field(root, key);
  if (value.type != JsonType::kString) fail("workload manifest top-level field is not a JSON string: " + key);
  require_non_nul(value.text, "workload manifest top-level field " + key);
  return value.text;
}

bool require_top_boolean(const JsonValue& root, const std::string& key) {
  const JsonValue& value = require_top_field(root, key);
  if (value.type != JsonType::kBoolean) fail("workload manifest top-level field is not a JSON boolean: " + key);
  return value.boolean;
}

bool only_decimal_digits(const std::string& text) {
...
long long require_top_positive_integer(const JsonValue& root, const std::string& key) {
  const JsonValue& value = require_top_field(root, key);
  if (value.type != JsonType::kNumber || value.text.empty()) {
    fail("workload manifest top-level field is not a JSON integer: " + key);
  }
  if (value.text[0] == '-' || value.text[0] == '+' || value.text[0] == '0' ||
      !only_decimal_digits(value.text)) {
    fail("workload manifest top-level field is not a positive canonical integer: " + key);
  }
  long long parsed = 0;
  for (char c : value.text) {
    if (parsed > (std::numeric_limits<long long>::max() - (c - '0')) / 10LL) {
      fail("workload manifest integer overflows: " + key);
    }
...
void require_exact_top_string(const JsonValue& root, const std::string& key, const std::string& expected) {
  const std::string actual = require_top_string(root, key);
  if (actual != expected) {
    fail("workload manifest top-level field mismatch for " + key);
  }
}
...
constexpr const char* kV4Root = "/workspace/experiments/tide_safe_c1_20260727/c2_speculative_fallback_v4";
constexpr const char* kV2Root = "/workspace/experiments/tide_safe_c1_20260727/c2_interval_bound_safe_v2_tiefree";
constexpr const char* kV2SealedTestIdsSha256 = "50ccb28263bf23e499b9c50e4d3e9800fca3949aa5b5ea8a454237d5987701ce";
constexpr const char* kV4WorkloadSchema = "gts-v4-fresh-sift-learn-workload-v1";
constexpr const char* kV4WorkloadStatus = "READY_FOR_C2_V4_FORMAL_AFTER_PRELEDGER_PARSER_GATE";
...
void require_direct_regular_file(const std::string& path, const std::string& label) {
  if (path.empty() || path[0] != '/') fail(label + " must be an absolute path");
  struct stat st {};
  if (::lstat(path.c_str(), &st) != 0) fail("cannot lstat " + label + ": " + path);
  if (!S_ISREG(st.st_mode)) fail(label + " must be a direct regular file: " + path);
  char resolved[PATH_MAX] = {};
  if (::realpath(path.c_str(), resolved) == nullptr) fail("cannot canonicalize " + label + ": " + path);
  if (path != std::string(resolved)) fail(label + " must not traverse a symlink: " + path);
}

void require_direct_directory(const std::string& path, const std::string& label) {
  if (path.empty() || path[0] != '/') fail(label + " must be an absolute path");
  struct stat st {};
  if (::lstat(path.c_str(), &st) != 0) fail("cannot lstat " + label + ": " + path);
  if (!S_ISDIR(st.st_mode)) fail(label + " must be a direct directory: " + path);
  char resolved[PATH_MAX] = {};
  if (::realpath(path.c_str(), resolved) == nullptr) fail("cannot canonicalize " + label + ": " + path);
  if (path != std::string(resolved)) fail(label + " must not traverse a symlink: " + path);
}

std::uintmax_t file_size_bytes(const std::string& path, const std::string& label) {
  struct stat st {};
  if (::stat(path.c_str(), &st) != 0 || st.st_size < 0) fail("cannot stat " + label + ": " + path);
  return static_cast<std::uintmax_t>(st.st_size);
}

std::size_t last_path_separator(const std::string& path) {
  for (std::size_t i = path.size(); i > 0; --i) {
    if (path[i - 1] == '/') return i - 1;
  }
  fail("path has no separator");
}

long long count_canonical_id_lines(const std::string& path, const std::string& label) {
  std::ifstream input(path, std::ios::binary);
  if (!input) fail("cannot open " + label + " for ID count");
  std::string line;
  long long count = 0;
  std::unordered_set<long long> seen_ids;
  while (std::getline(input, line)) {
    if (line.empty()) fail(label + " contains blank ID line");
    if (line.size() > 1 && line[0] == '0') fail(label + " contains noncanonical leading-zero ID");
    long long value = 0;
    for (unsigned char c : line) {
      if (c < '0' || c > '9') fail(label + " contains non-decimal ID line");
      if (value > (std::numeric_limits<long long>::max() - static_cast<long long>(c - '0')) / 10LL) {
        fail(label + " ID overflows signed range");
      }
      value = value * 10LL + static_cast<long long>(c - '0');
    }
    if (value < 0 || value >= kQueryN) fail(label + " ID outside v4 compact query range");
    if (!seen_ids.insert(value).second) fail(label + " contains duplicate ID");
    if (++count > kQueryN) fail(label + " has too many IDs");
  }
  if (!input.eof()) fail("failed while counting " + label + " IDs");
...
struct WorkloadBinding {
  ...
  std::vector<BoundFile> files;
};

void reject_unknown_or_missing_top_keys(const JsonValue& root) {
  if (root.type != JsonType::kObject) fail("workload manifest root must be an object");
  ...
  if (root.object.size() != expected.size()) fail("workload manifest top-level key count is not exact");
  for (const auto& entry : root.object) {
    if (expected.count(entry.first) != 1U) fail("workload manifest has unknown top-level key " + entry.first);
  }
}

BoundFile bind_manifest_file(const JsonValue& root, const std::string& label,
                             const std::string& path_key, const std::string& hash_key,
                             bool must_live_under_v4) {
  const std::string path = require_top_string(root, path_key);
  const std::string expected = require_top_string(root, hash_key);
  if (!is_sha256_hex(expected)) fail("workload manifest has invalid SHA-256 field " + hash_key);
  if (strict_existing_child_path(path, kV2Root)) fail("v2 root is forbidden for " + label);
  if (must_live_under_v4 && !strict_existing_child_path(path, kV4Root)) {
    fail(label + " must live under the isolated v4 root");
  }
  require_direct_regular_file(path, label);
  const std::string observed = sha256_hex_file(path);
  if (observed != expected) fail("workload input SHA-256 mismatch for " + label);
  return BoundFile{label, path, observed, file_size_bytes(path, label)};
}
...
WorkloadBinding validate_workload_binding(const Args& args) {
  if (!strict_existing_child_path(args.workload_manifest, kV4Root)) {
    fail("workload manifest must reside under isolated v4 root");
  }
  require_direct_regular_file(args.workload_manifest, "workload manifest");
  const std::string manifest_text = read_text_file_strict(args.workload_manifest);
  const JsonValue root = StrictJsonParser(manifest_text).parse_document();
  reject_unknown_or_missing_top_keys(root);
  require_exact_top_string(root, "schema", kV4WorkloadSchema);
  require_exact_top_string(root, "status", kV4WorkloadStatus);
  if (!require_top_boolean(root, "sealed_v2_test_forbidden")) {
    fail("workload manifest must explicitly forbid v2 sealed-test reuse");
  }
  require_exact_top_string(root, "v2_sealed_test_ids_sha256_forbidden", kV2SealedTestIdsSha256);
  if (require_top_positive_integer(root, "query_fvecs_count") != kQueryN) {
    fail("workload query_fvecs_count does not match v4 source contract");
  }
  if (require_top_positive_integer(root, "groundtruth_width") != kGtWidth) {
    fail("workload groundtruth_width does not match v4 source contract");
  }

  WorkloadBinding binding;
  binding.manifest_path = args.workload_manifest;
  binding.manifest_sha256 = sha256_hex_file(args.workload_manifest);
  binding.files.push_back(bind_manifest_file(root, "selection_pre_gt", "selection_pre_gt_path", "selection_pre_gt_sha256", true));
  binding.files.push_back(bind_manifest_file(root, "v3_forensic_record", "v3_forensic_record_path", "v3_forensic_record_sha256", true));
  binding.files.push_back(bind_manifest_file(root, "formal_protocol", "formal_protocol_path", "formal_protocol_sha256", true));
  const BoundFile base = bind_manifest_file(root, "base_fvecs", "base_fvecs_path", "base_fvecs_sha256", false);
  const BoundFile queries = bind_manifest_file(root, "query_fvecs", "query_fvecs_path", "query_fvecs_sha256", true);
  const BoundFile groundtruth = bind_manifest_file(root, "groundtruth_ivecs", "groundtruth_ivecs_path", "groundtruth_ivecs_sha256", true);
  const BoundFile mapping = bind_manifest_file(root, "mapping", "mapping_path", "mapping_sha256", true);
  if (args.base_fvecs != base.path || args.query_fvecs != queries.path ||
      args.groundtruth_ivecs != groundtruth.path) {
    fail("runner arguments do not exactly match strict top-level workload input paths");
  }
  binding.files.push_back(base);
  binding.files.push_back(queries);
  binding.files.push_back(groundtruth);
  binding.files.push_back(mapping);

  const std::array<StageBinding, 3> all_stages = {{{"calibration", "calibration_ids_path", "calibration_ids_sha256", "calibration_query_count"},
                                                     {"validation", "validation_ids_path", "validation_ids_sha256", "validation_query_count"},
                                                     {"sealed_test", "sealed_test_ids_path", "sealed_test_ids_sha256", "sealed_test_query_count"}}};
  std::string selected_ids_path;
  for (const StageBinding& stage : all_stages) {
    const BoundFile ids = bind_manifest_file(root, stage.logical_name + "_ids", stage.path_key, stage.hash_key, true);
    const long long expected_count = require_top_positive_integer(root, stage.count_key);
    const long long observed_count = count_canonical_id_lines(ids.path, stage.logical_name + " IDs");
    if (observed_count != expected_count) fail("workload ID count mismatch for " + stage.logical_name);
    if (args.stage == "calibration" && stage.logical_name == "calibration") selected_ids_path = ids.path;
    if (args.stage == "validation" && stage.logical_name == "validation") selected_ids_path = ids.path;
    if (args.stage == "test" && stage.logical_name == "sealed_test") selected_ids_path = ids.path;
    binding.files.push_back(ids);
  }
  if (selected_ids_path.empty() || args.query_ids != selected_ids_path) {
    fail("runner --query-ids does not exactly match strict top-level selected-stage path");
  }
  if (!args.gamma_vector_file.empty() && !strict_existing_child_path(args.gamma_vector_file, kV4Root)) {
    fail("evaluate gamma vector must reside under isolated v4 root");
  }
...
std::string receipt_json_escape(const std::string& raw) {
  std::ostringstream out;
...
void write_all_or_fail(int fd, const std::string& bytes, const std::string& label) {
  std::size_t offset = 0;
  while (offset < bytes.size()) {
    const ssize_t written = ::write(fd, bytes.data() + offset, bytes.size() - offset);
    if (written <= 0) fail("cannot write " + label);
    offset += static_cast<std::size_t>(written);
  }
}

void write_workload_validation_receipt(const Args& args, const WorkloadBinding& binding) {
  const std::string& path = args.validation_receipt;
  const std::string receipts_root = std::string(kV4Root) + "/preledger_receipts";
  if (!strict_child_path(path, receipts_root)) fail("validation receipt must be a child of v4 preledger_receipts");
  const std::size_t slash = last_path_separator(path);
  const std::string parent = path.substr(0, slash);
  require_direct_directory(parent, "validation receipt parent");
  struct stat existing {};
  if (::lstat(path.c_str(), &existing) == 0 || errno != ENOENT) {
    fail("validation receipt path already exists or cannot be checked");
  }
  require_direct_regular_file(args.binary_path, "running final v4 binary");
  const std::string binary_sha = sha256_hex_file(args.binary_path);
  ...
  receipt << "{\n"
          ...
          << "  \"binary\": {\"path\": \"" << receipt_json_escape(args.binary_path)
          << "\", \"sha256\": \"" << binary_sha << "\"},\n"
          << "  \"manifest\": {\"path\": \"" << receipt_json_escape(binding.manifest_path)
          << "\", \"sha256\": \"" << binding.manifest_sha256 << "\"},\n"
          << "  \"arguments\": {\"mode\": \"" << receipt_json_escape(args.mode)
          << "\", \"stage\": \"" << receipt_json_escape(args.stage) << "\"},\n"
          << "  \"strict_top_level_parser\": true,\n"
          << "  \"input_bindings\": [";
  for (std::size_t i = 0; i < binding.files.size(); ++i) {
    const BoundFile& item = binding.files[i];
    if (i) receipt << ',';
    receipt << "\n    {\"label\":\"" << receipt_json_escape(item.label)
            << "\",\"path\":\"" << receipt_json_escape(item.path)
            << "\",\"sha256\":\"" << item.sha256
            << "\",\"bytes\":" << item.bytes << '}';
  }
  receipt << "\n  ]\n}\n";
  const int fd = ::open(path.c_str(), O_WRONLY | O_CREAT | O_EXCL | O_NOFOLLOW, S_IRUSR | S_IWUSR | S_IRGRP);
  if (fd < 0) fail("cannot create validation receipt");
  try {
    write_all_or_fail(fd, receipt.str(), "validation receipt");
    if (::fsync(fd) != 0) fail("cannot fsync validation receipt");
    if (::close(fd) != 0) fail("cannot close validation receipt");
  } catch (...) {
    ::close(fd);
    ::unlink(path.c_str());
    throw;
...
std::uint64_t write_per_query_jsonl(const std::string& path, const std::string& role, int timed_rep,
  ...
  std::ofstream out(path);
  if (!out) fail("cannot write v4 per-query JSONL");
  for (std::size_t i = 0; i < run.final_guarded_per_query.size(); ++i) {
    ...
    out << std::setprecision(10)
        << "{\"schema\":\"safe-c2-v4-per-query-v1\",\"role\":\"" << role
        << "\",\"timed_rep\":" << timed_rep
  ...
  // The summary binds the byte-level FNV of the completed artifact, not an
  ...
  std::ifstream input(path, std::ios::binary);
  if (!input) fail("cannot reopen v4 per-query JSONL");
...
void write_baseline_invalid_diagnostic(const std::string& path, const Args& args,
  ...
  out << std::setprecision(10)
      << "{\n"
      << "  \"schema\": \"safe-c2-v4-baseline-invalid-diagnostic-v1\",\n"
      << "  \"status\": \"FAIL_BASELINE_REFERENCE_BEFORE_GAMMA\",\n"
      << "  \"v4_speculative_fallback\": true,\n"
      << "  \"v2_sealed_test_policy\": \"consumed historical provenance; forbidden for v4 input/tuning\",\n"
...
void write_summary(const std::string& path, const Args& args, const BuildTiming& build, const Runtime& runtime,
  ...
  out << "{\n"
      ...
      << "  \"status\": \"" << status << "\",\n"
      << "  \"scope\": \"Safe-C2 v4 speculative gamma traversal with gamma-only fallback on a strict fresh manifest-bound workload; calibration selects a fixed gamma vector only and held-out evaluation is separately gated\",\n"
      << "  \"implementation_identity\": \"v4 speculative-fallback skeleton; not v2 and not the submitted legacy C2 kernel\",\n"
      << "  \"v4_speculative_fallback\": true,\n"
      << "  \"v2_sealed_test_policy\": \"consumed historical provenance; forbidden for v4 gamma tuning, calibration, evaluation, retry, or input binding\",\n"
      << "  \"workload_binding\": {\"manifest\": \"" << json_escape(args.workload_manifest)
      << "\", \"schema\": \"" << kV4WorkloadSchema << "\", \"sha256_verified_in_runner\": true},\n"
      << "  \"mode\": \"" << args.mode << "\", \"stage\": \"" << args.stage << "\",\n"
...
int safe_c2_main(int argc, char** argv) {
  // This flag protects the validation-only error path as well as its success
  // path: no CUDA cleanup/API is reachable until after strict parser/hash
  // validation has succeeded and normal execution explicitly begins.
  bool normal_execution_phase_entered = false;
  Runtime runtime;
  float* selected_queries_h = nullptr;
  float* selected_queries_d = nullptr;
  try {
    const Args args = parse_args(argc, argv);
    // Strict root-object parser + byte hashes happen before any vector/GT
    // decoding, tree construction, CUDA allocation, CUDA event, or CUDA API.
    const WorkloadBinding workload = validate_workload_binding(args);
    if (args.validate_workload_only) {
      write_workload_validation_receipt(args, workload);
      return 0;
    }
    normal_execution_phase_entered = true;
    ...
    if (args.mode == "calibrate") {
      ...
      const auto all_ones = all_ones_gamma();
      // Calibration split only: gamma=1 establishes the reference.  The v4
      ...
      write_gamma_vector(args.output_dir + "/final_v4_speculative_gamma_vector.txt", gamma);
    } else {
      ...
      for (const auto& counters : paired_baseline_result.final_guarded_repetitions) {
        if (!counters.valid() || counters.correct != baseline_target ||
          ...
          fail("paired ABBA gamma=1 baseline is invalid or per-query nondeterministic; v4 heldout stage is blocked");
        }
      }
    ...
    const std::string status = passed ? (args.mode == "calibrate" ? "PASS_V4_CALIBRATION_GUARDED" : "PASS_V4_HELDOUT_GUARDED")
                                      : (args.mode == "calibrate" ? "FAIL_V4_CALIBRATION_GUARDED" : "FAIL_V4_HELDOUT_GUARDED");
