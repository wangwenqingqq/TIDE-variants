...
[[noreturn]] void fail(const std::string& m) { throw std::runtime_error("STATIC_AABB_TOPK_CANARY_V2_DISKGUARD: "+m); }
...
Args parse_args(int argc,char** argv) {
 ...
 if(a.projection_dims<=0||a.projection_dims>kDim) fail("projection width outside 1..128");
 return a;
}
...
std::vector<int> read_ids(const std::string& p) {
 ...
 if((int)v.size()!=32)fail("top-K canary requires exactly 32 IDs");
...
void write_result(const std::string& out,const Boxes& b,const HostTree& tree,const Eval& base,const Eval& cand,bool same,const std::vector<int>& qids) {
 ...
 f<<"{\"schema\":\"safe-c2-static-aabb-topk-canary-v2-diskguard\",\"status\":\""<<(ok?"PASS_STATIC_AABB_TOPK_CANARY_V2_DISKGUARD_V2_DISKGUARD":"FAIL_STATIC_AABB_TOPK_CANARY_V2_DISKGUARD_V2_DISKGUARD")<<"\",\"scope\":\"new standard-SIFT static-tree top-K correctness canary; no timing, update, calibration, heldout, validation, sealed, or formal claim\",\"gpu_executed\":true,\"formal_claim_eligible\":false,";
...
int main(int argc,char**argv) {
 ...
 try {
  const Args a=parse_args(argc,argv);const auto base=read_base(a.base);const auto qids=read_ids(a.ids);const auto q=read_query_rows(a.queries,qids);const auto dims=top_variance_dims(base,a.projection_dims);
  ...
  std::cerr<<"FAIL_STATIC_AABB_TOPK_CANARY_V2_DISKGUARD_V2_DISKGUARD: "<<e.what()<<std::endl;if(out_d)cudaFree(out_d);if(q_d)cudaFree(q_d);release_res();release_boxes(db);release_runtime(r);return 2;
 }
}
