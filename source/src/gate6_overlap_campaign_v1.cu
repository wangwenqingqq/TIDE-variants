...
PublicationResult publish_epoch_one(
  ...
  auto delta_device =
      std::make_shared<DeviceRun<4>>(options.gpu, delta_view);
...
void run_overlap(const Options &options) {
  ...
  auto probe_publication = publish_epoch_one(
      options, inputs.base, base_device, current_snapshot, probe_origin);
    ...
    updater = std::thread([&] {
      try {
        ...
        publication = publish_epoch_one(
            options, inputs.base, base_device, current_snapshot, retained_begin);
      } catch (...) {
  ...
  summary << "{\n"
          << "  \"experiment_id\": "
             "\"tide_20260903_read_while_update_epoch_consistency\",\n"
