...
def main() -> None:
    ...
    for bs in BLOCK_SIZES:
        ...
        for q in VERIFY_QUERIES:
            ...
            blow = np.maximum(bmin, cmax)
            ubden = qpc[q].astype(np.uint32) + blow.astype(np.uint32) - cmax
            ubden[ubden == 0] = 1
            survive = cmax.astype(np.uint32) * int(den[q]) >= int(num[q]) * ubden
            ...
            if len(pruned):
                ...
                for b in chosen:
                    ...
                    bad = inter * int(ubden[b]) > int(cmax[b]) * actual_den
        ...
        for q in range(nq):
            ...
            blow = np.maximum(bmin[b].astype(np.uint32), cmax)
            ubden = int(qpc[q]) + blow - cmax
            ubden[ubden == 0] = 1
            survives = cmax * int(den[q]) >= int(num[q]) * ubden
