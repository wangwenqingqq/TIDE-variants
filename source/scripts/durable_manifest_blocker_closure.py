#!/usr/bin/env python3
"""Two-phase crash-injected durable run protocol for Gate-5 blocker closure.
...
RUN_CRASH_STEPS = CRASH_STEPS[:4]
MANIFEST_CRASH_STEPS = CRASH_STEPS[4:]
...
def stage_run(
    store: Path, source: Path, epoch: int, crash_after: str | None
) -> dict[str, Any]:
    """Durably create one immutable run without changing the committed manifest."""
    initialize(store)
    before = recover(store)
    if epoch <= before["epoch"]:
        raise ValueError(f"epoch {epoch} must exceed current epoch {before['epoch']}")
    ...
    atomic_json(store / "STAGED.json", record)
    return record


def commit_staged(store: Path, crash_after: str | None) -> dict[str, Any]:
    """Commit a previously durable staged run into the manifest."""
    initialize(store)
    before = recover(store)
    staged_path = store / "STAGED.json"
    if not staged_path.is_file():
        raise ValueError("missing STAGED.json")
    run_record = json.loads(staged_path.read_text(encoding="utf-8"))
    run_path = store / run_record["path"]
    if not run_path.is_file() or sha256_file(run_path) != run_record["file_sha256"]:
        raise ValueError("staged run is absent or hash-invalid")
    header, payload_offset = read_run(run_path)
    if sha256_file(run_path, payload_offset) != run_record["payload_sha256"]:
        raise ValueError("staged run payload is hash-invalid")
    if run_record["epoch"] <= before["epoch"]:
        raise ValueError("staged epoch is not newer than the manifest")
    manifest_value = {
        "format": "tide-gate5-manifest-v1",
        "epoch": run_record["epoch"],
        "runs": before["runs"] + [run_record],
    }
    manifest = store / "CURRENT.json"
    manifest_temp = store / f"CURRENT.json.tmp.{os.getpid()}"
...
def materialize(store: Path, output: Path) -> dict[str, Any]:
    """Materialize committed payload bytes after validating the full manifest."""
    state = recover(store)
    output.parent.mkdir(parents=True, exist_ok=True)
    with output.open("wb") as destination:
        for run in state.get("runs", []):
            path = store / run["path"]
            _, payload_offset = read_run(path)
...
def main() -> int:
    ...
    stage_parser = subparsers.add_parser("stage-run")
    stage_parser.add_argument("--store", type=Path, required=True)
    stage_parser.add_argument("--input", type=Path, required=True)
    stage_parser.add_argument("--epoch", type=int, required=True)
    stage_parser.add_argument("--crash-after", choices=RUN_CRASH_STEPS)

    commit_staged_parser = subparsers.add_parser("commit-staged")
    commit_staged_parser.add_argument("--store", type=Path, required=True)
    commit_staged_parser.add_argument("--crash-after", choices=MANIFEST_CRASH_STEPS)

    materialize_parser = subparsers.add_parser("materialize")
    materialize_parser.add_argument("--store", type=Path, required=True)
    ...
    if args.command == "init":
        ...
        result = stage_run(args.store, args.input, args.epoch, args.crash_after)
    elif args.command == "commit-staged":
        result = commit_staged(args.store, args.crash_after)
    elif args.command == "materialize":
        result = materialize(args.store, args.output)
