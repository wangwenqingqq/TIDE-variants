...
sidecar only if a strict root-to-leaf ball-cover certificate
...
native-GTS generality.  v3 refuses test/heldout protocols and is usable only
...
class FrozenBallCoverTree:
    """Balanced frozen ball-cover tree with strict child-cover certificates.
    ...
    inside a child ball at every internal node.  When two cover balls overlap,
    the host routing rule deterministically chooses the nearer center (then
    stable center ID); visibility follows from membership in the chosen ball.
    Query pruning uses
    ...
    def certify_leaf(self, vector: np.ndarray) -> Optional[int]:
        ...
        while True:
            ...
            if d_left < left.radius:
                candidates.append((d_left, left.center_id, int(node.left_id)))
            if d_right < right.radius:
                candidates.append((d_right, right.center_id, int(node.right_id)))
            if not candidates:
                return None
            # This is a routing choice, not a proof by nearest-gap: every
            # candidate here already supplies a strict child-ball envelope.
...
def run(args: argparse.Namespace) -> None:
    ...
    summary = {
        "schema": "tide-safe-c1-reference-mirror-run-v3",
        ...
        "policy": {"name": args.policy, "certificate": "strict_child_ball_cover_path_nearest_center_tie_by_id", "leaf_size": args.leaf_size, "sidecar_capacity": args.sidecar_capacity, "delta_bound": args.delta_bound, "rebuild_policy": "forbidden"},
