...
// Controlled, single-translation-unit Safe-C1 G3 4K full-trace correctness gate.
//
...
constexpr char kReleaseRoot[] =
    ...
    "safe_c1_native_matrix_g3_v24_fulltrace_nativecertfix_jsonseal";
...
constexpr ExpectedFile kBootstrapFiles[] = {
    ...
    {"bootstrap_oracle_validation.json",
     "da0fe87a69c83d65dc652342e2bf7684650fabbe1453352a2ca50666d7486bcf", 0},
};
...
std::string ticket_attestation_json(const std::string& phase, int op_index,
  ...
  out << "{\"record\":\"ticket_attestation\",\"schema\":\"safe-c1-g3-ticket-attestation-v3\",\"phase\":\"" << json_escape(phase)
      << "\",\"op_index\":" << op_index
...
void require_snapshot_certificate_diagnostic_contract(const TraceOp& event,
  ...
  // This is a host replay over frozen native-tree bytes, not direct admission
  // and not a native query-traversal membership receipt.  Historical trace
  // labels remain source-mirror annotations only.  With the runtime guard
  // closed, every insert remains in global delta even with a unique route.
  ...
  require(placement.certificate.ok &&
              placement.certificate.matching_children == 1 &&
              placement.certificate.every_predicate_branch_mirrored &&
              placement.certificate.all_include_branch_not_relied_on,
          "frozen native sibling-min diagnostic must have one bound predicate path");
  ...
  std::array<int, 3> expected_children{};
  std::array<int, 3> expected_parents{};
  std::array<bool, 3> expected_last{};
  switch (event.op_index) {
    case 0:
    case 1:
    case 4:
      expected_children = {1, 13, 131};
      expected_parents = {0, 1, 13};
      expected_last = {false, false, false};
      break;
    case 13:
      expected_children = {10, 108, 1085};
      expected_parents = {0, 10, 108};
      expected_last = {true, false, false};
      break;
    case 16:
      expected_children = {7, 78, 790};
      expected_parents = {0, 7, 78};
      expected_last = {false, false, true};
      break;
    case 17:
      expected_children = {5, 60, 604};
      expected_parents = {0, 5, 60};
      expected_last = {false, true, false};
      break;
    default:
      runner_fail("unexpected insertion op_index in fixed frozen-native certificate contract");
  }

  require(placement.certificate.levels.size() == expected_children.size(),
          "frozen native certificate has wrong branch-witness depth");
  for (std::size_t index = 0; index < expected_children.size(); ++index) {
    const auto& level = placement.certificate.levels[index];
    require(level.parent_leaf_or_internal == expected_parents[index] &&
                level.child == expected_children[index] &&
                level.last_child == expected_last[index],
            "frozen native certificate has wrong sibling-min branch path");
    if (expected_last[index]) {
      require(level.next_sibling == -1 &&
                  level.predicate_branch ==
                      safe_c1_g3::GtsKnnPruningBranch::kPredicateLastSibling,
              "last-child certificate witness drifted from the native predicate");
    } else {
      require(level.next_sibling == expected_children[index] + 1 &&
                  level.predicate_branch ==
                      safe_c1_g3::GtsKnnPruningBranch::kPredicateNonLastSibling,
              "non-last certificate witness drifted from the native predicate");
    }
  }
}
...
void require_json_publication_self_test_or_fail() {
...
void fsync_regular_file_or_fail(const fs::path& path, const std::string& label) {
  const int fd = ::open(path.c_str(), O_RDONLY | O_CLOEXEC | O_NOFOLLOW);
  if (fd < 0) {
    runner_fail(label + " cannot be opened for fsync: " + std::strerror(errno));
  }
  struct stat info {};
  const int stat_result = ::fstat(fd, &info);
  const int sync_result = stat_result == 0 ? ::fsync(fd) : -1;
  const int close_result = ::close(fd);
  require(stat_result == 0 && S_ISREG(info.st_mode) && sync_result == 0 && close_result == 0,
          label + " regular-file fsync/close failed");
}

void require_valid_jsonl_or_fail(const std::string& text, const std::string& label,
                                 std::size_t expected_records) {
  const std::vector<std::string> lines = nonempty_lines(text, label);
  require(lines.size() == expected_records,
          label + " record count differs from the fixed execution contract");
  for (std::size_t index = 0; index < lines.size(); ++index) {
    require_valid_json_document_or_fail(
        lines[index], label + " record " + std::to_string(index));
  }
}
...
void write_text_file(const fs::path& path, const std::string& text) {
  ...
  output.close();
  require(static_cast<bool>(output), "cannot close output: " + path.string());
  fsync_regular_file_or_fail(path, "temporary output");
}

std::string readback_text_or_fail(const fs::path& path, const std::string& label) {
  return text_from_bytes(read_regular_file_strict(path), label);
}

void require_json_document_readback_or_fail(const fs::path& path, const std::string& expected,
                                            const std::string& label) {
  const std::string persisted = readback_text_or_fail(path, label);
  require(persisted == expected, label + " bytes changed after write");
  require_valid_json_document_or_fail(persisted, label + " after write");
}

void require_jsonl_readback_or_fail(const fs::path& path, const std::string& expected,
                                    const std::string& label, std::size_t expected_records) {
  const std::string persisted = readback_text_or_fail(path, label);
  require(persisted == expected, label + " bytes changed after write");
  require_valid_jsonl_or_fail(persisted, label + " after write", expected_records);
}
...
RunPaths create_staging_directory_or_fail(const fs::path& final_dir) {
  ...
  require(final_dir.parent_path() == runs_root,
          "run directory must be one new direct child of the fixed v20 runs directory");
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    g3_4k_pilot::require_json_publication_self_test_or_fail();
    ...
    g3_4k_pilot::require(!safe_c1_g3::G3_NATIVE_RANGE_RECEIPT_IMPLEMENTED &&
               ...
               "controlled range scope drifted from full immutable-base safe fallback");
    ...
    // This runner-owned set tracks exactly the immutable generation currently
    // resident in Matrix.  It is intentionally distinct from active, which
    // gains/losses global-delta rows between explicit rebuilds.
    std::set<StableId> frozen_base = active;
    ...
    g3_4k_pilot::require_range_safe_fallback_contract(
        bootstrap_range.export_data, fixture_data, frozen_base, active, /*query_id=*/2,
        g3_4k_pilot::kBootstrapRangeRadius, "bootstrap");
    ...
    bootstrap_attestation
        << "{\"record\":\"bootstrap_attestation\",\"schema\":\"safe-c1-g3-bootstrap-attestation-v3\","
        << "\"engine_trace_excludes_bootstrap\":true,"
        << "\"range_scope\":\"exact_full_immutable_base_plus_global_delta_safe_fallback_no_gts_range_receipt_no_range_performance_claim\","
        << "\"range_execution_mode\":\"exact_full_immutable_base_range_fallback_v1\","
        << "\"range_fallback_used\":true,\"range_leaf_receipt_available\":false,"
        << "\"range_performance_claim\":false,"
        << "\"direct_runtime_guard_open\":false"
        ...
        << ",\"range_base_candidate_count\":"
        << bootstrap_range.export_data.full_immutable_base_candidate_count
        << ",\"range_base_stable_ids_sha256\":\""
        << bootstrap_range.export_data.immutable_base_stable_ids_sha256 << "\""
        << ",\"range_base_local_to_stable_sha256\":\""
        << bootstrap_range.export_data.local_to_stable_sha256_value << "\""
        << ",\"range_engine_result_sha256\":\""
        << g3_4k_pilot::result_sha_for_attestation(bootstrap_range.export_data.results) << "\""
        << ",\"range_independent_result_sha256\":\""
        << g3_4k_pilot::result_sha_for_attestation(bootstrap_range_independent) << "\""
        << ",\"range_engine_receipt_sha256\":\""
        << bootstrap_range.export_data.post_rebuild_receipt_sha256 << "\""
        << ",\"range_engine_binding_sha256\":\""
        << bootstrap_range.export_data.post_rebuild_binding_sha256 << "\""
        << "}";
      ...
      if (event.op == "insert") {
        ...
        if (event.op == "knn") {
          ...
          g3_4k_pilot::compare_trace_query_to_independent_oracle(
              event, issued.export_data, fixture_data, frozen_base, active, persisted_oracle);
    ...
    const std::string engine_text = g3_4k_pilot::readback_text_or_fail(
        engine_tmp, "temporary engine trace before publication");
    g3_4k_pilot::require_valid_jsonl_or_fail(
        engine_text, "temporary engine trace before publication", trace.size());
    g3_4k_pilot::fsync_regular_file_or_fail(engine_tmp, "temporary engine trace");
    ...
    const std::string bootstrap_text = bootstrap_attestation.str();
    const std::string tickets_text = tickets.str();
    g3_4k_pilot::require_valid_json_document_or_fail(
        bootstrap_text, "bootstrap attestation before publication");
    g3_4k_pilot::require_valid_jsonl_or_fail(
        tickets_text, "ticket attestation before publication", ticket_records.size());
    g3_4k_pilot::write_text_file(bootstrap_tmp, bootstrap_text);
    g3_4k_pilot::write_text_file(tickets_tmp, tickets_text);
    g3_4k_pilot::require_json_document_readback_or_fail(
        bootstrap_tmp, bootstrap_text, "bootstrap attestation");
    g3_4k_pilot::require_jsonl_readback_or_fail(
        tickets_tmp, tickets_text, "ticket attestation", ticket_records.size());

    const std::filesystem::path engine_final = run_paths.staging_dir / "engine.jsonl";
    const std::filesystem::path bootstrap_final =
        run_paths.staging_dir / "bootstrap_attestation.json";
    const std::filesystem::path tickets_final =
        run_paths.staging_dir / "ticket_attestation.jsonl";
    g3_4k_pilot::rename_noreplace_or_fail(
        engine_tmp, engine_final, "engine trace publication");
    g3_4k_pilot::rename_noreplace_or_fail(
        bootstrap_tmp, bootstrap_final, "bootstrap attestation publication");
    g3_4k_pilot::rename_noreplace_or_fail(
        tickets_tmp, tickets_final, "ticket attestation publication");
    g3_4k_pilot::fsync_directory_or_fail(
        run_paths.staging_dir, "full-trace staging directory");
    g3_4k_pilot::rename_noreplace_or_fail(
        run_paths.staging_dir, run_paths.final_dir, "full-trace final directory publication");
    g3_4k_pilot::fsync_directory_or_fail(
        run_paths.final_dir.parent_path(), "full-trace runs directory");
    std::cout << "V24_SAFE_C1_FULLTRACE_GATE_OK\n";
    return 0;
