...
constexpr char kReleaseRoot[] =
    ...
    "safe_c1_native_matrix_g3_v6_guarded_token_pilot";
...
LaunchAdmission validate_launch_admission_or_fail(
  ...
  constexpr const char* required[] = {
      ...
      "run_body_path", "run_body_sha256"};
  require(fields.size() == std::size(required),
          "launch admission has unrecognized/missing keys");
  for (const char* key : required) (void)admission_value(fields, key);
  require(admission_value(fields, "schema") == "safe-c1-g3-single-tu-admission-v3" &&
...
GpuLaunchAuthorization consume_gpu_launch_authorization_or_fail(
  ...
  require(admission_value(token_fields, "schema") == "safe-c1-g3-gpu-launch-token-v1" &&
  ...
  constexpr const char* proof_required[] = {
      ...
      "run_name", "run_dir", "prelaunch_idle_check_sha256", "scope"};
  ...
  require(admission_value(proof_fields, "schema") == "safe-c1-g3-prelaunch-proof-v1" &&
