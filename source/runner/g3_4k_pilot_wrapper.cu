...
// Controlled, single-translation-unit Safe-C1 G3 4K frozen-snapshot
// certificate diagnostic.  It is not a correctness or performance pilot.
//
// This wrapper validates fixture/bootstrap/source-closure bytes, builds exactly
// one immutable native generation, and records only host replay facts for the
// frozen certificate predicate.  It performs no mutable insert/delete/rebuild,
// no KNN/range query, no native traversal-membership claim, and no performance
// measurement.  Direct sidecars remain compile-time disabled throughout.
//
...
//
// Range scope is deliberately narrow: after the branch-aligned vector
// predicate mirror failed its bootstrap oracle, the Matrix range path scans the
// current frozen immutable base exactly and then the complete global delta.
// This is a correctness-only fallback, not native GTS range traversal,
// archive-RNN equivalence, or a performance claim.  Direct sidecars are
// statically disabled; every mutable insertion in this pilot must return delta
// placement.
...
using safe_c1_g3::FrozenCertificateDiagnostic;
...
constexpr char kReleaseRoot[] =
    ...
    "safe_c1_native_matrix_g3_v18_json_receiptfix_bootstrapready";
...
constexpr ExpectedFile kSourceClosure[] = {
    {"src/g3_safe_c1_native_matrix.cu",
     "b2a334711339c7dbd6d23e15d0d9c6ade6572da6aa31932846c36ab48e11d74a", 0},
...
constexpr ExpectedFile kBootstrapFiles[] = {
    ...
    {"bootstrap_oracle_validation.json",
     "8a1ecc2328184ab0d8d4be5eabe2c2c1ed28ce574bba10994b396f5701c0ec40", 0},
};

[[noreturn]] void runner_fail(const std::string& reason) {
  throw std::runtime_error("G3 frozen-snapshot certificate diagnostic refused: " + reason);
}
...
LaunchArguments parse_launch_arguments_or_fail(int argc, char** argv) {
  require(argc == 5 && std::string_view(argv[1]) == "--run-dir" &&
          ...
          "usage is exactly: g3_4k_pilot_wrapper --run-dir <absolute-v11-runs-child> --admission <absolute-v11-admission>");
...
LaunchAdmission validate_launch_admission_or_fail(
  ...
  require(canonical_admission_root == admission_root &&
          ...
          "admission root must be fixed real v11 preflight/admissions directory");
  ...
  require(admission.parent_path() == admission_root && admission.extension() == ".admission",
          "admission must be a fixed real child of v11 preflight/admissions");
  ...
  require(fs::weakly_canonical(binary_root) == binary_root &&
          ...
          "executing binary does not equal admission-bound v11 preflight/bin path");
...
GpuLaunchAuthorization consume_gpu_launch_authorization_or_fail(
  ...
  require(admission_value(token_fields, "schema") == "safe-c1-g3-gpu-launch-token-v2" &&
              ...
              admission_value(token_fields, "purpose") ==
                  "safe_c1_g3_frozen_snapshot_certificate_diagnostic" &&
              sha256_bytes(read_regular_file_strict(run_launcher)) ==
                  admission.run_launcher_sha256 &&
...
std::vector<TraceOp> parse_and_validate_fixed_trace(const std::vector<std::uint8_t>& trace_bytes) {
  ...
  for (std::size_t i = 0; i < lines.size(); ++i) {
    ...
    if (op == "insert" || op == "delete") {
      ...
      // v11 runtime policy.  v11 validates it for input provenance only and
...
std::string ticket_attestation_json(const std::string& phase, int op_index,
  ...
  out << "{\"record\":\"ticket_attestation\",\"schema\":\"safe-c1-g3-ticket-attestation-v2\",\"phase\":\"" << json_escape(phase)
      << "\",\"op_index\":" << op_index
  ...
  if (query.kind == "range") {
    out << ",\"range_execution_mode\":\"exact_full_immutable_base_range_fallback_v1\""
        << ",\"range_fallback_used\":"
        << (query.exact_full_immutable_base_range_fallback ? "true" : "false")
        << ",\"range_base_candidate_count\":"
        << query.full_immutable_base_candidate_count
        << ",\"range_leaf_receipt_available\":false"
        << ",\"range_performance_claim\":false";
  }
  out << "}";
  return out.str();
}
...
class StrictJsonDocumentParser final {
 public:
  StrictJsonDocumentParser(std::string_view text, const std::string& label)
      : text_(text), label_(label) {}

  void parse_document_or_fail() {
    skip_whitespace();
    parse_value();
    skip_whitespace();
    if (position_ != text_.size()) fail("trailing non-whitespace byte");
  }

 private:
  [[noreturn]] void fail(const char* reason) const {
    runner_fail(label_ + " invalid JSON at byte " + std::to_string(position_) + ": " + reason);
  }
  ...
  static bool nonzero_digit(char value) { return value >= '1' && value <= '9'; }
  ...
  void expect(char expected, const char* reason) {
    if (!consume(expected)) fail(reason);
  }

  void enter_container() {
    require(depth_ < 64, label_ + " JSON nesting exceeds fixed safety bound");
    ++depth_;
  }

  void exit_container() {
    require(depth_ > 0, label_ + " JSON container depth underflow");
    --depth_;
  }
  ...
  void parse_value() {
    ...
    switch (peek()) {
      ...
      default:
        ...
        fail("unexpected value prefix");
    }
  }

  void parse_object() {
    expect('{', "expected object opener");
    enter_container();
    skip_whitespace();
    if (consume('}')) {
      exit_container();
    ...
    while (true) {
      skip_whitespace();
      if (at_end() || peek() != '"') fail("object key must be a string");
      parse_string();
      skip_whitespace();
      expect(':', "object key is missing colon");
      skip_whitespace();
      parse_value();
      skip_whitespace();
      if (consume('}')) {
      ...
      expect(',', "object members require a comma");
      skip_whitespace();
    }
  }

  void parse_array() {
    expect('[', "expected array opener");
    enter_container();
    skip_whitespace();
    if (consume(']')) {
      exit_container();
    ...
    while (true) {
      ...
      expect(',', "array elements require a comma");
      skip_whitespace();
    }
  }

  void parse_string() {
    expect('"', "expected string opener");
    while (!at_end()) {
      const unsigned char value = static_cast<unsigned char>(text_[position_++]);
      if (value == '"') return;
      if (value < 0x20U) fail("unescaped control byte in string");
      if (value != '\\') continue;
      ...
      switch (escape) {
        ...
        case 'u':
          for (int index = 0; index < 4; ++index) {
            if (at_end() || !hex_digit(text_[position_++])) {
  ...
  void parse_number() {
    ...
    if (consume('0')) {
      if (!at_end() && digit(peek())) fail("leading zero in number");
    } else {
      if (at_end() || !nonzero_digit(peek())) fail("invalid number integer component");
      do {
        ++position_;
      } while (!at_end() && digit(peek()));
    }
    if (consume('.')) {
      if (at_end() || !digit(peek())) fail("fraction lacks a digit");
      do {
        ++position_;
      } while (!at_end() && digit(peek()));
    }
    if (!at_end() && (peek() == 'e' || peek() == 'E')) {
      ...
      if (at_end() || !digit(peek())) fail("exponent lacks a digit");
      do {
        ++position_;
      } while (!at_end() && digit(peek()));
    }
...
void require_valid_json_document_or_fail(std::string_view text, const std::string& label) {
  StrictJsonDocumentParser parser(text, label);
  parser.parse_document_or_fail();
}

void require_receipt_json_self_test_or_fail() {
  constexpr std::string_view kValid =
      R"({"schema":"safe-c1-json-self-test-v1","candidate":[{"id":1,"value":-12.5e+2,"escaped":"A\u0041\n"}],"ok":true,"none":null})";
  constexpr std::string_view kMissingCandidatesArrayClose =
      R"({"candidates":[{"stable_id":1}})";
  require_valid_json_document_or_fail(kValid, "receipt JSON self-test valid fixture");
  bool rejected = false;
  try {
    require_valid_json_document_or_fail(kMissingCandidatesArrayClose,
                                        "receipt JSON self-test missing-array fixture");
  } catch (const std::exception&) {
    rejected = true;
  }
  require(rejected,
          "receipt JSON parser accepted a missing candidates array terminator");
}
...
RunPaths create_staging_directory_or_fail(const fs::path& final_dir) {
  ...
  require(final_dir.parent_path() == runs_root,
          "run directory must be one new direct child of the fixed v11 runs directory");
...
void require_range_safe_fallback_contract(
    const QueryExport& actual, const FixtureData& fixture,
    const std::set<StableId>& frozen_base, const std::set<StableId>& active,
    int query_id, DistanceSq radius_sq, const std::string& phase) {
  const std::vector<StableId> frozen_ids(frozen_base.begin(), frozen_base.end());
  std::vector<StableId> expected_delta;
  require(std::includes(active.begin(), active.end(), frozen_base.begin(), frozen_base.end()),
          phase + " active set lost a frozen-base ID without rebuild");
  std::set_difference(active.begin(), active.end(), frozen_base.begin(), frozen_base.end(),
                      std::back_inserter(expected_delta));
  const std::set<StableId> delta(expected_delta.begin(), expected_delta.end());

  require(actual.exact_full_immutable_base_range_fallback &&
              actual.full_immutable_base_candidate_count == static_cast<int>(frozen_base.size()) &&
              actual.production_full_live_scan,
          phase + " range does not identify the full immutable-base correctness fallback");
  require(actual.base_path == "exact_full_immutable_base_range_fallback_no_gts_receipt" &&
              !actual.receipt_before_leaf_materialization &&
              !actual.range_receipt_before_leaf_materialization &&
              !actual.range_predicate_mirror_branch_aligned &&
              actual.raw_receipt_leaf_pair_count == 0 &&
              actual.unique_receipt_leaf_count == 0 &&
              actual.gts_visited_leaf_ids.empty() && actual.receipt_leaf_spans.empty() &&
              actual.base_receipt_rows.empty() && actual.native_final_res_ids.empty() &&
              !actual.native_final_res_ids_cross_checked,
          phase + " range fallback exported a false GTS/leaf/native-ID receipt");
  require(!actual.direct_sidecars_exact_range_filtered &&
              actual.sidecar_source_leaf_pairs.empty() &&
              actual.sidecar_candidate_ids.empty() && actual.sidecar_results.empty(),
          phase + " range fallback has direct-sidecar evidence despite closed guard");
  require(actual.id_list_capacity > 0 &&
              actual.immutable_base_stable_ids_sha256 == local_stable_set_sha(frozen_ids),
          phase + " range fallback frozen-base identity/count mismatch");
  require(actual.delta_candidate_ids == expected_delta,
          phase + " range fallback did not scan exactly the global delta");

  const std::vector<StableDistance> base_expected =
      exact_oracle(fixture, frozen_base, query_id, /*is_knn=*/false, 0, radius_sq);
  // exact_oracle intentionally rejects an empty *active* set.  Empty global
  // delta is nevertheless the normal bootstrap/post-rebuild partition, whose
  // exact expected answer is the canonical empty vector.
  std::vector<StableDistance> delta_expected;
  if (!delta.empty()) {
    delta_expected = exact_oracle(fixture, delta, query_id, /*is_knn=*/false, 0, radius_sq);
  }
  require_equal_results(actual.base_results, base_expected,
                        phase + " range fallback full immutable-base exact scan");
  require_equal_results(actual.delta_results, delta_expected,
                        phase + " range fallback global-delta exact scan");
}

void compare_trace_query_to_independent_oracle(
    const TraceOp& event, const QueryExport& actual, const FixtureData& fixture,
    const std::set<StableId>& frozen_base, const std::set<StableId>& active,
    const std::map<int, OracleRecord>& persisted_oracle) {
  ...
  if (is_knn) {
    ...
    require_range_safe_fallback_contract(actual, fixture, frozen_base, active,
                                         event.query_id, event.radius_sq, "trace");
  }
}
...
struct DiagnosticCandidate {
  StableId stable_id = -1;
  int trace_op_index = -1;
  const char* source_mirror_expected_role = "";
  const char* source_mirror_certificate_annotation = "";
};

void json_float_or_null(std::ostringstream* out, float value) {
  require(out != nullptr, "null JSON float sink");
...
std::string frozen_certificate_candidate_json(
    const DiagnosticCandidate& candidate, const FrozenCertificateDiagnostic& diagnostic) {
  const safe_c1_g3::StrictCertificate& certificate = diagnostic.certificate;
  std::ostringstream out;
  out << "{\"stable_id\":" << candidate.stable_id
      << ",\"fixture_trace_op_index\":" << candidate.trace_op_index
      << ",\"fixture_source_mirror_expected_role\":\""
      << json_escape(candidate.source_mirror_expected_role) << "\""
      << ",\"fixture_source_mirror_certificate_annotation\":\""
      << json_escape(candidate.source_mirror_certificate_annotation) << "\""
      << ",\"fixture_source_mirror_annotation_only\":true"
      << ",\"certificate_scope\":\"frozen_snapshot_host_replay_no_native_query_membership\""
      << ",\"certificate_ok\":" << (certificate.ok ? "true" : "false")
      << ",\"failure_reason\":\"" << json_escape(certificate.failure_reason) << "\""
      << ",\"failure_level\":" << certificate.failure_level
      << ",\"matching_children\":" << certificate.matching_children
      << ",\"sidecar_leaf_id\":" << certificate.sidecar_leaf_id
      << ",\"knn_visibility_only\":" << (certificate.knn_visibility_only ? "true" : "false")
      << ",\"range_correctness_not_claimed\":"
      << (certificate.range_correctness_not_claimed ? "true" : "false")
      << ",\"all_include_branch_not_relied_on\":"
      << (certificate.all_include_branch_not_relied_on ? "true" : "false")
      << ",\"every_predicate_branch_mirrored\":"
      << (certificate.every_predicate_branch_mirrored ? "true" : "false")
      << ",\"levels\":[";
  for (std::size_t i = 0; i < diagnostic.levels.size(); ++i) {
    if (i) out << ',';
    const safe_c1_g3::CertificateDiagnosticLevel& level = diagnostic.levels[i];
    out << "{\"level\":" << level.level
        << ",\"parent\":" << level.parent
        << ",\"parent_valid\":" << (level.parent_valid ? "true" : "false")
        << ",\"parent_pivot_local_row\":" << level.parent_pivot_local_row
        << ",\"parent_pivot_stable_id\":" << level.parent_pivot_stable_id
        << ",\"pivot_distance\":";
    json_float_or_null(&out, level.pivot_distance);
    out << ",\"diagnostic_strict_match_count\":" << level.diagnostic_strict_match_count
        << ",\"siblings\":[";
    for (std::size_t j = 0; j < level.siblings.size(); ++j) {
      if (j) out << ',';
      const safe_c1_g3::CertificateSiblingDiagnostic& sibling = level.siblings[j];
      out << "{\"child\":" << sibling.child
          << ",\"next_sibling\":" << sibling.next_sibling
          << ",\"child_pivot_local_row\":" << sibling.child_pivot_local_row
          << ",\"child_pivot_stable_id\":" << sibling.child_pivot_stable_id
          << ",\"child_empty\":" << (sibling.child_empty ? "true" : "false")
          << ",\"child_pivot_matches_parent\":"
          << (sibling.child_pivot_matches_parent ? "true" : "false")
          << ",\"child_min_dis_finite\":"
          << (sibling.child_min_dis_finite ? "true" : "false")
          << ",\"child_max_distance_finite\":"
          << (sibling.child_max_distance_finite ? "true" : "false")
          << ",\"next_sibling_min_minus_child_max_distance_finite\":"
          << (sibling.next_sibling_min_minus_child_max_distance_finite ? "true" : "false")
          << ",\"last_child\":" << (sibling.last_child ? "true" : "false")
          << ",\"next_sibling_available\":"
          << (sibling.next_sibling_available ? "true" : "false")
          << ",\"next_sibling_min_dis_finite\":"
          << (sibling.next_sibling_min_dis_finite ? "true" : "false")
          << ",\"upper_is_unbounded_last_child\":"
          << (sibling.upper_is_unbounded_last_child ? "true" : "false")
          << ",\"strict_native_predicate_match\":"
          << (sibling.strict_native_predicate_match ? "true" : "false")
          << ",\"lower\":";
      json_float_or_null(&out, sibling.lower);
      out << ",\"upper\":";
      json_float_or_null(&out, sibling.upper);
      out << ",\"child_max_distance\":";
      json_float_or_null(&out, sibling.child_max_distance);
      out << ",\"next_sibling_min_minus_child_max_distance\":";
      json_float_or_null(&out, sibling.next_sibling_min_minus_child_max_distance);
      out << ",\"pivot_distance\":";
      json_float_or_null(&out, sibling.pivot_distance);
      out << '}';
    }
    out << "]}";
  }
...
int main(int argc, char** argv) {
  using g3_4k_pilot::FrozenCertificateDiagnostic;
  ...
  try {
    ...
    if (argc == 2 && std::string_view(argv[1]) == "--self-test-receipt-json") {
      g3_4k_pilot::require_clean_loader_environment_or_fail();
      g3_4k_pilot::require_receipt_json_self_test_or_fail();
      std::cout << "V18_RECEIPT_JSON_SELF_TEST_OK\n";
      return 0;
    }
    // Every control-plane binding remains before Matrix/CUDA entry.
    ...
    const std::filesystem::path root = std::filesystem::weakly_canonical(
        std::filesystem::path(g3_4k_pilot::kReleaseRoot));
    ...
    const g3_4k_pilot::PinnedFileSet source_audit_bytes =
        g3_4k_pilot::bind_expected_files_once(root, g3_4k_pilot::kSourceClosure,
                                               std::size(g3_4k_pilot::kSourceClosure),
                                               "source closure");
    ...
    g3_4k_pilot::require(!safe_c1_g3::G3_DIRECT_SIDECAR_RUNTIME_GUARD_OPEN,
                          "diagnostic refuses an open direct-sidecar runtime guard");
    g3_4k_pilot::require(!safe_c1_g3::G3_NATIVE_RANGE_RECEIPT_IMPLEMENTED &&
                              safe_c1_g3::G3_RANGE_SAFE_FULL_IMMUTABLE_BASE_FALLBACK_IMPLEMENTED,
                          "diagnostic source scope drifted from the guard-closed range safety baseline");
    ...
    const std::filesystem::path receipt_tmp =
        run_paths.staging_dir / ".snapshot_certificate_diagnostic.json.tmp";
    ...
    g3_4k_pilot::require(static_cast<int>(active.size()) == g3_4k_pilot::kBaseCount,
                          "initial immutable active-set cardinality drift");
    g3_4k_pilot::require(g3_4k_pilot::local_stable_set_sha(
                              std::vector<StableId>(active.begin(), active.end())) ==
                              g3_4k_pilot::kInitialLiveSha,
                          "initial immutable active-set digest drift");

    // The preceding calls have only read pinned bytes and staged an unpublished
    // output.  This is the sole Matrix/CUDA entry boundary.
    ...
    const g3_4k_pilot::GpuLaunchAuthorization gpu_authorization =
        g3_4k_pilot::consume_gpu_launch_authorization_or_fail(root, launch_args,
                                                                launch_admission);
    NativeSafeC1Matrix matrix(/*sidecar_leaf_capacity=*/2,
                              /*requested_k=*/g3_4k_pilot::kK);
    ...
    g3_4k_pilot::require_topology_receipt(initial_build, "initial immutable diagnostic build",
                                          active, g3_4k_pilot::kInitialLiveSha);

    constexpr std::array<g3_4k_pilot::DiagnosticCandidate, 6> kCandidates = {{
        {4876, 0, "direct_same_leaf", "certified"},
        {4898, 1, "direct_same_leaf", "certified"},
        {5303, 4, "capacity_delta", "certified"},
        {4097, 13, "certificate_delta", "gap_or_boundary"},
        {4099, 16, "certificate_delta", "gap_or_boundary"},
        {4102, 17, "certificate_delta", "gap_or_boundary"},
    }};
    std::array<FrozenCertificateDiagnostic, kCandidates.size()> diagnostics;
    for (std::size_t i = 0; i < kCandidates.size(); ++i) {
      const auto trace_it = std::find_if(
          trace.begin(), trace.end(), [&candidate = kCandidates[i]](const g3_4k_pilot::TraceOp& event) {
            return event.op == "insert" && event.stable_id == candidate.stable_id;
          });
      g3_4k_pilot::require(trace_it != trace.end() &&
                                trace_it->op_index == kCandidates[i].trace_op_index &&
                                trace_it->fixture_role == kCandidates[i].source_mirror_expected_role,
                            "diagnostic candidate does not match its pinned trace annotation");
      diagnostics[i] = matrix.inspect_initial_frozen_certificate(kCandidates[i].stable_id);
      g3_4k_pilot::require(diagnostics[i].tree_version == initial_build.tree_version &&
                                diagnostics[i].state_epoch == 1 &&
                                diagnostics[i].sidecar_live == 0 && diagnostics[i].delta_live == 0 &&
                                diagnostics[i].raw_predicate_cross_checked,
                            "frozen diagnostic state/predicate binding drifted");
    }

    std::ostringstream receipt;
    receipt << "{\"schema\":\"safe-c1-g3-native-snapshot-certificate-diagnostic-v2\""
            << ",\"mode\":\"initial_immutable_tree_only\""
            << ",\"diagnostic_only\":true"
            << ",\"direct_runtime_guard_open\":false"
            << ",\"direct_insertion_enabled\":false"
            << ",\"mutation_count\":0,\"knn_queries_executed\":0"
            << ",\"range_queries_executed\":0,\"rebuilds_after_initial\":0"
            << ",\"certificate_scope\":\"frozen_snapshot_host_replay_no_native_query_membership\""
            << ",\"certificate_is_not_direct_admission\":true"
            << ",\"native_traversal_membership_checked\":false"
            << ",\"range_correctness_not_claimed\":true,\"performance_claim\":false"
            << ",\"tree_version\":" << initial_build.tree_version
            << ",\"state_epoch\":" << diagnostics.front().state_epoch
            << ",\"tree_height\":" << initial_build.tree_height
            << ",\"fanout\":" << initial_build.fanout
            << ",\"node_capacity\":" << initial_build.node_capacity
            << ",\"nonempty_node_count\":" << initial_build.nonempty_node_count
            << ",\"base_count\":" << initial_build.base_count
            << ",\"base_dimension\":" << g3_4k_pilot::kDimension
            << ",\"metric\":\"L2\""
            << ",\"strict_epsilon\":" << safe_c1_g3::kStrictEpsilon
            << ",\"raw_predicate_cross_checked\":true"
            << ",\"sidecar_live_before\":0,\"sidecar_live_after\":0"
            << ",\"delta_live_before\":0,\"delta_live_after\":0"
            << ",\"fixture_annotation_binding\":\"pinned_trace_role_checked_metadata_and_selection_receipt_hash_bound\""
            << ",\"fixture_manifest_sha256\":\""
            << g3_4k_pilot::sha256_bytes(g3_4k_pilot::pinned_bytes(fixture_bytes, "manifest.json"))
            << "\""
            << ",\"fixture_selection_receipt_sha256\":\""
            << g3_4k_pilot::sha256_bytes(g3_4k_pilot::pinned_bytes(
                   fixture_bytes, "selection_receipt.json"))
            << "\""
            << ",\"fixture_trace_sha256\":\""
            << g3_4k_pilot::sha256_bytes(g3_4k_pilot::pinned_bytes(fixture_bytes, "trace.jsonl"))
            << "\""
            << ",\"fixture_metadata_sha256\":\""
            << g3_4k_pilot::sha256_bytes(g3_4k_pilot::pinned_bytes(fixture_bytes, "metadata.json"))
            << "\""
    ...
    for (std::size_t i = 0; i < kCandidates.size(); ++i) {
      if (i) receipt << ',';
      receipt << g3_4k_pilot::frozen_certificate_candidate_json(kCandidates[i], diagnostics[i]);
    }
    receipt << "]}";
    const std::string receipt_text = receipt.str();
    g3_4k_pilot::require_valid_json_document_or_fail(
        receipt_text, "snapshot diagnostic receipt before publication");
    g3_4k_pilot::write_text_file(receipt_tmp, receipt_text);
    const std::string persisted_receipt = g3_4k_pilot::text_from_bytes(
        g3_4k_pilot::read_regular_file_strict(receipt_tmp),
        "temporary snapshot diagnostic receipt");
    g3_4k_pilot::require(persisted_receipt == receipt_text,
                          "temporary snapshot diagnostic receipt bytes changed after write");
    g3_4k_pilot::require_valid_json_document_or_fail(
        persisted_receipt, "snapshot diagnostic receipt after write before publication");
    const std::filesystem::path published_receipt =
        run_paths.staging_dir / "snapshot_certificate_diagnostic.json";
    g3_4k_pilot::rename_noreplace_or_fail(
        receipt_tmp, published_receipt, "snapshot diagnostic receipt publication");
    g3_4k_pilot::fsync_directory_or_fail(
        run_paths.staging_dir, "snapshot diagnostic staging directory");
    g3_4k_pilot::rename_noreplace_or_fail(
        run_paths.staging_dir, run_paths.final_dir, "snapshot diagnostic final directory publication");
    g3_4k_pilot::fsync_directory_or_fail(
        run_paths.final_dir.parent_path(), "snapshot diagnostic runs directory");
    std::cout << "V18_FROZEN_SNAPSHOT_CERTIFICATE_DIAGNOSTIC_OK\n";
    return 0;
