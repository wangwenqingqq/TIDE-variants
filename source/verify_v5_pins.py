#!/usr/bin/env python3
"""Fail-closed CPU-only integrity and source-policy verifier for C1 v5."""
...
HELPERS={"guard","measured_engine","profile_engine","profile_guard","profile_manifest_utility","strict_semantic_verifier","measured_analyzer","profile_verifier","measured_finalizer","formal_aggregator","manifest_utility","pin_verifier","pin_generator","build_script","build_manifest_creator"}
...
def verify_profile_execution_plan(pins:dict)->None:
    ...
    if plan.get("schema")!="gtspp-c1-v5-profile-execution-plan-v1": raise SystemExit("profile execution plan schema")
...
def verify_trace_provenance(pins:dict,root:pathlib.Path)->None:
    """Fail before GPU launch if the v5-local trace provenance is inconsistent."""
...
def static_source_policy(root:pathlib.Path)->None:
    ...
    for token in (
        ...
        if token not in c1: raise SystemExit(f"missing C1 v5 policy token {token!r}")
    ...
    measured_guard=(root/"run_c1_workspace_microbenchmark_guard_v5.sh").read_text()
    measured_verifier=(root/"verify_c1_semantics_v5.py").read_text()
    profile_guard=(root/"run_c1_profile_guard_v5.sh").read_text()
    profile_manifest=(root/"c1_v5_profile_manifest.py").read_text()
    profile_verifier=(root/"verify_c1_profile_artifacts_v5.py").read_text()
    finalizer=(root/"finalize_c1_evidence_v5.py").read_text()
    aggregator=(root/"aggregate_c1_formal_evidence_v5.py").read_text()
    ...
    for token in ("C1_V5_TRUST_ROOT=''","C1_V5_LAUNCHER=''","C1_V5_TRUST_GUARD_SHA=''","C1_V5_TRUST_PINS_SHA=''","C1_V5_TRUST_PIN_VERIFY_SHA=''","C1_ALLOW_GPU0=''","C1_RUN_OUT=''"):
        if token in measured_guard: raise SystemExit(f"measured guard erases inherited trust environment {token!r}")
    for token in ("C1_V5_PROFILE_TRUST_ROOT=''","C1_V5_PROFILE_LAUNCHER=''","C1_V5_PROFILE_TRUST_GUARD_SHA=''","C1_V5_PROFILE_TRUST_PINS_SHA=''","C1_V5_PROFILE_TRUST_PIN_VERIFY_SHA=''","C1_ALLOW_GPU0=''","C1_PROFILE_OUT=''"):
        if token in profile_guard: raise SystemExit(f"profile guard erases inherited trust environment {token!r}")
    for token in ("visible_cuda_device_uuid","CUDA UUID must exactly match manifest/session UUID","telemetry PCI discontinuity"):
        if token not in measured_verifier: raise SystemExit(f"missing measured verifier binding token {token!r}")
    for token in ("--trace=cuda","--cuda-memory-usage=true","--profile-only","profile_nsys_stats_complete","um_sum","C1_V5_PROFILE_TRUST_GUARD_SHA","profile_execution_plan_v5.json","CUDA UUID/session"):
        if token not in profile_guard: raise SystemExit(f"missing profile guard policy token {token!r}")
    for token in ("profile_run_manifest_v5.json","nsys_argv","profile_artifacts","profile.sqlite","um_sum"):
        if token not in profile_manifest: raise SystemExit(f"missing profile manifest policy token {token!r}")
    for token in ("profile_run_manifest_v5.json","outer guarded nsys invocation manifest","profile_nsys_stats_complete","SQLite format 3","um_sum","CUDA UUID differs from manifest/session UUID","profile telemetry PCI differs from outer prelaunch"):
        if token not in profile_verifier: raise SystemExit(f"missing profile verifier policy token {token!r}")
    for token in ("gtspp-c1-v5-measured-evidence-v2","gpu_pci_bus_id_normalized"):
        if token not in finalizer: raise SystemExit(f"missing measured finalizer binding token {token!r}")
    for token in ("gtspp-c1-v5-profile-verification-v3","cross-root GPU PCI disagreement","gtspp-c1-v5-formal-evidence-aggregate-v3","aggregate self pin/provenance mismatch","aggregate_script_sha256","SELF_PATH"):
...
def main()->int:
    ...
    if p.get("schema")!="gtspp-c1-v5-pins-v1" or p.get("root")!=str(root): raise SystemExit("pins schema/root mismatch")
    ...
    if bm.get("schema")!="gtspp-c1-v5-build-manifest-v1": raise SystemExit("build manifest schema")
