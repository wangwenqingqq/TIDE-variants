#!/usr/bin/env bash
# Guarded C1 held-out gamma launcher: empirical static GTS safety gate.
#
...
#     "range_extra_ids": 0
#   }
# A nonzero counter or missing field is a failed C1 held-out gamma run, never a pass.
set -Eeuo pipefail
...
[[ -n "${C1_GAMMA_SCALE:-}" && -n "${C1_QUERY_BEGIN:-}" && -n "${C1_QUERY_END:-}" ]] || {
  echo "C1_GAMMA_SCALE, C1_QUERY_BEGIN, and C1_QUERY_END must be exported" >&2; exit 64;
}
[[ "$C1_GAMMA_SCALE" =~ ^[0-9]+([.][0-9]+)?$ && "$C1_QUERY_BEGIN" =~ ^[0-9]+$ && "$C1_QUERY_END" =~ ^[0-9]+$ ]] || {
  echo "invalid C1 held-out policy environment" >&2; exit 64;
}
(( C1_QUERY_END > C1_QUERY_BEGIN )) || { echo "C1 held-out query block is empty" >&2; exit 64; }
...
[[ "$HOST" == "$EXPECTED_HOST" ]] || {
  echo "This C1-HELDOUT-GAMMA template is locked to 8p host $EXPECTED_HOST, got $HOST" >&2; exit 77
}
...
SOURCE_MANIFEST="$EXP_ROOT/c1_heldout_gamma_probe/source_manifest.sha256"
...
def file_hash(path):
    ...
    return h.hexdigest()
obj = {
  "experiment_id": "C1-HELDOUT-GAMMA",
  "title": "C1 held-out gamma static correctness gate",
  "scope": "Empirical C1 static gamma gate on a declared held-out query block; not a universal no-false-negative or performance claim. Pass also preserves the independent dynamic smoke oracle.",
  "status": status,
  ...
  "c1_heldout_policy": {
      "gamma_scale": float(get("C1_GAMMA_SCALE")),
      "query_block_begin": int(get("C1_QUERY_BEGIN")),
      "query_block_end_exclusive": int(get("C1_QUERY_END")),
      "selection_contract": "caller chooses gamma only from a disjoint calibration block; this card is one declared holdout/test gate"
  },
...
fail() {
  ...
  echo "C1-HELDOUT-GAMMA blocked/failed at $STAGE: $MESSAGE" >&2
  exit 1
}
...
validate_c1_static_gate() {
  "$PYTHON" - "$RUNNER_OUT/engine_summary.json" "$C1_GAMMA_SCALE" "$C1_QUERY_BEGIN" "$C1_QUERY_END" <<'PY'
import json, math, sys
p, gamma, begin, end = sys.argv[1:]
with open(p, encoding="utf-8") as f: s = json.load(f)
if s.get("schema") != "c1-heldout-gamma-gate-v1" or s.get("status") != "PASS_C1_HELDOUT_STATIC_GATE":
    raise SystemExit("runner summary is not a passing C1 held-out gate")
policy = s.get("c1_gamma_policy", {})
if not math.isclose(float(policy.get("gamma_scale", float("nan"))), float(gamma), rel_tol=0.0, abs_tol=1e-7):
    raise SystemExit("runner gamma differs from declared card policy")
if policy.get("query_block_begin") != int(begin) or policy.get("query_block_end_exclusive") != int(end):
    raise SystemExit("runner query block differs from declared card policy")
probe = s.get("gts_static_vector_topk_probe", {})
for key in ("id_mismatch", "distance_mismatch", "ambiguous_boundary_ties"):
    if probe.get(key) != 0:
        raise SystemExit(f"static C1 gate has {key}={probe.get(key)!r}")
if probe.get("status") != "PASS" or probe.get("queries") != int(end) - int(begin):
    raise SystemExit("static C1 gate query count/status invalid")
PY
}
...
if ! validate_c1_static_gate > "$LOG_DIR/c1_static_gate_check.log" 2>&1; then
  fail "c1_static_gate" "runner summary did not prove the declared held-out static gamma gate; see logs/c1_static_gate_check.log"
fi

STAGE="complete"; MESSAGE="declared C1 held-out static gamma gate and independent exact-oracle dynamic smoke passed"
export E1GI_STAGE="$STAGE" E1GI_MESSAGE="$MESSAGE" E1GI_RUNNER_EXIT="$RUNNER_EXIT"
export E1GI_VALIDATOR_EXIT="$VALIDATOR_EXIT" E1GI_GPU_USED_JSON="$GPU_USED_JSON" E1GI_GPU0_UUID="$GPU0_UUID"
write_card "PASS_VALIDATED_C1_HELDOUT_GAMMA"
printf '%s\n' "$OUT_DIR"
