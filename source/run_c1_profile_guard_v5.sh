#!/usr/bin/env bash
# V5 profile outer guard. Only this process may launch nsys/C1Microbench for the
...
PLAN="$ROOT/run_c1_profile_primary_v5.sh"
PLAN_JSON="$ROOT/profile_execution_plan_v5.json"
PINS="$ROOT/hardened_static_pins_v5.json"
PIN_VERIFY="$ROOT/verify_v5_pins.py"
MANIFEST="$ROOT/c1_v5_profile_manifest.py"
VERIFIER="$ROOT/verify_c1_profile_artifacts_v5.py"
...
die() { printf 'C1-V5-PROFILE-GUARD BLOCKED: %s\n' "$*" >&2; exit 69; }
...
write_guard_card() {
  ...
  status="$status" reason="$reason" GPU_UUID="$SESSION_UUID" "$PYTHON" -I - "$PROFILE_OUT/profile_guard_card_v5.json" <<'PY'
...
old.update({"schema":"gtspp-c1-v5-profile-guard-card-v1","updated_utc":datetime.datetime.now(datetime.timezone.utc).isoformat(),
 ...
 "snapshots_dir":str(p.parent/"gpu_snapshots"),"manifest":str(p.parent/"profile_run_manifest_v5.json")})
tmp=p.with_name(p.name+'.tmp');tmp.write_text(json.dumps(old,indent=2,sort_keys=True)+'\n');tmp.replace(p)
PY
}
manifest_event() { "$PYTHON" -I "$MANIFEST" event --out "$PROFILE_OUT" --kind "$1" --label "$2" --variant "$3" --detail "$4"; }
manifest_final() { "$PYTHON" -I "$MANIFEST" final --out "$PROFILE_OUT" --status "$1" --reason "$2"; }
verify_source_only() {
  ...
  "$PYTHON" -I "$PIN_VERIFY" --root "$ROOT" --pins "$PINS" --static-source || return 1
  "$PYTHON" -I - "$PLAN_JSON" <<'PY' || return 1
...
need(plan.get("schema")=="gtspp-c1-v5-profile-execution-plan-v1","schema")
...
need(card.get('schema')=='gtspp-c1-microbench-run-card-v5' and done.get('schema')=='gtspp-c1-microbench-completion-v5','schema')
...
run_profile_variant() {
  ...
  "$PYTHON" -I "$MANIFEST" child --out "$PROFILE_OUT" --rep 1 --variant "$variant" --binary "$bin" --cmake-cache "$cache" --variant-out "$out" --base '/workspace/legacy_workspace/GTS/Datasets/sift1m/sift_base.txt' --trace "$ROOT/inputs/sift1m_10k_442_first1024_type2_query_only.txt" --uuid "$SESSION_UUID" || die 'profile child manifest binding'
...
cleanup() {
  ...
  if [[ "$SESSION_READY" == 1 ]];then
    ...
    if ! rm -f -- "$PROFILE_OUT/.c1_profile_guard_session_v5.json";then rc=69;FINAL_STATUS='SESSION_CLEANUP_INCOMPLETE';FINAL_REASON='profile session removal failed';fi
    ...
    if [[ "$rc" -eq 0 && "$FINAL_STATUS" == COMPLETE ]];then
      ...
      elif ! "$PYTHON" -I "$VERIFIER" --profile-root "$PROFILE_OUT" --pins "$PINS";then rc=69;FINAL_STATUS='PROFILE_VERIFICATION_FAILED';FINAL_REASON='strict profile verifier failed'
      fi
...
C1_V5_PROFILE_TRUST_ROOT="$(read_env C1_V5_PROFILE_TRUST_ROOT)";C1_V5_PROFILE_LAUNCHER="$(read_env C1_V5_PROFILE_LAUNCHER)";C1_V5_PROFILE_TRUST_GUARD_SHA="$(read_env C1_V5_PROFILE_TRUST_GUARD_SHA)";C1_V5_PROFILE_TRUST_PINS_SHA="$(read_env C1_V5_PROFILE_TRUST_PINS_SHA)";C1_V5_PROFILE_TRUST_PIN_VERIFY_SHA="$(read_env C1_V5_PROFILE_TRUST_PIN_VERIFY_SHA)";C1_ALLOW_GPU0="$(read_env C1_ALLOW_GPU0)";RUN_OUT="$(read_env C1_PROFILE_OUT)"
[[ "$0" == "$GUARD" && "$C1_V5_PROFILE_TRUST_ROOT" == YES && "$C1_V5_PROFILE_LAUNCHER" == "$LAUNCHER" ]] || die 'reviewed profile trust-root required'
...
[[ "$(sha "$GUARD")" == "$C1_V5_PROFILE_TRUST_GUARD_SHA" && "$(sha "$PINS")" == "$C1_V5_PROFILE_TRUST_PINS_SHA" && "$(sha "$PIN_VERIFY")" == "$C1_V5_PROFILE_TRUST_PIN_VERIFY_SHA" ]] || die 'profile trust-root bootstrap hash mismatch'
...
BASE="$ROOT/profiles";OUT="$("$REALPATH" -m -- "$RUN_OUT")" || die 'bad profile output';is_under "$OUT" "$BASE" && [[ "$(basename "$OUT")" == c1_v5_profile_* && ! -e "$OUT" && ! -L "$OUT" ]] || die 'unsafe profile output'
...
"$PYTHON" -I "$MANIFEST" init --out "$PROFILE_OUT" --root "$ROOT" --pins "$PINS" --launcher "$LAUNCHER" --guard "$GUARD" -- "$GUARD" || die 'profile manifest init'
...
"$PYTHON" -I "$MANIFEST" gpu --out "$PROFILE_OUT" --uuid "$SESSION_UUID" --snapshot "$snapshot" || die 'profile manifest GPU'
"$PYTHON" -I - "$PROFILE_OUT/.c1_profile_guard_session_v5.json" "$SESSION_UUID" <<'PY'
import json,pathlib,sys
p=pathlib.Path(sys.argv[1]);p.write_text(json.dumps({'schema':'gtspp-c1-v5-profile-session-v1','physical_gpu_index':0,'physical_gpu_uuid':sys.argv[2]})+'\n')
PY
chmod 0600 "$PROFILE_OUT/.c1_profile_guard_session_v5.json" || die 'profile session chmod'
SESSION_READY=1
