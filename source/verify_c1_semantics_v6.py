#!/usr/bin/env python3
"""Strict CPU-only differential-semantic gate for isolated C1 v6 measured runs."""
...
def check_one(root:pathlib.Path,rep:int,v:str,qids:list[int],inspector:pathlib.Path,expected_uuid:str,expected_pci:str|None,g:Gate)->dict[str,Any]:
    ...
    expected=GATES[v]
    g.need(card.get("schema")=="gtspp-c1-microbench-run-card-v6",f"{label}: run-card schema")
    g.need(done.get("schema")=="gtspp-c1-microbench-completion-v6",f"{label}: completion schema")
...
def main()->int:
    ...
    try:
        ...
        if root.parent!=RUNS_ROOT or not root.name.startswith("c1_v6_measured_"):
            raise ValueError("run root must be a direct c1_v6_measured_* child of the v6 runs root")
        raw_regular(str(root/"run_manifest_v6.json"),"run manifest")
        pins_path=raw_regular(str(a.pins),"pins")
        if pins_path!=PINS_PATH: raise ValueError("pins path must be canonical v6 hardened pin file")
        pins=read_json(pins_path,"pins")
    except Exception as exc: raise SystemExit(f"setup failed: {exc}")
    g.need(pins.get("schema")=="gtspp-c1-v6-pins-v1","pins schema")
    ...
    manifest=g.attempt("run manifest",lambda:read_json(root/"run_manifest_v6.json","run manifest")) or {}
    guard=g.attempt("guard card",lambda:read_json(root/"guard_run_card_v6.json","guard card")) or {}
    g.need(manifest.get("schema")=="gtspp-c1-v6-run-manifest-v2" and manifest.get("execution_mode")=="MEASURED","manifest measured schema/mode")
    ...
    g.need(guard.get("schema")=="gtspp-c1-v6-guard-card-v2" and guard.get("physical_gpu_index")==0 and guard.get("physical_gpu_uuid")==m_uuid,"guard/manifest GPU provenance")
    ...
    result={
        "schema":"gtspp-c1-v6-semantic-verification-v1","verified_utc":utc(),"run_root":str(root),"pins_sha256":pin_sha,
    ...
    out=root/"semantic_verification_v6.json"
