...
def cmd_make(args: argparse.Namespace) -> None:
    ...
    if not (protocol_id.startswith("hnsw-dev-") or protocol_id.startswith("hnsw-test-")):
        raise ValueError("protocol id must begin hnsw-dev- or hnsw-test-.")
...
def cmd_run(args: argparse.Namespace) -> None:
    ...
    if scope not in {"development_smoke_only", "development_calibration_only", "heldout_test_once"}:
        raise ValueError(f"unsupported evidence_scope: {scope!r}")
    ...
    execution_plan_info: dict[str, Any] | None = None
    if args.execution_plan is not None:
        plan_path = Path(args.execution_plan).resolve()
        plan_blob = plan_path.read_bytes()
        execution_plan = json.loads(plan_blob)
        require_equal(execution_plan.get("protocol_manifest_sha256"), manifest_digest, "execution plan protocol binding")
        require_equal(execution_plan.get("hnsw_config"), {"M": args.M, "ef_construction": args.ef_construction, "ef_search": args.ef_search, "index_seed": args.index_seed}, "execution plan config binding")
        execution_plan_info = {"path": str(plan_path), "sha256": hashlib.sha256(plan_blob).hexdigest()}
    ...
    summary = {
        ...
        "status": {"development_smoke_only": "PASS_DEVELOPMENT_SMOKE", "development_calibration_only": "PASS_DEVELOPMENT_CALIBRATION", "heldout_test_once": "PASS_HELDOUT_TEST_ONCE"}[scope] if failure is None else "FAIL_VISIBILITY",
        "evidence_scope": scope,
        "claim_boundary": manifest["claim_boundary"],
        "protocol": {"directory": str(protocol_dir), "manifest_sha256": manifest_digest, "trace_sha256": trace_digest},
...
def main() -> None:
    ...
    make.add_argument("--scope", choices=("development_smoke_only", "development_calibration_only", "heldout_test_once"), default="development_smoke_only")
    ...
    run.add_argument("--execution-plan", default=None, help="pre-run JSON that binds the protocol manifest and exact HNSW configuration")
