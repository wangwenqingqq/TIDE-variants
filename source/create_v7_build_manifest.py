#!/usr/bin/env python3
"""CPU-only manifest for the isolated v7 source and compiled binaries."""
...
def main() -> None:
    ...
    inspector_source=root/"tools"/"canonical_inspect_v7.cpp"
    inspector_binary=root/"builds"/"canonical_inspect_v7"
    ...
    out={"schema":"gtspp-c1-v7-build-manifest-v1","generated_utc":datetime.now(timezone.utc).isoformat(),
    ...
    (root/"worktree_build_manifest_v7.json").write_text(json.dumps(out,indent=2,sort_keys=True)+"\n")
    print(json.dumps({"source_files":len(files),"variants":len(variants),"manifest":str(root/"worktree_build_manifest_v7.json")}))
