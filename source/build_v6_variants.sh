...
[[ -f "$WT/src/c1_microbench.cu" && -f "$WT/include/update.cuh" ]] || { echo 'missing v6 worktree' >&2; exit 64; }
...
INSPECT_SRC="$ROOT/tools/canonical_inspect_v6.cpp"
INSPECT_BIN="$ROOT/builds/canonical_inspect_v6"
[[ -f "$INSPECT_SRC" && ! -L "$INSPECT_SRC" ]] || { echo 'missing canonical inspector source' >&2; exit 64; }
"$CXX_BIN" -std=c++17 -O3 -Wall -Wextra -Werror "$INSPECT_SRC" -o "$INSPECT_BIN" >"$ROOT/build_logs/build_canonical_inspect_v6.log" 2>&1
[[ -x "$INSPECT_BIN" && ! -L "$INSPECT_BIN" ]] || { echo 'canonical inspector build failed' >&2; exit 70; }
echo 'V6_CPU_CANONICAL_INSPECTOR_BUILT'
...
for variant in "${variants[@]}"; do
  ...
  echo "V6_CPU_NVCC_BUILT $variant"
done
python3 -B "$ROOT/create_v6_build_manifest.py" --root "$ROOT"
echo 'V6_BUILD_COMPLETE_NO_CUDA_BINARY_EXECUTED'
