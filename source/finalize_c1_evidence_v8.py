#!/usr/bin/env python3
"""Final CPU-only attestation written only after the v8 guard reaches COMPLETE."""
...
def main()->int:
    ...
    manifest=obj(root/"run_manifest_v8.json","manifest"); card=obj(root/"guard_run_card_v8.json","guard card")
    semantic_path=root/"semantic_verification_v8.json"; analysis_path=root/"analysis_summary_v8.json"
    semantic=obj(semantic_path,"semantic report"); analysis=obj(analysis_path,"analysis report")
    need(manifest.get("schema")=="gtspp-c1-v8-run-manifest-v2" and manifest.get("execution_mode")=="MEASURED","manifest mode/schema")
    ...
    need(not (root/".c1_guard_session_v8.json").exists(),"guard session residue")
    ...
    result={"schema":"gtspp-c1-v8-measured-evidence-v2","attested_utc":datetime.now(timezone.utc).isoformat(),"run_root":str(root),"pins_sha256":pin_sha,
      "manifest_sha256":sha(root/"run_manifest_v8.json"),"guard_card_sha256":sha(root/"guard_run_card_v8.json"),
    ...
    out=root/"final_evidence_v8.json";tmp=out.with_suffix(".tmp");tmp.write_text(json.dumps(result,indent=2,sort_keys=True)+"\n");tmp.replace(out)
