...
// transitive archive tree.cuh CHECK macro throws on CUDA failure; the Safe-C1
// caller catches it, enters FailedStop, and never emits a successful receipt.
...
// Stage-0 host-only forensic state. It never owns or reuses CUDA allocations.
struct SafeC1FreshAllocStackEvent {
	int kind = 0;  // 0=push, 1=pop
	int qs = 0, qe = 0, cur_level = 0, qnum_up = 0, offset_n = 0, qs_up = 0, size_a = 0;
	std::uint64_t depth_after = 0;
};
struct SafeC1FreshAllocSizeListWrite { int level = -1; int value = 0; };
struct SafeC1FreshAllocTraversalStep {
	int qs = 0, qe = 0, cur_level = 0, qnum_up = 0, offset_n = 0, qs_up = 0, size_a = 0;
	int qnum_l = 0, offset_p = 0, offset_up_p = 0, nnum_l = 0;
	int pnum_level = -1, pnum_level_total = -1, leaf_lnum = -1, receipt_stride = -1;
	bool update_disk_before = false, used_label_cnode = false, update_disk_after = false;
};
enum SafeC1FreshAllocDirectSlot {
	SAFE_C1_FRESH_ALLOC_LOCAL_RESULT_IDS = 0,
	SAFE_C1_FRESH_ALLOC_RES_DIS = 1,
	SAFE_C1_FRESH_ALLOC_SIZE_LIST = 2,
	SAFE_C1_FRESH_ALLOC_DISK = 3,
	SAFE_C1_FRESH_ALLOC_P_LIST_K = 4,
};
enum SafeC1FreshAllocDirectOperation {
	SAFE_C1_FRESH_ALLOC_DIRECT_ALLOC = 0,
	SAFE_C1_FRESH_ALLOC_DIRECT_FREE = 1,
};
struct SafeC1FreshAllocDirectCudaEvent {
	int slot = -1;
	int operation = -1;
	std::uint64_t requested_bytes = 0;
};
struct SafeC1FreshAllocTelemetry {
	std::uint64_t allocation_epoch = 0;
	bool entry_stack_empty = false, receipt_cleared = false, update_disk_at_header_entry = false;
	bool entry_named_global_scratch_ptrs_null = false;
	bool normal_return = false, exit_stack_empty = false;
	int qnum = 0, k = 0, tree_h = 0;
	std::uint64_t raw_cuda_mem_avail_bytes = 0, cuda_mem_total_bytes = 0;
	std::uint64_t policy_available_bytes = 0, p_list_elements = 0, p_list_requested_bytes = 0;
	std::uint64_t res_dis_requested_bytes = 0, size_list_requested_bytes = 0;
	std::uint64_t disk_requested_bytes = 0, local_result_ids_requested_bytes = 0;
	bool res_dis_allocated = false, size_list_allocated = false, disk_allocated = false, p_list_allocated = false;
	bool p_list_freed = false, size_list_freed = false, disk_freed = false;
	bool res_dis_freed_by_adapter = false, local_result_ids_allocated = false, local_result_ids_freed_by_adapter = false;
	std::uint64_t stack_push_count = 0, stack_pop_count = 0, max_stack_depth = 0;
	std::vector<SafeC1FreshAllocStackEvent> stack_events;
	std::vector<SafeC1FreshAllocSizeListWrite> size_list_writes;
	std::vector<SafeC1FreshAllocTraversalStep> traversal_steps;
	std::vector<SafeC1VisitedLeafPair> ordered_leaf_pairs;
	std::vector<SafeC1FreshAllocDirectCudaEvent> direct_allocation_events;
};
static std::uint64_t safe_c1_fresh_alloc_epoch_counter = 0;
static SafeC1FreshAllocTelemetry safe_c1_last_fresh_alloc_telemetry;
void safe_c1_fresh_alloc_begin_epoch(int qnum, int k, int tree_h);
void safe_c1_fresh_alloc_record_stack_event(
	int kind, int qs_value, int qe_value, int cur_level_value, int qnum_up_value,
	int offset_n_value, int qs_up_value, int size_a_value);
inline void safe_c1_fresh_alloc_record_direct_cuda_event(
	int slot, int operation, std::uint64_t requested_bytes) {
	if (slot < SAFE_C1_FRESH_ALLOC_LOCAL_RESULT_IDS || slot > SAFE_C1_FRESH_ALLOC_P_LIST_K ||
		operation < SAFE_C1_FRESH_ALLOC_DIRECT_ALLOC || operation > SAFE_C1_FRESH_ALLOC_DIRECT_FREE) {
		safe_c1_traversal_fail_stop("invalid fresh-allocation direct CUDA event");
	}
	safe_c1_last_fresh_alloc_telemetry.direct_allocation_events.push_back(
		{slot, operation, requested_bytes});
}
inline void safe_c1_fresh_alloc_record_size_list_write(int level, int value) {
	safe_c1_last_fresh_alloc_telemetry.size_list_writes.push_back({level, value});
}
#include "file.cuh"
#include "tree.cuh"
#include "config.cuh"
...
inline void safe_c1_fresh_alloc_begin_epoch(int qnum_value, int k_value, int tree_h_value) {
	const bool stack_empty = st.empty();
	const bool named_globals_null =
		res_dis == nullptr && size_list == nullptr && disk == nullptr && p_list_k == nullptr;
	// This gate is deliberately before local_result_ids is allocated by the
	// matrix adapter.  A stale traversal stack or named scratch pointer cannot
	// be silently carried into a purportedly fresh-allocation epoch.
	if (!stack_empty || !named_globals_null || update_disk) {
		safe_c1_traversal_fail_stop("non-fresh Stage-0 state before allocation epoch");
	}
	safe_c1_last_visited_leaf_pairs.clear();
	const bool receipt_cleared = safe_c1_last_visited_leaf_pairs.empty();
	if (!receipt_cleared) {
		safe_c1_traversal_fail_stop("unable to clear Stage-0 traversal receipt");
	}
	safe_c1_last_fresh_alloc_telemetry = SafeC1FreshAllocTelemetry{};
	safe_c1_last_fresh_alloc_telemetry.allocation_epoch = ++safe_c1_fresh_alloc_epoch_counter;
	safe_c1_last_fresh_alloc_telemetry.entry_stack_empty = stack_empty;
	safe_c1_last_fresh_alloc_telemetry.entry_named_global_scratch_ptrs_null = named_globals_null;
	safe_c1_last_fresh_alloc_telemetry.receipt_cleared = receipt_cleared;
	safe_c1_last_fresh_alloc_telemetry.update_disk_at_header_entry = update_disk;
	safe_c1_last_fresh_alloc_telemetry.qnum = qnum_value;
	safe_c1_last_fresh_alloc_telemetry.k = k_value;
	safe_c1_last_fresh_alloc_telemetry.tree_h = tree_h_value;
	safe_c1_last_fresh_alloc_telemetry.max_stack_depth = 0;
}

inline void safe_c1_fresh_alloc_record_stack_event(
	int kind, int qs_value, int qe_value, int cur_level_value, int qnum_up_value,
	int offset_n_value, int qs_up_value, int size_a_value) {
	SafeC1FreshAllocStackEvent event;
	event.kind = kind; event.qs = qs_value; event.qe = qe_value; event.cur_level = cur_level_value;
	event.qnum_up = qnum_up_value; event.offset_n = offset_n_value; event.qs_up = qs_up_value;
	event.size_a = size_a_value; event.depth_after = static_cast<std::uint64_t>(st.size());
	safe_c1_last_fresh_alloc_telemetry.stack_events.push_back(event);
	if (kind == 0) {
		++safe_c1_last_fresh_alloc_telemetry.stack_push_count;
		if (event.depth_after > safe_c1_last_fresh_alloc_telemetry.max_stack_depth)
			safe_c1_last_fresh_alloc_telemetry.max_stack_depth = event.depth_after;
	} else {
		++safe_c1_last_fresh_alloc_telemetry.stack_pop_count;
	}
}

inline int safe_c1_fresh_alloc_checked_receipt_int(double value, const char* label) {
	const double lower = static_cast<double>(std::numeric_limits<int>::min());
	...
	if (!std::isfinite(value) || std::trunc(value) != value || value < lower || value > upper) {
		safe_c1_traversal_fail_stop(label);
	}
	return static_cast<int>(value);
}

// The archive traversal uses process-global scratch pointers.  This guard is
// armed only for the copied Stage-0 vector KNN implementation and releases
// successfully obtained named scratch allocations on any exceptional path.
// It deliberately emits no successful-lifecycle event: a throwing query must
// never produce a usable witness.
struct SafeC1FreshAllocGlobalScratchCleanup {
	bool active = true;
	~SafeC1FreshAllocGlobalScratchCleanup() noexcept {
		if (!active) return;
...
void searchIndexKnnV2(short *data_d, TN *node_list, int *id_list, int *max_node_num, float *query_data,
				  int *res_ids, int qnum, int k, int tree_h, int *data_info, int *empty_list, char *data_s, int *size_s)
{
	// The matrix adapter begins this telemetry epoch before local_result_ids is
	// allocated, so every named direct CUDA allocation belongs to one ledger.
	if (safe_c1_last_fresh_alloc_telemetry.allocation_epoch == 0 ||
		safe_c1_last_fresh_alloc_telemetry.qnum != qnum ||
		safe_c1_last_fresh_alloc_telemetry.k != k ||
		safe_c1_last_fresh_alloc_telemetry.tree_h != tree_h ||
		!safe_c1_last_fresh_alloc_telemetry.entry_stack_empty ||
		!safe_c1_last_fresh_alloc_telemetry.entry_named_global_scratch_ptrs_null ||
		!safe_c1_last_fresh_alloc_telemetry.receipt_cleared ||
		safe_c1_last_fresh_alloc_telemetry.update_disk_at_header_entry ||
		!st.empty() || res_dis != nullptr || size_list != nullptr || disk != nullptr || p_list_k != nullptr) {
		safe_c1_traversal_fail_stop("missing fresh-allocation epoch before vector KNN");
	}
	SafeC1FreshAllocGlobalScratchCleanup fresh_alloc_exception_cleanup;
	cout << "Searching..." << endl;

	safe_c1_last_fresh_alloc_telemetry.res_dis_requested_bytes =
		static_cast<std::uint64_t>(qnum) * static_cast<std::uint64_t>(k) * sizeof(float);
	CHECK(cudaMallocManaged((void **)&res_dis, qnum * k * sizeof(float)));
	safe_c1_last_fresh_alloc_telemetry.res_dis_allocated = true;
	safe_c1_fresh_alloc_record_direct_cuda_event(
		SAFE_C1_FRESH_ALLOC_RES_DIS, SAFE_C1_FRESH_ALLOC_DIRECT_ALLOC,
		safe_c1_last_fresh_alloc_telemetry.res_dis_requested_bytes);
	safe_c1_last_fresh_alloc_telemetry.size_list_requested_bytes =
		static_cast<std::uint64_t>(tree_h + 1) * sizeof(int);
	CHECK(cudaMallocManaged((void **)&size_list, (tree_h + 1) * sizeof(int)));
	safe_c1_last_fresh_alloc_telemetry.size_list_allocated = true;
	safe_c1_fresh_alloc_record_direct_cuda_event(
		SAFE_C1_FRESH_ALLOC_SIZE_LIST, SAFE_C1_FRESH_ALLOC_DIRECT_ALLOC,
		safe_c1_last_fresh_alloc_telemetry.size_list_requested_bytes);
	safe_c1_last_fresh_alloc_telemetry.disk_requested_bytes =
		static_cast<std::uint64_t>(qnum) * sizeof(float);
	CHECK(cudaMalloc((void **)&disk, qnum * sizeof(float)));
	safe_c1_last_fresh_alloc_telemetry.disk_allocated = true;
	safe_c1_fresh_alloc_record_direct_cuda_event(
	...
	size_t total;
	safe_c1_traversal_require_cuda_success(cudaMemGetInfo(&avail, &total), "cudaMemGetInfo");
	safe_c1_last_fresh_alloc_telemetry.raw_cuda_mem_avail_bytes = static_cast<std::uint64_t>(avail);
	safe_c1_last_fresh_alloc_telemetry.cuda_mem_total_bytes = static_cast<std::uint64_t>(total);
	size_t limit = 4ULL * 1024 * 1024 * 1024;
	if (avail > limit * 2)
	{
		avail = limit;
	...
	// Allocate memory
	safe_c1_last_fresh_alloc_telemetry.policy_available_bytes = static_cast<std::uint64_t>(avail);
	size_a = avail / (sizeof(double));
	safe_c1_last_fresh_alloc_telemetry.p_list_elements = static_cast<std::uint64_t>(size_a);
	safe_c1_last_fresh_alloc_telemetry.p_list_requested_bytes =
		static_cast<std::uint64_t>(size_a) * sizeof(double);
	CHECK(cudaMalloc((void **)&p_list_k, size_a * sizeof(double)));
	safe_c1_last_fresh_alloc_telemetry.p_list_allocated = true;
	safe_c1_fresh_alloc_record_direct_cuda_event(
	...
	CHECK(cudaMemset(size_list, 0, (tree_h + 1) * sizeof(int)));
	size_list[0] = qnum;
	safe_c1_fresh_alloc_record_size_list_write(0, qnum);
	size_a -= qnum;
	size_avg = size_a / tree_h;
	nnum_l = TREE_ORDER;
	qnum_l = min(size_avg / (nnum_l + nnum_l / TREE_ORDER * 3), qnum);
	size_a -= qnum_l * (nnum_l + nnum_l / TREE_ORDER);
	size_list[1] = qnum_l * (nnum_l + nnum_l / TREE_ORDER);
	safe_c1_fresh_alloc_record_size_list_write(1, qnum_l * (nnum_l + nnum_l / TREE_ORDER));
	for (int i = 0; i < qnum; i += qnum_l)
	{
		int end = min(i + qnum_l - 1, qnum - 1);
		st.push(i);
		...
		st.push(0);
		st.push(size_a);
		safe_c1_fresh_alloc_record_stack_event(0, i, end, 1, qnum, 1, 0, size_a);
	}
	initPListKnn<<<(qnum + THREAD_NUM - 1) / THREAD_NUM, THREAD_NUM>>>(p_list_k, qnum);
	safe_c1_traversal_require_cuda_success(cudaDeviceSynchronize(), "cudaDeviceSynchronize");
	cudaError_t cudaStatus = cudaGetLastError();
	...
	while (!st.empty())
	{
		...
		nnum_l = pow(TREE_ORDER, cur_level);
		int block_num = (qnum_l * nnum_l + THREAD_NUM - 1) / THREAD_NUM;
		safe_c1_fresh_alloc_record_stack_event(1, qs, qe, cur_level, qnum_up, offset_n, qs_up, size_a);
		safe_c1_last_fresh_alloc_telemetry.traversal_steps.push_back({});
		SafeC1FreshAllocTraversalStep& safe_c1_telemetry_step =
			safe_c1_last_fresh_alloc_telemetry.traversal_steps.back();
		safe_c1_telemetry_step.qs = qs; safe_c1_telemetry_step.qe = qe;
		safe_c1_telemetry_step.cur_level = cur_level; safe_c1_telemetry_step.qnum_up = qnum_up;
		safe_c1_telemetry_step.offset_n = offset_n; safe_c1_telemetry_step.qs_up = qs_up;
		safe_c1_telemetry_step.size_a = size_a; safe_c1_telemetry_step.qnum_l = qnum_l;
		safe_c1_telemetry_step.offset_p = offset_p; safe_c1_telemetry_step.offset_up_p = offset_up_p;
		safe_c1_telemetry_step.nnum_l = nnum_l; safe_c1_telemetry_step.update_disk_before = update_disk;
		...
		if (cur_level < tree_h)
		{
			...
			pnum_level = pnum_level / TREE_ORDER;
			int pnum_level_total = nnum_l / TREE_ORDER;
			safe_c1_telemetry_step.pnum_level = pnum_level;
			safe_c1_telemetry_step.pnum_level_total = pnum_level_total;

			if (update_disk == false && (pnum_level < k || cur_level <= 2))
			{
				safe_c1_telemetry_step.used_label_cnode = true;
				labelCNode<<<block_num, THREAD_NUM>>>(empty_list, qnum_l, qnum_up, nnum_l, p_list_k, offset_p, offset_up_p, offset_n, qs, qs_up);
				safe_c1_traversal_require_cuda_success(cudaDeviceSynchronize(), "cudaDeviceSynchronize");
				cudaStatus = cudaGetLastError();
				safe_c1_traversal_require_cuda_success(cudaStatus, "labelCNode");
			...
			safe_c1_telemetry_step.update_disk_after = update_disk;
		}
		else
		{
			// Processing data in leaf node.
			...
			// Gets the prefix sum of p_list at leaf node layer.
			int lnum = thrust::reduce(thrust::device, p_list_k + offset_p, p_list_k + offset_p + (le - ls) * nnum_up, 0);
			safe_c1_telemetry_step.leaf_lnum = lnum;
			safe_c1_telemetry_step.update_disk_after = update_disk;
			thrust::exclusive_scan(thrust::device, p_list_k + offset_p, p_list_k + offset_p + (le - ls) * nnum_up,
								   p_list_k + offset_p);
			...
			if (lnum > 0) {
				...
				safe_c1_telemetry_step.receipt_stride = receipt_stride;
				...
				for (int receipt_index = 0; receipt_index < lnum; ++receipt_index) {
					const int leaf_id = safe_c1_fresh_alloc_checked_receipt_int(
						receipt_leaf[receipt_index], "invalid non-integral GTS KNN leaf receipt");
					const int query_id = safe_c1_fresh_alloc_checked_receipt_int(
						receipt_query[receipt_index], "invalid non-integral GTS KNN query receipt");
		...
		if (cur_level < tree_h)
		{
			...
			if (cur_level_low < tree_h)
			{
				...
				int qnum_l_low = min(size_avg_low / (nnum_l_low + nnum_l_low / TREE_ORDER * 3), qnum_up);
				size_list[cur_level_low] = qnum_l_low * (nnum_l_low + nnum_l_low / TREE_ORDER * 3);
				safe_c1_fresh_alloc_record_size_list_write(
					cur_level_low, qnum_l_low * (nnum_l_low + nnum_l_low / TREE_ORDER * 3));
				for (int i = 0; i < qnum_l; i += qnum_l_low)
				{
					int end = min(i + qs + qnum_l_low - 1, qe);
					st.push(i + qs);
					...
					st.push(qs);
					st.push(size_a - size_list[cur_level_low]);
					safe_c1_fresh_alloc_record_stack_event(
						0, i + qs, end, cur_level_low, qnum_l, offset_n_low, qs,
						size_a - size_list[cur_level_low]);
				}
			}
			else
			{
				...
				size_list[cur_level_low] = size_l * (MAX_SIZE * 3 + 3);
				safe_c1_fresh_alloc_record_size_list_write(
					cur_level_low, static_cast<int>(size_l * (MAX_SIZE * 3 + 3)));
				unsigned long qnum_l_low = size_l / (nnum_l);
				printf("qnum_l_low: %lu\n", static_cast<unsigned long>(qnum_l_low));
				for (int i = 0; i < qnum_l; i += qnum_l_low)
				{
					...
					st.push(size_a - size_list[cur_level_low]);
					safe_c1_fresh_alloc_record_stack_event(
						0, i, end, cur_level_low, qnum_l, offset_n, qs,
						size_a - size_list[cur_level_low]);
				}
			}
		}
	...
	// A normal witness is legal only after every pushed traversal frame was
	// consumed and every header-owned direct allocation is still live here.
	safe_c1_last_fresh_alloc_telemetry.exit_stack_empty = st.empty();
	if (!safe_c1_last_fresh_alloc_telemetry.exit_stack_empty || p_list_k == nullptr ||
		size_list == nullptr || disk == nullptr || res_dis == nullptr) {
		safe_c1_traversal_fail_stop("incomplete Stage-0 vector KNN exit state");
	}
	safe_c1_last_fresh_alloc_telemetry.ordered_leaf_pairs = safe_c1_last_visited_leaf_pairs;
	safe_c1_traversal_require_cuda_success(cudaFree(p_list_k), "cudaFree p_list_k");
	p_list_k = nullptr;
	safe_c1_last_fresh_alloc_telemetry.p_list_freed = true;
	safe_c1_fresh_alloc_record_direct_cuda_event(
		SAFE_C1_FRESH_ALLOC_P_LIST_K, SAFE_C1_FRESH_ALLOC_DIRECT_FREE,
		safe_c1_last_fresh_alloc_telemetry.p_list_requested_bytes);
	safe_c1_traversal_require_cuda_success(cudaFree(size_list), "cudaFree size_list");
	size_list = nullptr;
	safe_c1_last_fresh_alloc_telemetry.size_list_freed = true;
	safe_c1_fresh_alloc_record_direct_cuda_event(
		SAFE_C1_FRESH_ALLOC_SIZE_LIST, SAFE_C1_FRESH_ALLOC_DIRECT_FREE,
		safe_c1_last_fresh_alloc_telemetry.size_list_requested_bytes);
	safe_c1_traversal_require_cuda_success(cudaFree(disk), "cudaFree disk");
	disk = nullptr;
	safe_c1_last_fresh_alloc_telemetry.disk_freed = true;
	safe_c1_fresh_alloc_record_direct_cuda_event(
		SAFE_C1_FRESH_ALLOC_DISK, SAFE_C1_FRESH_ALLOC_DIRECT_FREE,
		safe_c1_last_fresh_alloc_telemetry.disk_requested_bytes);
	// Only a completed traversal clears its mutable disk-update state for the
	// next epoch.  The entry gate therefore detects stale state before this.
	update_disk = false;
	safe_c1_last_fresh_alloc_telemetry.normal_return = true;
	fresh_alloc_exception_cleanup.dismiss();
}