...
#include "tree.cuh"
#include "g3_safe_search_v2_fresh_alloc_telemetry.cuh"
...
struct CertificateSiblingDiagnostic {
  ...
  float child_max_distance = std::numeric_limits<float>::quiet_NaN();
  float next_sibling_min_minus_child_max_distance = std::numeric_limits<float>::quiet_NaN();
  ...
  bool child_max_distance_finite = false;
  bool next_sibling_min_minus_child_max_distance_finite = false;
  bool last_child = false;
...
struct FrozenCertificateDiagnostic {
  ...
  int sidecar_live = -1;
  int delta_live = -1;
  bool raw_predicate_cross_checked = false;
};
      ...
      for (int slot = 0; slot < fanout; ++slot) {
        ...
        if (child >= 0 && child < node_count) {
          ...
          if (!sibling.child_empty) {
            ...
            if (child < static_cast<int>(max_distance.size())) {
              sibling.child_max_distance = max_distance[child];
              sibling.child_max_distance_finite =
                  std::isfinite(sibling.child_max_distance);
            }
          }
        }
        ...
        if (!sibling.last_child) {
          ...
          if (sibling.next_sibling >= 0 && sibling.next_sibling < node_count) {
            ...
            if (sibling.next_sibling_available) {
              ...
              if (sibling.child_max_distance_finite &&
                  sibling.next_sibling_min_dis_finite) {
                sibling.next_sibling_min_minus_child_max_distance =
                    sibling.upper - sibling.child_max_distance;
                sibling.next_sibling_min_minus_child_max_distance_finite = true;
              }
            }
          }
        }
...
struct TraversalReceipt {
  ...
  // Explicitly distinguish the receipt-defined Safe-C1 result contract from
  // legacy res_ids diagnostic completeness.
  ...
  std::vector<float> native_final_res_distances;
  std::vector<SafeC1VisitedLeafPair> ordered_raw_leaf_pairs;
  SafeC1FreshAllocTelemetry fresh_allocation_telemetry;
  // Attribution-only audit of the legacy res_ids buffer: every non-sentinel
  // slot is checked against the receipt, but this buffer never defines the
  // Safe-C1 candidate/result contract or a completeness claim.
...
// A successful Stage-0 witness must expose the full, direct CUDA allocation
// lifecycle in program order.  This is checked in the adapter before any
// telemetry is exported; the runner repeats the same contract before writing.
inline void require_complete_fresh_allocation_lifecycle(
    const SafeC1FreshAllocTelemetry& telemetry) {
  if (!telemetry.entry_stack_empty || !telemetry.entry_named_global_scratch_ptrs_null ||
      !telemetry.receipt_cleared || telemetry.update_disk_at_header_entry ||
      !telemetry.normal_return || !telemetry.exit_stack_empty ||
      !telemetry.local_result_ids_allocated || !telemetry.res_dis_allocated ||
      !telemetry.size_list_allocated || !telemetry.disk_allocated ||
      !telemetry.p_list_allocated || !telemetry.p_list_freed ||
      !telemetry.size_list_freed || !telemetry.disk_freed ||
      !telemetry.res_dis_freed_by_adapter || !telemetry.local_result_ids_freed_by_adapter) {
    fail("incomplete Stage-0 named fresh-allocation lifecycle");
  }
  ...
  const std::array<ExpectedEvent, 10> expected{{
      ...
      {SAFE_C1_FRESH_ALLOC_DISK, SAFE_C1_FRESH_ALLOC_DIRECT_FREE,
       telemetry.disk_requested_bytes},
      {SAFE_C1_FRESH_ALLOC_RES_DIS, SAFE_C1_FRESH_ALLOC_DIRECT_FREE,
       telemetry.res_dis_requested_bytes},
      {SAFE_C1_FRESH_ALLOC_LOCAL_RESULT_IDS, SAFE_C1_FRESH_ALLOC_DIRECT_FREE,
       telemetry.local_result_ids_requested_bytes},
  }};
  if (telemetry.direct_allocation_events.size() != expected.size()) {
    fail("Stage-0 direct CUDA event ledger cardinality mismatch");
  }
  for (std::size_t index = 0; index < expected.size(); ++index) {
    const SafeC1FreshAllocDirectCudaEvent& actual =
        telemetry.direct_allocation_events[index];
    if (actual.slot != expected[index].slot || actual.operation != expected[index].operation ||
        actual.requested_bytes != expected[index].bytes) {
      fail("Stage-0 direct CUDA event ledger order/value mismatch");
    }
  }
}
...
inline TraversalReceipt run_gts_base_topk_with_receipt(
  ...
  // Begin the diagnostic ledger before the first named direct CUDA allocation.
  // Do not pre-reset update_disk here: begin_epoch must observe and reject any
  // state inherited from a prior failed/unfinished traversal.  Both clean
  // header exit and exception cleanup reset it only after their own contract.
  safe_c1_fresh_alloc_begin_epoch(qnum, k, runtime.tree_height);
  int* local_result_ids = nullptr;
  const std::uint64_t local_result_ids_bytes =
      static_cast<std::uint64_t>(qnum) * static_cast<std::uint64_t>(k) * sizeof(int);
  // The try scope begins before the first local allocation.  Any host/CUDA
  // exception after a successful allocation leaves no reusable scratch state.
  try {
    G3_CUDA(cudaMallocManaged(reinterpret_cast<void**>(&local_result_ids),
                              static_cast<std::size_t>(local_result_ids_bytes)));
    safe_c1_last_fresh_alloc_telemetry.local_result_ids_allocated = true;
    safe_c1_last_fresh_alloc_telemetry.local_result_ids_requested_bytes = local_result_ids_bytes;
    safe_c1_fresh_alloc_record_direct_cuda_event(
        SAFE_C1_FRESH_ALLOC_LOCAL_RESULT_IDS, SAFE_C1_FRESH_ALLOC_DIRECT_ALLOC,
        local_result_ids_bytes);
    ...
    // This immutable by-value snapshot is the sole receipt source for every
    // adapter export.  No later read of the mutable global receipt vector is
    // permitted after header return.
    const SafeC1FreshAllocTelemetry fresh_header = safe_c1_last_fresh_alloc_telemetry;
    if (!fresh_header.normal_return || !fresh_header.exit_stack_empty ||
        !fresh_header.entry_named_global_scratch_ptrs_null ||
        !fresh_header.local_result_ids_allocated || !fresh_header.res_dis_allocated ||
        !fresh_header.size_list_allocated || !fresh_header.disk_allocated ||
        !fresh_header.p_list_allocated || !fresh_header.p_list_freed ||
        !fresh_header.size_list_freed || !fresh_header.disk_freed) {
      fail("fresh-allocation diagnostic observed incomplete pre-adapter lifecycle");
    }
    if (fresh_header.ordered_leaf_pairs.size() >
        static_cast<std::size_t>(std::numeric_limits<int>::max())) {
      fail("native GTS receipt pair count exceeds adapter int contract");
    }
    const int required_base_results = std::min(k, runtime.base_count);
    output.native_result_required_count = required_base_results;
    if (required_base_results > 0 && fresh_header.ordered_leaf_pairs.empty()) {
      fail("native GTS top-k receipt selected no leaf for a nonempty immutable base");
    }
    output.ordered_raw_leaf_pairs = fresh_header.ordered_leaf_pairs;
    output.raw_receipt_leaf_pair_count =
        static_cast<int>(fresh_header.ordered_leaf_pairs.size());
    for (const SafeC1VisitedLeafPair& pair : fresh_header.ordered_leaf_pairs) {
    ...
    if (static_cast<int>(output.base_receipt_rows.size()) < required_base_results) {
      fail("native GTS receipt materialized fewer base rows than the requested top-k contract");
    }
    ...
    if (static_cast<int>(output.base_results.size()) < required_base_results) {
      fail("native GTS receipt exact scan cannot supply the requested base top-k");
    }

    // res_ids/res_dis are diagnostics only and never define the exact
    // receipt-derived candidate set.
    if (!res_dis) fail("native traversal returned without res_dis for Stage-0 witness");
    std::set<LocalRow> native_result_locals;
    for (int index = 0; index < qnum * k; ++index) {
      const LocalRow local = local_result_ids[index];
      output.native_final_res_ids.push_back(local);
      output.native_final_res_distances.push_back(res_dis[index]);
      if (local == -1) continue;
      if (local < 0 || local >= runtime.base_count) {
      ...
      if (!native_result_locals.insert(local).second) {
        fail("native top-k res_ids contains a duplicate non-sentinel local row");
      }
    }
    // The archive-owned res_ids buffer is a diagnostic cross-check only.  The
    // Safe-C1 result is defined above by exact scanning of all receipt leaf
    // rows, which already must supply required_base_results.  Do not promote
    // this buffer's non-sentinel count into a hidden completeness contract;
    // the sidecar records both the count and this explicit comparison, while
    // every non-sentinel slot remains required to be valid, unique, and
    // receipt-attributable.
    output.native_result_complete =
    ...
    G3_CUDA(cudaFree(res_dis));
    res_dis = nullptr;
    safe_c1_last_fresh_alloc_telemetry.res_dis_freed_by_adapter = true;
    safe_c1_fresh_alloc_record_direct_cuda_event(
        SAFE_C1_FRESH_ALLOC_RES_DIS, SAFE_C1_FRESH_ALLOC_DIRECT_FREE,
        safe_c1_last_fresh_alloc_telemetry.res_dis_requested_bytes);
    G3_CUDA(cudaFree(local_result_ids));
    local_result_ids = nullptr;
    safe_c1_last_fresh_alloc_telemetry.local_result_ids_freed_by_adapter = true;
    safe_c1_fresh_alloc_record_direct_cuda_event(
        SAFE_C1_FRESH_ALLOC_LOCAL_RESULT_IDS, SAFE_C1_FRESH_ALLOC_DIRECT_FREE,
        safe_c1_last_fresh_alloc_telemetry.local_result_ids_requested_bytes);
    const SafeC1FreshAllocTelemetry& completed = safe_c1_last_fresh_alloc_telemetry;
    require_complete_fresh_allocation_lifecycle(completed);
    if (!st.empty() || res_dis != nullptr || size_list != nullptr || disk != nullptr ||
        p_list_k != nullptr || local_result_ids != nullptr) {
      fail("Stage-0 direct CUDA lifecycle left live traversal scratch");
    }
    output.fresh_allocation_telemetry = completed;
    return output;
  } catch (...) {
    if (res_dis != nullptr) { (void)cudaFree(res_dis); res_dis = nullptr; }
    if (local_result_ids != nullptr) { (void)cudaFree(local_result_ids); local_result_ids = nullptr; }
    throw;
  }
}
...
struct QueryExport {
  ...
  int full_immutable_base_candidate_count = 0;
  int id_list_capacity = 0;
  // Explicit legacy res_ids diagnostic metadata; these do not alter results.
  int native_result_required_count = 0;
  bool native_result_complete = false;
  // Attribution-only audit of the legacy res_ids buffer: every non-sentinel
  // slot is checked against the receipt, but this buffer never defines the
...
class NativeSafeC1Matrix {
 public:
  ...
  [[nodiscard]] FrozenCertificateDiagnostic inspect_initial_frozen_certificate(
    ...
    std::lock_guard<std::mutex> api_guard(api_mutex_);
    try {
      if (G3_DIRECT_SIDECAR_RUNTIME_GUARD_OPEN ||
          mode_ != EngineMode::kAwaitingPostRebuildOracle || tree_version_ != 1 ||
          state_epoch_ != 1 || !runtime_.ready() || !frozen_.initialized() ||
          state_.sidecar_live() != 0 || state_.delta_live() != 0) {
        enter_failed_stop_noexcept();
      ...
      // The raw diagnostic repeats the strict comparison only as an explanatory
      // field.  Bind it back to the existing certifier's outcome before it can
      // be published, so it cannot silently become a divergent second predicate.
      const auto require_diagnostic = [](bool condition, const std::string& reason) {
        if (!condition) fail(reason);
      };
      const auto require_raw_predicate_cross_check =
          [&require_diagnostic](FrozenCertificateDiagnostic* value) {
            require_diagnostic(value != nullptr, "null frozen certificate diagnostic");
            const StrictCertificate& certificate = value->certificate;
            for (std::size_t level_index = 0;
                 level_index < certificate.levels.size(); ++level_index) {
              require_diagnostic(level_index < value->levels.size(),
                      "raw frozen certificate omits a selected certificate level");
              const CertificateDiagnosticLevel& raw = value->levels[level_index];
              const CertificateLevel& selected = certificate.levels[level_index];
              require_diagnostic(raw.diagnostic_strict_match_count == 1,
                      "raw frozen predicate does not have exactly one selected child");
              const auto found = std::find_if(
                  raw.siblings.begin(), raw.siblings.end(),
                  [&selected](const CertificateSiblingDiagnostic& sibling) {
                    return sibling.child == selected.child &&
                           sibling.strict_native_predicate_match;
                  });
              require_diagnostic(found != raw.siblings.end(),
                      "raw frozen predicate selected child diverges from certifier");
            }
            if (certificate.failure_reason == "native_sibling_gap_or_boundary" ||
                certificate.failure_reason == "native_sibling_overlap_or_ambiguous") {
              require_diagnostic(certificate.failure_level >= 0 &&
                          static_cast<std::size_t>(certificate.failure_level) <
                              value->levels.size(),
                      "raw frozen predicate omits certificate failure level");
              require_diagnostic(value->levels[static_cast<std::size_t>(certificate.failure_level)]
                          .diagnostic_strict_match_count == certificate.matching_children,
                      "raw frozen predicate matching-child count diverges from certifier");
            }
            value->raw_predicate_cross_checked = true;
          };
      require_raw_predicate_cross_check(&diagnostic);

      diagnostic.tree_version = tree_version_;
      diagnostic.state_epoch = state_epoch_;
      diagnostic.sidecar_live = state_.sidecar_live();
      diagnostic.delta_live = state_.delta_live();
      ...
      if (mode_ != EngineMode::kAwaitingPostRebuildOracle || tree_version_ != 1 ||
          ...
          diagnostic.tree_version != tree_version_ ||
          diagnostic.state_epoch != state_epoch_ || diagnostic.sidecar_live != 0 ||
          diagnostic.delta_live != 0 || !diagnostic.raw_predicate_cross_checked) {
        enter_failed_stop_noexcept();
        fail("initial frozen certificate diagnostic observed mutable-state or predicate-binding drift");
      }
      return diagnostic;
    } catch (...) {
      enter_failed_stop_noexcept();
 ...
 private:
  ...
  QueryExport merge_query_partitions(const std::string& kind, int query_id, DistanceSq radius_sq,
    ...
    output.native_final_res_distances = base.native_final_res_distances;
    output.ordered_raw_leaf_pairs = base.ordered_raw_leaf_pairs;
    output.fresh_allocation_telemetry = base.fresh_allocation_telemetry;
    if (!range_full_base_fallback) {
