...
#include "../src/g3_safe_c1_native_matrix_silent_only.cu"
...
Args parse_args(int argc, char** argv) {
  ...
  if (args.mode == "dry-run") {
    ...
    const char* nonce = std::getenv("SAFE_C1_LATENCY_TRADEOFF_SILENT_ONLY_GUARD_NONCE");
    const char* visible = std::getenv("CUDA_VISIBLE_DEVICES");
    require(nonce != nullptr && args.timing_guard_nonce == nonce,
            "timing requires matching silent-only guard-provided nonce");
...
void write_observation(std::ostream& output, const Observation& row) {
  output << "{\"schema\":\"fair-safe-c1-latency-silent-only-e1-v1\""
         << ",\"diagnostic_variant\":\"optimization_diagnostic_silent_only\""
...
void write_dry_run(const Args& args, const Bundle& bundle, const AdmissionWitness& admission,
  ...
  events << "{\"schema\":\"fair-safe-c1-latency-silent-only-e1-v1\""
         << ",\"diagnostic_variant\":\"optimization_diagnostic_silent_only\""
         << ",\"publication_eligible\":false"
         << ",\"record\":\"dry_run\""
         << ",\"gpu_used\":false"
  ...
  summary << "{\"schema\":\"fair-safe-c1-latency-silent-only-e1-v1\""
          << ",\"diagnostic_variant\":\"optimization_diagnostic_silent_only\""
          << ",\"publication_eligible\":false"
          << ",\"mode\":\"dry-run\""
          << ",\"status\":\"PASS_SILENT_ONLY_DIAGNOSTIC_CPU_ONLY_PLAN\""
          << ",\"gpu_used\":false"
          << ",\"not_same_api\":true"
          << ",\"timing_claim\":\"latency_quality_tradeoff_only\""
          << ",\"scope\":\"silent-only diagnostic: only two vector-topk native stdout statements are suppressed; frozen immutable base plus external insertion-only global delta; no CUDA engine instantiated\""
...
int run(const Args& args) {
  ...
  summary << "{\"schema\":\"fair-safe-c1-latency-silent-only-e1-v1\""
          ...
          << ",\"publication_eligible\":false"
          << ",\"mode\":\"timing\""
          << ",\"status\":\"PASS_SILENT_ONLY_DIAGNOSTIC_TIMING\""
          << ",\"not_same_api\":true"
          << ",\"timing_claim\":\"latency_quality_tradeoff_only\""
          << ",\"scope\":\"silent-only diagnostic: only two vector-topk native stdout statements are suppressed; native query_knn candidate API versus current exact query_range(UINT64_MAX) full-result immutable-base fallback; not a fair same-API speedup\""
  ...
  summary << ",\"quality\":{\"native_exact_set_count\":" << native_exact
          ...
          << ",\"limitations\":[\"not_same_api\",\"no CUDA event/kernel timing\",\"external delta is CPU wrapper work\",\"no delete/range-workload/rebuild/direct-sidecar claim\",\"optimization_diagnostic_silent_only\",\"non_publication_variant\"]"
          << "}\n";
  summary.close();
