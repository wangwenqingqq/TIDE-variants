#!/usr/bin/env bash
# CPU-only measured-plan emitter for the v6 outer guard.
...
SELF="$ROOT/run_c1_four_variants_v6.sh"
[[ "$0" == "$SELF" && -f "$SELF" && ! -L "$SELF" ]] || { echo 'C1-V6-PLAN BLOCKED: noncanonical engine path' >&2; exit 64; }
[[ "${PYTHONOPTIMIZE:-}" == "" ]] || { echo 'C1-V6-PLAN BLOCKED: PYTHONOPTIMIZE must be unset' >&2; exit 64; }
[[ "${1:-}" == --emit-measured-plan && "$#" -eq 1 ]] || {
  echo 'Usage: run_c1_four_variants_v6.sh --emit-measured-plan' >&2; exit 64; }
