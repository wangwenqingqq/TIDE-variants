// Safe-C2 v2 tie-free corrected-after-submit full-SIFT1M interval-bound gate.
// V2 retains the Safe-C2 traversal/gamma algorithm and metric, but its frozen
// query membership excludes only public-GT-ambiguous v1 IDs before any GPU run.
//
// Scope boundary:
...
//
// This file and its headers live only under c2_interval_bound_safe. The copied
// `search_v2.cuh` has exactly two Safe-C2 semantic changes: (i) vector KNN uses
// max_dis_d in Eq. (1), and (ii) the runner certifies/replaces intervals outward
// after construction before any query. Its fixed 1 GiB workspace cap is shared
// by every stage and is not a submitted-performance configuration.
...
struct ReferenceCounters {
  ...
  int boundary_ties = 0;
  // One strict top-k overlap count per selected query; never aggregated away
  // when Safe-C2 decides whether gamma may be retained.
...
void append_timed_run(StageResult* target, const QueryRun& one) {
  target->query_wall_ms_sum += one.query.wall_ms;
  target->query_gpu_ms_sum += one.query.gpu_ms;
  target->d2h_ids_wall_ms_sum += one.d2h_ids_wall_ms;
  target->d2h_distances_wall_ms_sum += one.d2h_distances_wall_ms;
  add_counters(&target->reference_sum, one.reference);
  target->repetitions.push_back(one.reference);
}
...
// Held-out performance pairing only. Calibration uses its own correctness-only
// probes and must never be reported as a timing comparison.  Each timed pair
// alternates B->G and G->B so cache/thermal/order effects cannot consistently
// favor Safe-C2. Constant uploads occur before the timed API interval.
PairedAbbaStageResult run_paired_abba_stage(
  ...
  result.safe_c2.warmups = warmups; result.safe_c2.timed_reps = timed_reps;
  std::array<float, RP_MAX_LEVELS> all_ones{}; all_ones.fill(1.0F);
  auto execute = [&](const std::array<float, RP_MAX_LEVELS>& configuration, bool timed, StageResult* target) {
    upload_gamma(configuration);
    QueryRun one = execute_once(runtime, query_d, query_count, base, all_queries, gt, selected_ids,
                                ids_d, ids_h, distances_h, timed);
    frozen.assert_unchanged(runtime);
    if (timed) append_timed_run(target, one);
  };
  try {
    for (int warmup = 0; warmup < warmups; ++warmup) {
      // Shared, unmeasured warmups in a fixed order; timing begins only after
      // both configurations have had identical warmup count.
      execute(all_ones, false, nullptr);
      execute(gamma, false, nullptr);
      ++result.shared_warmup_pairs;
    }
    for (int rep = 0; rep < timed_reps; ++rep) {
      if ((rep % 2) == 0) {
        result.timed_pair_order.push_back("baseline_then_safe_c2");
        execute(all_ones, true, &result.baseline);
        execute(gamma, true, &result.safe_c2);
      } else {
        result.timed_pair_order.push_back("safe_c2_then_baseline");
        execute(gamma, true, &result.safe_c2);
        execute(all_ones, true, &result.baseline);
      }
    }
...
bool same_per_query_overlap(const ReferenceCounters& left, const ReferenceCounters& right) {
  return left.valid() && right.valid() &&
      left.per_query_correct == right.per_query_correct;
}
...
StageGateStats stage_gate(const StageResult& stage, const ReferenceCounters& baseline) {
  StageGateStats result;
  if (!baseline.valid() || baseline.per_query_correct.empty() || stage.repetitions.empty()) return result;
...
// This diagnostic is deliberately written before the fail-closed baseline abort.
// It records reference/output counters and first_error without changing the
// Safe-C2 traversal, gamma selection, Recall@10 metric, or admission decision.
void write_baseline_invalid_diagnostic(const std::string& path, const Args& args,
  ...
  const int query_count = static_cast<int>(
      baseline.reference_sum.total / std::max(1, baseline.timed_reps * kK));
  const ReferenceCounters& first =
      baseline.repetitions.empty() ? baseline.reference_sum : baseline.repetitions.front();
  out << std::setprecision(10)
      << "{\n"
      << "  \"schema\": \"safe-c2-tiefree-baseline-invalid-diagnostic-v2\",\n"
      << "  \"status\": \"FAIL_BASELINE_REFERENCE_BEFORE_GAMMA\",\n"
      << "  \"corrected_after_submit\": true,\n"
      << "  \"v2_tiefree\": true,\n"
      << "  \"mode\": \"" << args.mode << "\", \"stage\": \"" << args.stage << "\",\n"
      << "  \"reason\": \"calibration baseline reference check failed\",\n"
      << "  \"no_gamma_candidate_was_issued\": true,\n"
      << "  \"first_timed_repetition_reference\": ";
...
void write_summary(const std::string& path, const Args& args, const BuildTiming& build, const Runtime& runtime,
  ...
  out << std::setprecision(10);
  out << "{\n"
      << "  \"schema\": \"safe-c2-corrected-after-submit-tiefree-run-v2\",\n"
      << "  \"status\": \"" << status << "\",\n"
      << "  \"scope\": \"Safe-C2 v2 tie-free corrected-after-submit static SIFT-1M vector-KNN interval-bound experiment; empirical recall evaluation only\",\n"
      << "  \"implementation_identity\": \"v2 tie-free corrected-after-submit; not the submitted legacy C2 kernel\",\n"
      << "  \"corrected_after_submit\": true,\n"
      << "  \"v2_tiefree\": true,\n"
      << "  \"mode\": \"" << args.mode << "\", \"stage\": \"" << args.stage << "\",\n"
  ...
  out << "},\n  \"interval_certificate\": {\"status\": \"PASS_OUTWARD_SAFE_REPAIRED_AND_REVERIFIED\", \"corrected_after_submit\": true, \"repair_policy\": \"post-build D2H reconstruction then outward H2D replacement; block if post-repair directional verification fails\", \"method\": \"current padded leaf memberships propagated to heap ancestors\", \"node_count\": " << interval_certificate.node_count
      ...
      << ", \"before_interval_hash\": " << interval_certificate.before_interval_hash
      << ", \"after_interval_hash\": " << interval_certificate.after_interval_hash
      ...
      << ", \"per_query_no_regression_gate\": {\"passed\": " << (gate_stats.passed ? "true" : "false")
      ...
      << "  \"timing_comparison\": {\"eligible_for_speed_comparison\": " << (!abba_timed_pair_order.empty() ? "true" : "false")
      << ", \"scheme\": \"" << (!abba_timed_pair_order.empty() ? "heldout ABBA: alternate baseline->Safe-C2 and Safe-C2->baseline after shared warmup" : "calibration correctness-only; not eligible for speed comparison") << "\""
  ...
  if (paired_baseline != nullptr) {
    ...
    write_stage_json(out, *paired_baseline,
                     static_cast<int>(paired_baseline->reference_sum.total / std::max(1, paired_baseline->timed_reps * kK)));
    const double baseline_recall = paired_baseline->reference_sum.total
        ? static_cast<double>(paired_baseline->reference_sum.correct) / paired_baseline->reference_sum.total : 0.0;
    const double c2_recall = result.reference_sum.total
        ? static_cast<double>(result.reference_sum.correct) / result.reference_sum.total : 0.0;
    out << ",\n  \"paired_recall_delta_c2_minus_baseline\": " << (c2_recall - baseline_recall) << ",\n";
  }
  ...
  out << "],\n  \"gpu\": {\"name\": \"" << json_escape(gpu_name) << "\"},\n"
      << "  \"not_established\": [\"universal no-false-negative safety for gamma>1\", \"dynamic update/C3 behavior\", \"drift/recalibration deployment\", \"performance equivalence to the submitted artifact\", \"equivalence to the submitted legacy C2 implementation\"]\n"
      << "}\n";
...
int safe_c2_main(int argc, char** argv) {
  ...
  try {
    ...
    if (args.mode == "calibrate") {
      ...
      const ReferenceCounters calibration_baseline = baseline.repetitions[0];
      ...
      for (int level = runtime.tree_height - 1; level >= kFreezeLevel + 1; --level) {
        ...
        for (int iter = 0; iter < kBinaryIterations; ++iter) {
          ...
          const PerQueryGateStats candidate_gate = per_query_no_regression(c, calibration_baseline);
          const bool accepted = c.valid() && c.correct >= baseline_target && candidate_gate.passed;
          calibration_trace.push_back(CalibrationRecord{level, iter, mid, c.correct, c.valid(),
        ...
        const PerQueryGateStats confirm_gate = per_query_no_regression(c, calibration_baseline);
        const bool confirmed = c.valid() && c.correct >= baseline_target && confirm_gate.passed;
        calibration_trace.push_back(CalibrationRecord{level, kBinaryIterations, gamma[level], c.correct, c.valid(),
                                                      confirm_gate.violating_queries, confirm_gate.min_per_query_delta, confirmed});
      ...
      stage_result = run_stage(runtime, frozen, selected_queries_d, selected_count, base, all_queries, gt,
                               selected_ids, 0, 1);
      gate_stats = stage_gate(stage_result, calibration_baseline);
      passed = gate_stats.passed;
      write_gamma_vector(args.output_dir + "/final_safe_gamma_vector.txt", gamma);
    } else {
      ...
      // Pre-registered paired control for this same heldout split.  It is a
      // fixed all-ones baseline only: its result cannot alter gamma/mu/order.
      const PairedAbbaStageResult paired = run_paired_abba_stage(
          ...
          gamma, args.warmup_reps, args.timed_reps);
      paired_baseline_result = paired.baseline;
      stage_result = paired.safe_c2;
      abba_timed_pair_order = paired.timed_pair_order;
      abba_shared_warmup_pairs = paired.shared_warmup_pairs;
      has_paired_baseline = true;
      if (paired_baseline_result.repetitions.empty()) fail("missing paired ABBA baseline result");
      const ReferenceCounters heldout_baseline = paired_baseline_result.repetitions.front();
      baseline_target = heldout_baseline.correct;
      for (const auto& counters : paired_baseline_result.repetitions) {
        if (!counters.valid() || counters.correct != baseline_target || !same_per_query_overlap(counters, heldout_baseline)) {
          fail("paired ABBA baseline is invalid or per-query nondeterministic; Safe-C2 holdout stage is blocked");
        }
      }
      gate_stats = stage_gate(stage_result, heldout_baseline);
      passed = gate_stats.passed;
    }
    ...
    const std::string status = passed ? (args.mode == "calibrate" ? "PASS_SAFE_C2_CALIBRATION" : "PASS_SAFE_C2_HELDOUT_STAGE")
                                      : (args.mode == "calibrate" ? "FAIL_SAFE_C2_CALIBRATION" : "FAIL_SAFE_C2_HELDOUT_STAGE");
