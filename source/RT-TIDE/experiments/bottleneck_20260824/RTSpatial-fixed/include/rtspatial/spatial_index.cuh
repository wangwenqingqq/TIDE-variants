...
struct Config {
  ...
  // Experiment-only override: 0 keeps LibRTS cost model.
  int fixed_parallelism = 0;
  // Point-range experiment only; general envelope intersections still need both passes.
 ...
 public:
  ...
  void Query(Predicate p, ArrayView<envelope_t> queries, void* arg,
    ...
    case Predicate::kIntersects: {
      ...
      if (!config_.skip_forward) {
        rt_engine_.CopyLaunchParams(cuda_stream, params);
        rt_engine_.Render(cuda_stream, id, dims);
      }
      int parallelism = config_.fixed_parallelism > 0
                            ? config_.fixed_parallelism
                            : CalculateBestParallelism(queries, cuda_stream);
