...
// Optional hook for the isolated Safe-C1 adapter. The legacy top-k traversal
// writes each selected frozen leaf node ID at mergeLNodeKnn. It is null for
// historical callers, so their behavior remains unchanged.
int *safe_c1_leaf_export = nullptr;
int *safe_c1_leaf_export_count = nullptr;
int safe_c1_leaf_export_capacity = 0;
int *safe_c1_leaf_export_overflow = nullptr;
...
__global__ void mergeLNodeKnn(int ls, int le, double *p_list_k, int offset_up_p, int offset_up_n, int qs_up, int offset_p, int *size_list,
							  int cur_level, int qnum_up, int nnum_up, int *safe_c1_leaf_export,
							  int *safe_c1_leaf_export_count, int safe_c1_leaf_export_capacity,
							  int *safe_c1_leaf_export_overflow)
{
	int tid = blockDim.x * blockIdx.x + threadIdx.x;
	int total_num = gridDim.x * blockDim.x;
	int ofst_up = (nnum_up / TREE_ORDER * 3) * qnum_up; // Offset at upper level.
	...
	for (int i = tid; i < qnum_up * nnum_up; i += total_num)
	{
		...
		if (p_list_k[offset_up_p + ofst_up + i] == 1 && qid_p >= ls && qid_p < le)
		{
			...
			if (safe_c1_leaf_export != nullptr && safe_c1_leaf_export_count != nullptr &&
				safe_c1_leaf_export_capacity > 0)
			{
				const int safe_c1_slot = atomicAdd(safe_c1_leaf_export_count, 1);
				if (safe_c1_slot < safe_c1_leaf_export_capacity)
					safe_c1_leaf_export[safe_c1_slot] = nid;
				else if (safe_c1_leaf_export_overflow != nullptr)
					atomicExch(safe_c1_leaf_export_overflow, 1);
			}
		}
	}
}
	...
	while (!st.empty())
	{
		...
		{ // Processing data in leaf node.
			...
			mergeLNodeKnn<<<block_num, THREAD_NUM>>>(ls, le, p_list_k, offset_up_p, offset_up_n, qs_up, offset_p, size_list,
													 cur_level, qnum_up, nnum_up, safe_c1_leaf_export, safe_c1_leaf_export_count, safe_c1_leaf_export_capacity, safe_c1_leaf_export_overflow);
			cudaDeviceSynchronize();
			cudaStatus = cudaGetLastError();
			if (cudaStatus != cudaSuccess)
