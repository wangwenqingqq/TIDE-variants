// Search v4 source-only successor. Speculative gamma traversal with gamma-only fallback diagnostics.
// Derived from immutable Safe-C2 v2 source; v2 remains untouched.
...
#include "aabb_static_bound_v2_diskguard.cuh"

// Isolated submitted-VLDB C2 experiment control.  The archived API otherwise
// derives a 4 GiB workspace from whichever shared GPU is idle, which makes
// timing/occupancy non-reproducible.  This fixed cap applies identically to
// every baseline/candidate/validation/test call; it changes batching only and
// is not a submitted-performance configuration.
#ifndef GTS_C2_QUERY_WORKSPACE_CAP_BYTES
#define GTS_C2_QUERY_WORKSPACE_CAP_BYTES (1ULL * 1024ULL * 1024ULL * 1024ULL)
#endif
using namespace std;
// [新增] 用于收集训练数据的结构体
...
// V4 diagnostic only: a gamma-only prune is a node that the certified
// unscaled Eq. (1) test would keep but gamma-scaled speculative traversal
// would drop. The caller owns these per-local-query arrays. They are never a
// proof that gamma>1 is a metric lower bound.
struct GammaOnlyPruneTrace {
	int nid;
	int level;
	float lb_tri;
	float lb_eff;
	float disk_at_decision;
};

__device__ __forceinline__ void recordGammaOnlyPrune(int qid, int nid, int level,
		float lb_tri, float lb_eff, float disk_at_decision,
		int *gamma_only_pruned, GammaOnlyPruneTrace *gamma_only_trace)
{
	if (gamma_only_pruned != nullptr) atomicExch(&gamma_only_pruned[qid], 1);
	if (gamma_only_trace != nullptr && atomicCAS(&gamma_only_trace[qid].nid, -1, nid) == -1) {
		gamma_only_trace[qid].level = level;
		gamma_only_trace[qid].lb_tri = lb_tri;
		gamma_only_trace[qid].lb_eff = lb_eff;
		gamma_only_trace[qid].disk_at_decision = disk_at_decision;
	}
}
...
// Safe-C2 corrected interval traversal: max_dis_d is the certified upper cover of this node.
__global__ void nodeProcessKnn(TN *node_list, const float *max_dis_d, float *disk, int *empty_list, int qnum_l, int qnum_up, int nnum_l, double *p_list_k,
								   ...
								   int *gamma_only_pruned, GammaOnlyPruneTrace *gamma_only_trace)
{
	...
	for (int i = tid; i < nnum_l * qnum_l; i += total_num)
	{
		...
		if (needs_process)
		{
			...
			// Safe-C2 / submitted Eq. (1): the two-sided interval lower bound.
			// max_dis_d[nid] is certified after build as an outward upper cover.
			const float lower_from_min = node.min_dis - dis_q;
			const float lower_from_max = dis_q - max_dis_d[nid];
			const float dis_lb = fmaxf(0.0f, fmaxf(lower_from_min, lower_from_max));

			// V4: gamma is speculative only. Keep the exact Eq. (1) condition
			// separate, and record every node that gamma alone would discard.
			const float disk_at_decision = disk[qid];
			const int unscaled_keep = (dis_lb <= disk_at_decision) ? 1 : 0;
			const float delta = rp_predict(dis_lb, disk_at_decision, cur_level);
			const float dis_lb_eff = dis_lb + delta;
			const int scaled_keep = (dis_lb_eff <= disk_at_decision) ? 1 : 0;
			if (unscaled_keep && !scaled_keep) {
				recordGammaOnlyPrune(qid, nid, cur_level, dis_lb, dis_lb_eff, disk_at_decision,
								gamma_only_pruned, gamma_only_trace);
			}
			if (scaled_keep) {
					  ...
					  int qnum, float r, int tree_h, int *data_info, int *empty_list, char *data_s, int *size_s)
{
	...
	size_t limit = GTS_C2_QUERY_WORKSPACE_CAP_BYTES; 
	if (avail > limit * 2) {
		avail = limit;
	} else {
		avail = avail / 2;
	...
	while (!st.empty())
	{
		...
		{ // Processing node.
			...
			else
			{
				...
				nodeProcessKnn<<<block_num, THREAD_NUM>>>(node_list, max_dis_d, disk, empty_list, qnum_l, qnum_up, nnum_l, p_list_k, offset_p,
														  offset_up_p, offset_n, qs, qs_up, cur_level, nullptr, nullptr);
				cudaDeviceSynchronize();
				cudaStatus = cudaGetLastError();
				if (cudaStatus != cudaSuccess)
...
void searchIndexKnnV2(short *data_d, TN *node_list, int *id_list, int *max_node_num, float *query_data,
				  int *res_ids, int qnum, int k, int tree_h, int *data_info, int *empty_list, char *data_s, int *size_s)
{
	// V4 fallback baseline path: every CUDA/thrust failure below is fatal.
	cout << "Searching..." << endl;
	...
	// Get GPU available memory. Fail closed rather than allowing an
	// uninitialized workspace size to affect the traversal.
	size_t avail;
	size_t total;
...
// V4 speculative vector+IDs traversal. It records gamma-only pruning but does
// not itself claim correctness; the runner must apply gamma=1 fallback before
// returning final results for every flagged query.
// knn query (return top-k ids and distances) - vector queries
void searchIndexKnnV3Speculative(short *data_d, TN *node_list, int *id_list, int *max_node_num, float *query_data,
				  int *res_ids, int qnum, int k, int tree_h, int *data_info, int *empty_list, char *data_s, int *size_s,
								 int *gamma_only_pruned, GammaOnlyPruneTrace *gamma_only_trace)
{
	// V3 speculative path: every CUDA/thrust failure below is fatal.
	cout << "Searching..." << endl;

	CHECK(cudaMallocManaged((void **)&res_dis, qnum * k * sizeof(float)));
	CHECK(cudaMallocManaged((void **)&size_list, (tree_h + 1) * sizeof(int)));
	CHECK(cudaMalloc((void **)&disk, qnum * sizeof(float)));
	if (gamma_only_pruned != nullptr) CHECK(cudaMemset(gamma_only_pruned, 0, qnum * sizeof(int)));
	if (gamma_only_trace != nullptr) CHECK(cudaMemset(gamma_only_trace, 0xFF, qnum * sizeof(GammaOnlyPruneTrace)));
	...
	while (!st.empty())
	{
		...
		if (cur_level < tree_h)
		{
			...
			else
			{
				...
				block_num = (nnum_l * qnum_l + THREAD_NUM - 1) / THREAD_NUM;
				nodeProcessKnn<<<block_num, THREAD_NUM>>>(node_list, max_dis_d, disk, empty_list, qnum_l, qnum_up, nnum_l, p_list_k, offset_p,
										  offset_up_p, offset_n, qs, qs_up, cur_level, gamma_only_pruned, gamma_only_trace);
				CHECK(cudaDeviceSynchronize());
				CHECK(cudaGetLastError());
			}
		}
...
// Static-only vector top-K entry appended below. It is independent from the
// legacy speculative entry above: its only node prune kernel is nodeProcessKnnStaticAabb.

// Static Safe-C2-AABB v2 disk-guard node decision. It retains only certified radial
// Eq.(1) plus a projected coordinate-box lower bound. There is no gamma,
// diagnostic flag, fallback, update, or insertion path in this kernel.
__global__ void nodeProcessKnnStaticAabb(
    TN *node_list, const float *max_dis_d, float *disk, int *empty_list,
    int qnum_l, int qnum_up, int nnum_l, double *p_list_k,
    int offset_p, int offset_up_p, int offset_n, int qs, int qs_up, int cur_level,
    const float *query_data, StaticProjectedAabbDeviceView aabb_view)
{
  int tid = blockDim.x * blockIdx.x + threadIdx.x;
  int total_num = gridDim.x * blockDim.x;
  for (int i = tid; i < nnum_l * qnum_l; i += total_num) {
    const int qid_p = i % qnum_l;
    const int qid_p_up = qid_p + qs - qs_up;
    const int qid = qid_p + qs;
    const int nid_p = i / qnum_l;
    const int nid_parent_p = nid_p / TREE_ORDER;
    const int nid = nid_p + offset_n;
    const int temp = (nnum_l / TREE_ORDER / TREE_ORDER * 3);
    const int ofst_up = temp * qnum_up;
    const int ofst = (nnum_l / TREE_ORDER * 3) * qnum_l;
    const int idx_p = nid_parent_p * qnum_l + qid_p;
    p_list_k[offset_p + ofst + i] = 0;
    if (p_list_k[offset_up_p + ofst_up + nid_parent_p * qnum_up + qid_p_up] == 1 &&
        empty_list[nid] == 0) {
      const TN node = node_list[nid];
      const float dis_q = p_list_k[offset_p + ofst / 3 + idx_p];
      const float radial_lb = fmaxf(0.0f, fmaxf(node.min_dis - dis_q, dis_q - max_dis_d[nid]));
      const float disk_at_decision = disk[qid];
      bool keep = radial_lb <= disk_at_decision;
      if (keep) {
        const float* query = query_data + static_cast<size_t>(qid) * static_cast<size_t>(aabb_view.full_dimension);
        if (static_aabb_strict_prune(aabb_view, nid, query, disk_at_decision)) keep = false;
      }
      if (keep) p_list_k[offset_p + ofst + i] = 1;
    }
  }
}

void searchIndexKnnStaticAabbV2DiskGuard(short *data_d, TN *node_list, int *id_list, int *max_node_num, float *query_data,
				  int *res_ids, int qnum, int k, int tree_h, int *data_info, int *empty_list, char *data_s, int *size_s, StaticProjectedAabbDeviceView aabb_view)
{
	...
	while (!st.empty())
	{
		...
		if (cur_level < tree_h)
		{
			...
			else
			{
				...
				// Process the nodes of the current layer and determine if the node will be pruned.
				block_num = (nnum_l * qnum_l + THREAD_NUM - 1) / THREAD_NUM;
				nodeProcessKnnStaticAabb<<<block_num, THREAD_NUM>>>(node_list, max_dis_d, disk, empty_list, qnum_l, qnum_up, nnum_l, p_list_k, offset_p,
									  offset_up_p, offset_n, qs, qs_up, cur_level, query_data, aabb_view);
				CHECK(cudaDeviceSynchronize());
				CHECK(cudaGetLastError());
			}
		}
