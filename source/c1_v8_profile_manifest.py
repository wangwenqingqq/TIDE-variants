#!/usr/bin/env python3
"""Atomic C1-v8 profile provenance manifest utility.
...
def root_and_out(args: argparse.Namespace) -> tuple[pathlib.Path, pathlib.Path]:
    root = secure_dir(args.root, "v8 root")
    out = secure_dir(args.out, "profile output")
    if root != pathlib.Path(ROOT_DEFAULT) or out.parent != root / "profiles" or not out.name.startswith("c1_v8_profile_"):
...
def init(args: argparse.Namespace) -> None:
    ...
    if launcher != root / "launch_c1_profile_v8.sh" or guard != root / "run_c1_profile_guard_v8.sh":
        raise SystemExit("canonical profile trust path required")
    pin = load(pins, "pins")
    entries: list[dict[str, Any]] = []
    flatten(pin, entries)
    if pin.get("schema") != "gtspp-c1-v8-pins-v1" or not entries:
