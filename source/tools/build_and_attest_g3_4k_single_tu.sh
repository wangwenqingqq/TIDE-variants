#!/bin/sh
# The only supported entry point for controlled single-TU link and attestation.
set -eu
ROOT="/workspace/experiments/tide_safe_c1_20260727/safe_c1_native_matrix_g3_v5_failstop_boundinput_pilot"
BODY="$ROOT/tools/.build_and_attest_g3_4k_single_tu.body.sh"
[ -f "$BODY" ] && [ ! -L "$BODY" ] || {
  echo "missing or unsafe controlled build body" >&2
  exit 69
}
