#!/usr/bin/env python3
"""Read-only Fable5 matched-trace preflight run before any NVML action.
...
CONTRACT_NAME = "fable5_matched_trace_preflight_v1.json"
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        "schema": "fable5-matched-trace-preflight-report-v1",
        "status": "PASS_FABLE5_MATCHED_TRACE_STATIC_PREFLIGHT",
        "execution_boundary": {"gpu_used": False, "cuda_binary_executed": False, "nvml_queried": False},
        "scope": "Read-only ABI/layout/identity/event-sequence gate before run-directory creation and NVML; not a native certificate or GPU correctness result.",
        "root": str(root),
        "bundle": str(bundle),
        "trace": str(trace),
...
def main() -> int:
    ...
    try:
        ...
        report = {
            "schema": "fable5-matched-trace-preflight-report-v1",
            "status": "FAIL_FABLE5_MATCHED_TRACE_STATIC_PREFLIGHT",
            "execution_boundary": {"gpu_used": False, "cuda_binary_executed": False, "nvml_queried": False},
    ...
    return 0 if report["status"] == "PASS_FABLE5_MATCHED_TRACE_STATIC_PREFLIGHT" else 2
