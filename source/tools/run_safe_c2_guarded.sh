#!/usr/bin/env bash
# Fail-closed launcher for corrected-after-submit Safe-C2 only.
...
BINARY="$ROOT/bin/GTS_safe_c2_interval_sift1m"
...
AUDIT="$ROOT/tools/audit_safe_c2_static.py"
...
LOCK="$ROOT/.locks/safe_c2_physical_gpu0.lock"
...
Usage:
  ...
  SAFE_C2_GPU0_APPROVED=I_CONFIRM_PHYSICAL_GPU0_IS_EXCLUSIVE \
...
It never kills, restarts, or otherwise touches any pre-existing process. Safe-C2 is
corrected-after-submit and must never be labeled as a run of submitted legacy C2.
USAGE
}

die() {
  echo "Safe-C2 GPU0 guard: $*" >&2
  exit 64
}
...
assert p['schema'] == 'safe-c2-corrected-after-submit-protocol-v1'
...
assert q['schema'] == 'safe-c2-corrected-after-submit-preflight-v1'
s = source.read_text()
...
print('Safe-C2 corrected-after-submit static contract: PASS')
PY
  } >"$out" 2>&1
}

# This hashes all immutable raw inputs before any GPU query or lock acquisition.
# It requires both frozen JSON artifacts to agree with each other and the actual files.
verify_frozen_data_hashes() {
  local out=$1
  "$PYTHON" - "$PROTOCOL" "$PREFLIGHT" "$BASE" "$QUERY" "$GT" "$out" <<'PY'
import hashlib
import json
import pathlib
import sys
...
protocol_path, preflight_path, base_path, query_path, gt_path, out_path = map(pathlib.Path, sys.argv[1:])
...
need(protocol.get('schema') == 'safe-c2-corrected-after-submit-protocol-v1', 'wrong protocol schema')
need(preflight.get('schema') == 'safe-c2-corrected-after-submit-preflight-v1', 'wrong preflight schema')
actual_preflight_sha = digest(preflight_path)
need(protocol.get('data', {}).get('preflight_path') == str(preflight_path), 'protocol preflight path mismatch')
need(protocol.get('data', {}).get('preflight_sha256') == actual_preflight_sha, 'protocol preflight SHA mismatch')

rows = []
for label, protocol_key, preflight_key, supplied in (
    ('base', 'base', 'base', base_path),
    ('query', 'query', 'query', query_path),
    ('groundtruth', 'groundtruth', 'groundtruth', gt_path),
):
    p = protocol.get('data', {}).get(protocol_key, {})
    q = preflight.get('inputs', {}).get(preflight_key, {})
    need(supplied.is_file(), f'{label} raw input missing: {supplied}')
    need(p.get('path') == str(supplied), f'{label} protocol path mismatch')
    need(q.get('path') == str(supplied), f'{label} preflight path mismatch')
    p_sha, q_sha = p.get('sha256'), q.get('sha256')
    need(isinstance(p_sha, str) and len(p_sha) == 64, f'{label} invalid protocol SHA')
    need(isinstance(q_sha, str) and len(q_sha) == 64, f'{label} invalid preflight SHA')
    need(p_sha == q_sha, f'{label} protocol/preflight SHA disagreement')
    actual = digest(supplied) if supplied.is_file() else ''
    need(actual == p_sha == q_sha, f'{label} actual SHA mismatch')
    rows.append((label, supplied, p_sha, q_sha, actual, supplied.stat().st_size if supplied.is_file() else -1))

if errors:
    raise SystemExit('Safe-C2 frozen data hash verification FAIL:\n - ' + '\n - '.join(errors))

lines = [
    'data_hash_verification=PASS',
    'verified_utc=' + datetime.now(timezone.utc).isoformat(),
    'protocol_path=' + str(protocol_path),
    'protocol_sha256_actual=' + digest(protocol_path),
    'preflight_path=' + str(preflight_path),
    'preflight_sha256_protocol_expected=' + protocol['data']['preflight_sha256'],
    'preflight_sha256_actual=' + actual_preflight_sha,
]
for label, supplied, p_sha, q_sha, actual, nbytes in rows:
    lines.extend([
        f'{label}_path={supplied}',
        f'{label}_protocol_sha256_expected={p_sha}',
        f'{label}_preflight_sha256_expected={q_sha}',
        f'{label}_actual_sha256={actual}',
        f'{label}_actual_bytes={nbytes}',
    ])
out_path.write_text('\n'.join(lines) + '\n')
print('Safe-C2 frozen data hash verification PASS:', out_path)
PY
}

resolve_physical_gpu0_uuid() {
  local raw line index uuid extra
  if ! raw="$(nvidia-smi --id=0 --query-gpu=index,uuid --format=csv,noheader,nounits 2>&1)"; then
...
# Every pre-launch GPU inspection is strict: one valid row, no active compute
# application, zero utilization, and memory usage within the fixed tolerance.
gpu0_snapshot_and_require_idle() {
  local label=$1 dir=$2
  local status status_line apps apps_clean
  local index uuid total mem util extra
  if ! status="$(nvidia-smi --id=0 --query-gpu=index,uuid,memory.total,memory.used,utilization.gpu --format=csv,noheader,nounits 2>&1)"; then
  ...
  if ! apps="$(nvidia-smi --id=0 --query-compute-apps=pid,process_name,used_memory --format=csv,noheader,nounits 2>&1)"; then
...
assert d['schema'] == 'safe-c2-corrected-after-submit-run-v1'
...
terminate_child_group() {
  ...
  if [[ -n "$CHILD_PGID" ]]; then
    ...
    case "$state" in
      verified)
        echo "Safe-C2 GPU0 guard: terminating only Safe-C2 child process group $CHILD_PGID" >&2
      ...
      empty)
        ;;
      *)
        echo "Safe-C2 GPU0 guard: refusing to signal unverifiable process group $CHILD_PGID; lock will be retained if it remains live" >&2
  ...
  if [[ -n "$CHILD_LAUNCHER_PID" ]]; then
    if ! wait_for_launcher_exit "$CHILD_LAUNCHER_PID"; then
      ...
      if ! wait_for_launcher_exit "$CHILD_LAUNCHER_PID"; then
        echo "Safe-C2 GPU0 guard: setsid launcher did not exit; lock will be retained" >&2
        CHILD_CLEANUP_FAILED=1
      fi
    fi
  fi
  ...
  if [[ -n "$CHILD_PGID" && "$(isolated_group_state "$CHILD_PGID")" != empty ]]; then
    echo "Safe-C2 GPU0 guard: child process group $CHILD_PGID remains after TERM/KILL/reap; lock will be retained" >&2
    CHILD_CLEANUP_FAILED=1
  fi
...
release_lock() {
  if [[ "$LOCK_HELD" -eq 1 ]]; then
    rmdir "$LOCK" 2>/dev/null || {
      echo "Safe-C2 GPU0 guard: unable to remove owned lock $LOCK; retaining it" >&2
      return 1
    }
    LOCK_HELD=0
  fi
...
cleanup_on_exit() {
  ...
  if [[ "$CHILD_CLEANUP_FAILED" -eq 0 ]]; then
    ...
    echo "Safe-C2 GPU0 guard: retaining $LOCK because child session could not be proven reaped" >&2
  fi
  exit "$rc"
}
...
on_signal() {
  local sig=$1 rc=$2
  echo "Safe-C2 GPU0 guard: received $sig; aborting/reaping only its own child session before lock release" >&2
  exit "$rc"
}
...
run_phase() {
  ...
  mkdir "$phase_dir"
  # Last safety inspection after lock acquisition and immediately before this
  # exact CUDA workload authorization. No pre-existing process is ever touched.
  ...
  # This post-phase record is also strict; a nonidle/malformed state blocks
  # any next phase and fails the sequence rather than silently continuing.
  gpu0_snapshot_and_require_idle "after_${phase}" "$phase_dir"
  return "$rc"
}
...
case "$1" in
  --verify-static)
    ...
    echo 'Safe-C2 CPU-only static verification PASS; no GPU query or CUDA binary execution.'
    exit 0 ;;
  --execute) ;;
  *) usage; exit 64 ;;
esac

[[ -v SAFE_C2_GPU0_APPROVED && "$SAFE_C2_GPU0_APPROVED" == "I_CONFIRM_PHYSICAL_GPU0_IS_EXCLUSIVE" ]] || die 'set SAFE_C2_GPU0_APPROVED=I_CONFIRM_PHYSICAL_GPU0_IS_EXCLUSIVE after shared-GPU review'
[[ "$(hostname)" == "$HOST_REQUIRED" ]] || die "must run on 8p host $HOST_REQUIRED"
...
[[ -x "$BINARY" && -r "$SRC" && -r "$PROTOCOL" && -r "$PREFLIGHT" && -r "$SOURCE_MANIFEST" ]] || die 'missing Safe-C2 artifact'
...
verify_frozen_data_hashes "$PRE/data_hash_verification.env"

resolve_physical_gpu0_uuid
gpu0_snapshot_and_require_idle before_lock "$PRE"
mkdir -p "$(dirname "$LOCK")"
if ! mkdir "$LOCK" 2>/dev/null; then
  die "Safe-C2 physical-GPU0 lock exists: $LOCK"
fi
LOCK_HELD=1
...
RUN_ID="safe_c2_sift1m_heldout_v1_$(date -u +%Y%m%dT%H%M%SZ)"
...
cp "$PRE"/gpu0_*_before_lock* "$OUT/" 2>/dev/null || true
{
  ...
  echo "host=$(hostname)"
  echo 'implementation_identity=Safe-C2 corrected-after-submit; not submitted legacy C2'
  echo 'physical_gpu_index=0'
  echo "physical_gpu_uuid=$GPU0_UUID"
  ...
  echo 'sequence=calibration_correctness_only_then_validation_ABBA_if_pass_then_sealed_test_ABBA_if_validation_pass'
  echo 'gpu0_idle_policy=one_valid_status_row;compute_apps_explicitly_empty;utilization_percent=0;memory_used_mib<=256;checked_before_lock_and_immediately_before_each_phase'
...
gpu0_snapshot_and_require_idle after_all "$OUT"
if [[ "$CAL_RC" == 0 && "$VAL_RC" == 0 && "$TEST_RC" == 0 ]]; then
  echo "Safe-C2 full held-out sequence PASS: $OUT"
  exit 0
fi
if [[ "$CAL_RC" != 0 ]]; then
  echo "Safe-C2 stopped after calibration (no held-out evaluation): $OUT" >&2
elif [[ "$VAL_RC" != 0 ]]; then
  echo "Safe-C2 validation did not pass; sealed test was not run: $OUT" >&2
else
  echo "Safe-C2 sealed test did not pass: $OUT" >&2
fi
exit 3
