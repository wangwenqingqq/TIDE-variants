#!/usr/bin/python3
"""Single-GPU, NVML-guarded launch for a native-GTS candidate-recall probe.

This launcher is intentionally not a performance harness or correctness proof.
It admits physical GPU 1, executes one scoped probe, then runs an independent
CPU validator for overlap metrics.  It never calls nvidia-smi, kills processes,
...
RUNS = ROOT / "runs"
BIN = ROOT / "bin/fair_safe_c1_native_recall_e1_runner.sm80"
RUNNER = ROOT / "runner/fair_safe_c1_native_recall_e1_runner.cu"
BUILD_RECEIPT = ROOT / "build/fair_safe_c1_native_recall_e1_runner.sm80.build.env"
OBJECT = ROOT / "build/fair_safe_c1_native_recall_e1_runner.sm_80.o"
COMPILE_LOG = ROOT / "build/compile_native_recall_sm80_v1_20260729T1225Z.log"
LINK_LOG = ROOT / "build/link_native_recall_sm80_v1_20260729T1225Z.log"
HELP_LOG = ROOT / "build/help_native_recall_sm80_v1_20260729T1225Z.log"
COMPILE_HELPER = ROOT / "tools/compile_native_recall.sh"
...
VALIDATOR = ROOT / "tools/validate_native_recall_output.py"
...
APPROVED_GRANT = (
    "safe-c1-native-recall-e1-v1-20260729",
    "archive_user-20260729-native-recall-e1-05",
    120,
)
...
def preflight_static(gpu: int, run_name: str, approval_id: str, ttl: int) -> dict[str, str]:
    ...
    expected_receipt = {
        "schema": "fair-safe-c1-native-recall-build-v1",
        "arch": "sm_80",
...
def main() -> int:
    ...
    card: dict[str, Any] = {
        "schema": "fair-safe-c1-native-recall-guard-v1",
        "status": "ADMITTED_NOT_STARTED",
        "scope": (
            "quality probe only: each projected KNN uses the archived receipt-selected "
            "native GTS candidate path plus exact global delta; no exactness/performance/" 
            "direct-sidecar/delete/projected-range/rebuild claim"
        ),
    ...
    return 0 if card["status"] == "PASS_RECALL_PROBE" else 2
