...
def main():
    ...
    if changed_slots != [46230] or initial["id_slots"][46230] != -1 or final["id_slots"][46230] != CANDIDATE:
    ...
    if changed_max != {MAX_NODE}:
        fail("max whitelist")
    if by_initial[MAX_NODE]["max_dis_bits"] != 1194013565 or by_final[MAX_NODE]["max_dis_bits"] != 1194016574:
    ...
    if [record.get("record") for record in records] != [
        "meta", "plan", "postcommit_d2h_audit", "native_knn_4099", "exact_full_live_range_fallback"]:
        fail("engine record order")
    meta, plan, audit, knn, range_record = records
    if meta["candidate_id"] != CANDIDATE or plan["route_nodes"] != ROUTE or plan["write_slot"] != 46230:
        fail("engine plan binding")
    if plan["expected_sizes_before"] != [4096, 409, 40, 4] or plan["expected_sizes_after"] != [4097, 410, 41, 5]:
        fail("engine size plan")
    if plan["max_update_nodes"] != [78]:
