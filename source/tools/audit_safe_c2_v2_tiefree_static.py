...
def main() -> int:
    ...
    require(protocol.get('split', {}).get('selection_audit_path') == protocol.get('tie_free_v2', {}).get('selection_audit_path'), 'protocol split/tie-free selection-audit path alias mismatch', failures)
    require(protocol.get('split', {}).get('selection_audit_sha256') == protocol.get('tie_free_v2', {}).get('selection_audit_sha256'), 'protocol split/tie-free selection-audit SHA alias mismatch', failures)
    require(protocol.get('tie_free_v2', {}).get('selection_audit_path') == preflight.get('tie_free_v2', {}).get('selection_audit_path'), 'protocol/preflight selection-audit path mismatch', failures)
    require(protocol.get('tie_free_v2', {}).get('selection_audit_sha256') == preflight.get('tie_free_v2', {}).get('selection_audit_sha256'), 'protocol/preflight selection-audit SHA mismatch', failures)
    require(protocol.get('tie_free_v2', {}).get('fp32_selected_checks') == preflight.get('tie_free_v2', {}).get('fp32_selected_checks'), 'protocol/preflight fp32 admission statistics mismatch', failures)
    ...
    require(protocol.get('resource_policy', {}).get('write_boundary') == str(ROOT), 'v2 protocol write boundary does not equal v2 root', failures)
    ...
    require(split.get('selected_evaluation_exhaustive_over_original_10000') is False, 'v2 selected-evaluation coverage field is wrong', failures)
    require(split.get('selected_plus_excluded_partition_original_10000') is True, 'v2 selected+excluded partition field is wrong', failures)
    require(preflight.get('split', {}).get('selected_plus_excluded_partition_original_10000') is True, 'preflight does not bind selected+excluded v1 partition', failures)
    ...
    for required in (
        ...
        'volatile float sum = 0.0F;', 'sum = sum + term;', 'source-level fp32 recurrence',
    ):
    ...
    if not failures:
        ...
        if VERIFY_OUT.is_file():
            ...
            for part, expected_max_d2 in {'calibration': 128166, 'validation': 133413, 'test': 150288}.items():
                c = v.get('fp32_selected_checks', {}).get(part, {})
                require(c.get('fp32_boundary_ties') == 0, f'{part} selected fp32 boundary ties are nonzero', failures)
                require(c.get('fp32_stored_top10_mismatches') == 0, f'{part} selected fp32 top10 mismatches are nonzero', failures)
                require(c.get('fp32_equal_distance_distinct_d2_pairs') == 0, f'{part} selected fp32 distance collapse is nonzero', failures)
                require(c.get('max_d2') == expected_max_d2, f'{part} selected fp32 max_d2 changed', failures)
    ...
    payload = {
        ...
        'checks': {
            ...
            'write_boundary': str(ROOT),
            'fp32_admission': 'selected IDs have zero fp32 boundary ties, stored-top10 mismatches, and distinct-d2 distance collapses',
