#!/usr/bin/python3.12
"""Pure-stdlib independent validator for the native-GTS candidate-recall probe.

It replays the sealed frozen-base/insertion-only/KNN-only projection with an
exact integer oracle, independently recomputes candidate overlap metrics, and
does not treat the candidate path as an exactness or timing result.
"""
...
def validate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for expected_event, record in zip(events, records):
        ...
        candidate_count = record.get("native_base_candidate_count")
        visited_leaf_count = record.get("native_visited_leaf_count")
        require(isinstance(record.get("native_base_candidate_path"), str) and
                bool(record["native_base_candidate_path"]) and
                isinstance(candidate_count, int) and not isinstance(candidate_count, bool) and
                header["k"] <= candidate_count <= header["base_n"] and
                isinstance(visited_leaf_count, int) and not isinstance(visited_leaf_count, bool) and
                visited_leaf_count >= 0 and
                record.get("native_gts_candidate_path_used_for_trace") is True and
                record.get("typed_exact_full_immutable_base_fallback_used_for_trace") is False,
                f"KNN native candidate declaration mismatch at {op_index}")
        ...
        actual_base = parse_rows(record.get("base_candidate_topk"), f"base_candidate_topk event {op_index}", header["k"])
        ...
        expected_full = canonical_topk(pool, mapping, queries, argument, active,
                                       header["dimension"], header["k"])
        base_overlap = len({row[0] for row in actual_base} & {row[0] for row in expected_base})
        merged_overlap = len({row[0] for row in actual_merged} & {row[0] for row in expected_full})
        require(record.get("base_overlap_count") == base_overlap and
                record.get("merged_overlap_count") == merged_overlap and
                record.get("base_exact_match") is (actual_base == expected_base) and
                record.get("merged_exact_match") is (actual_merged == expected_full),
                f"candidate recall metrics mismatch at {op_index}")
        sequence.update(f"{op_index}:{argument}:{active_hash(active)}:".encode("ascii"))
        for label, rows in (("candidate_base", actual_base), ("candidate_merged", actual_merged),
        ...
        sequence.update(b"\n")
        base_exact_match_count += int(actual_base == expected_base)
        merged_exact_match_count += int(actual_merged == expected_full)
    ...
    require(summary.get("schema") == "fair-safe-c1-native-candidate-recall-probe-v1" and
            summary.get("mode") == "recall_probe" and summary.get("status") == "PASS_PROBE",
    ...
    require(summary.get("base_immutable") is True and
            summary.get("native_gts_candidate_path_used_for_trace") is True and
            summary.get("typed_exact_full_immutable_base_fallback_used_for_trace") is False and
    ...
    return {
        "schema": "fair-safe-c1-native-candidate-recall-output-validator-v1",
        "status": "PASS",
        "bundle": str(bundle),
        ...
        "oracle": "pure-stdlib exact integer squared L2; canonical order=(distance_sq,stable_id); independently checks native-candidate overlap metrics",
    }
