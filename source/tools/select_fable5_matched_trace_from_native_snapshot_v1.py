...
def canonical_preflight_contract(path: Path) -> Path:
    """Allow only an existing direct-child contract under this isolated root."""
    resolved = path.resolve(strict=True)
    manifests = (ROOT / "manifests").resolve(strict=True)
    require(resolved.parent == manifests and resolved.suffix == ".json", "preflight_contract_must_be_direct_manifest_json")
    return resolved


def load_preflight_contract(source_bundle: Path, header: dict[str, Any],
                            contract_path: Path = PREFLIGHT_CONTRACT_PATH) -> dict[str, Any]:
    """Load and byte-pin the exact pre-NVML contract without importing its tool."""
    contract_path = canonical_preflight_contract(contract_path)
    try:
...
def candidate_contract(contract: dict[str, Any], slots: dict[str, int], trace_sha256: str,
                       contract_path: Path = PREFLIGHT_CONTRACT_PATH) -> dict[str, Any]:
    contract_path = canonical_preflight_contract(contract_path)
    ...
    candidate = {
        ...
        "preflight_contract": {"relative_path": contract_path.relative_to(ROOT).as_posix(), "sha256": sha256_file(contract_path)},
...
def main() -> int:
    ...
    parser.add_argument("--preflight-contract", type=Path, default=PREFLIGHT_CONTRACT_PATH,
                        help="direct-child manifest contract; default is the original G2 contract")
    ...
    try:
        ...
        contract = load_preflight_contract(args.bundle, header, args.preflight_contract)
        ...
        candidate = candidate_contract(contract, slots, trace_sha, args.preflight_contract)
