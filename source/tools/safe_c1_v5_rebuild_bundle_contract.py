#!/usr/bin/env python3
"""CPU-only contract helpers for the isolated Safe-C1 G2 rebuild witness v5.
...
SCHEMA = "safe-c1-g2-rebuild-witness-contract-v5"
ENGINE_SCHEMA = "safe-c1-g2-rebuild-witness-v5-search-native"
CURRENT_RUNNER_STABLE_ID_LAYOUT_MODE = "identity_full_immutable_pool_seeded_stable_ids_v5"
...
def load_identity_layout(bundle: Path, header: TraceHeader) -> tuple[list[int], list[int]]:
    ...
    if mapping != list(range(header.pool_n)):
        raise BundleContractError("stable_id_to_pool_row_not_identity_for_v5")
    if initial != list(range(header.base_n)):
        raise BundleContractError("initial_base_not_identity_prefix_for_v5")
