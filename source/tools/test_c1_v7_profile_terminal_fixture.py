#!/usr/bin/env python3
"""CPU-only midpoint-failure terminal-state fixture for the C1-v7 profile guard.
...
CARD_NAME = "profile_guard_card_v7.json"
...
def main() -> int:
    ...
    root = args.root
    need(root.is_absolute() and root.is_dir() and not root.is_symlink() and root.resolve(strict=True) == ROOT_DEFAULT, "canonical v7 root required")
    guard = root / GUARD_NAME
    need(guard.is_file() and not guard.is_symlink(), "profile guard missing")
    with tempfile.TemporaryDirectory(prefix="c1_v7_profile_terminal_fixture_") as temporary:
        out = pathlib.Path(temporary) / "c1_v7_profile_fixture_terminal"
        out.mkdir()
        ...
        need(not (out / ".c1_profile_guard_session_v7.json").exists(), "midpoint fixture must not create GPU session marker")
