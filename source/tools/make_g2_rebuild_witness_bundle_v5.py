...
script copies only immutable input bytes into a new v5 bundle; it never copies a
v2/v3 engine result as v5 evidence and never invokes CUDA.
"""
...
def assert_canonical_root(root: Path) -> None:
    if root.resolve() != ROOT.resolve():
        fail('root_must_equal_canonical_v5_root')
...
def choose_base_delete(bundle: Path, g1b_contract: dict[str, str]) -> tuple[int, dict[str, Any]]:
    # Reads only v5-local immutable bytes after they were copied, so this
    ...
    # all population fields/payloads are copied, while v5 writes a new trace.
    import struct
...
def main() -> int:
    ...
    try:
        ...
        raise SystemExit('out_must_be_under_new_v5_bundles_root') from exc
    if out.exists():
    ...
    try:
        ...
        # remain external read-only provenance and are never copied as v5 evidence.
        ...
        # Need the original v3 header fields to create v5's new trace.  Parse
        # the known packed ABI without importing a v3 module into v5.
        import struct
        ...
        spec = {
            'schema': 'safe-c1-g2-rebuild-witness-v5-bundle-spec',
        ...
        manifest = {
            'schema': 'safe-c1-g2-rebuild-witness-v5-bundle-manifest',
            ...
            'engine_schema_required': 'safe-c1-g2-rebuild-witness-v5-search-native',
            ...
            'external_v2_selection': {
                ...
                'copied_into_v5_bundle': False,
                'role': 'read-only selection provenance only; never a v5 result'},
            'external_v3_immutable_payload_source': {
                'path': str(V3_BUNDLE), 'role': 'immutable input bytes only; no v3 runner output copied as v5 evidence',
