#!/usr/bin/env python3
"""CPU-only executable fixtures for the C1-v7 profile artifact manifest.
...
UTILITY_NAME = "c1_v7_profile_manifest.py"
...
def secure_root(value: pathlib.Path) -> pathlib.Path:
    ...
    need(resolved == value == ROOT_DEFAULT, "fixture root must be canonical v7 root")
...
def make_output(parent: pathlib.Path, variant: str, tag: str, *, empty_um: bool, valid_empty_um_witness: bool, empty_non_um: bool = False) -> tuple[pathlib.Path, list[pathlib.Path], pathlib.Path]:
    out = parent / ("c1_v7_profile_fixture_" + variant + "_" + tag)
    out.mkdir()
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="c1_v7_profile_manifest_fixture_") as temporary:
