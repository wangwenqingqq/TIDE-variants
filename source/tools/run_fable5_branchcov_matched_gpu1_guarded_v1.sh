...
# The signed branch manifest binds this guard to the constructed micro-witness,
# exact candidate bundle, trace, contract, runner, and CPU-only build receipt.
# This is read-only and precedes trace preflight, run-directory creation, and NVML.
MANIFEST_PRECHECK_JSON=''
if ! MANIFEST_PRECHECK_JSON="$("$PYTHON_BIN" - "$ROOT" "$MANIFEST" "$BUNDLE" "$TRACE" "$TRACE_SHA256" "$CANDIDATE_CONTRACT" "$CANDIDATE_CONTRACT_SHA256" "$BINARY" "$BINARY_SHA256" "$SOURCE_SHA256" "$TRACE_PREFLIGHT_CONTRACT" <<'PY'
import hashlib,json,sys
from pathlib import Path
root=Path(sys.argv[1]).resolve(strict=True); manifest=Path(sys.argv[2]).resolve(strict=True); bundle=Path(sys.argv[3]).resolve(strict=True); trace=Path(sys.argv[4]).resolve(strict=True); trace_sha=sys.argv[5]; candidate=Path(sys.argv[6]).resolve(strict=True); candidate_sha=sys.argv[7]; binary=Path(sys.argv[8]).resolve(strict=True); binary_sha=sys.argv[9]; source_sha=sys.argv[10]; contract=Path(sys.argv[11]).resolve(strict=True)
def sha(p):
 h=hashlib.sha256()
 with p.open('rb') as f:
 ...
 return h.hexdigest()
def under(relative):
 if not isinstance(relative,str) or Path(relative).is_absolute(): raise SystemExit('manifest_relative_path_invalid')
 p=(root/relative).resolve(strict=True)
 try:p.relative_to(root)
 except ValueError:raise SystemExit('manifest_path_escapes_root')
 return p
m=json.load(open(manifest))
if m.get('schema')!='fable5-branchcov-matched-gpu-runner-v1' or m.get('status')!='CPU_BUILT_PRE_GPU': raise SystemExit('wrong_branch_manifest_schema_or_status')
if m.get('execution_boundary')!={'compiled':True,'cuda_binary_executed':False,'gpu_used':False,'nvml_queried':False}: raise SystemExit('branch_manifest_execution_boundary_invalid')
v=m.get('variant',{})
if v.get('id')!='constructed_branch_coverage_micro_witness_v1' or 'performance' not in v.get('prohibited_claims',[]): raise SystemExit('branch_manifest_variant_invalid')
r=m.get('runner',{})
if under(r.get('source'))!=root/'src/fable5_matched_gpu_runner_v1.cu' or r.get('source_sha256')!=source_sha: raise SystemExit('branch_manifest_runner_source_mismatch')
if under(r.get('binary'))!=binary or r.get('binary_sha256')!=binary_sha or sha(binary)!=binary_sha: raise SystemExit('branch_manifest_runner_binary_mismatch')
receipt=under(r.get('cpu_build_receipt'))
if r.get('cpu_build_receipt_sha256')!=sha(receipt): raise SystemExit('branch_manifest_build_receipt_mismatch')
rd=json.load(open(receipt))
if rd.get('status')!='PASS_CPU_BUILD_ONLY_NOT_EXECUTED' or rd.get('binary_sha256')!=binary_sha or rd.get('source_sha256')!=source_sha: raise SystemExit('branch_manifest_build_receipt_semantics_mismatch')
c=m.get('candidate',{})
if under(c.get('bundle'))!=bundle or under(c.get('trace'))!=trace or c.get('trace_sha256')!=trace_sha or sha(trace)!=trace_sha: raise SystemExit('branch_manifest_trace_mismatch')
if under(c.get('candidate_contract'))!=candidate or c.get('candidate_contract_sha256')!=candidate_sha or sha(candidate)!=candidate_sha: raise SystemExit('branch_manifest_candidate_contract_mismatch')
if under(c.get('branch_preflight_contract'))!=contract or c.get('branch_preflight_contract_sha256')!=sha(contract): raise SystemExit('branch_manifest_preflight_contract_mismatch')
print(json.dumps({'status':'PASS_BRANCHCOV_MANIFEST_PRECHECK','variant':v['id'],'bundle':str(bundle),'trace_sha256':trace_sha,'candidate_contract_sha256':candidate_sha},sort_keys=True))
PY
)"; then
  printf '%s\n' "$MANIFEST_PRECHECK_JSON" >&2
  fail 'branch manifest precheck failed before trace preflight/run-directory/NVML'
fi
