...
// Isolated Safe-C1 traversal receipt. This copied header records only leaf pairs
// emitted by the actual GTS KNN traversal; it does not route sidecars itself.
			...
			if (lnum > 0) {
				...
				for (int receipt_index = 0; receipt_index < lnum; ++receipt_index) {
					...
					if (leaf_id < 0 || query_id < 0 || query_id >= qnum) {
						fprintf(stderr, "Safe-C1 invalid GTS leaf receipt: q=%d leaf=%d\n", query_id, leaf_id);
						exit(1);
					}
