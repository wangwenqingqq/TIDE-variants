// Safe-C3 single-candidate ancestor/fence correctness harness.
//
...
// narrow: one preloaded inactive raw-pool object (stable ID 4099), a strictly
...
constexpr int kSafeCandidateStableId = 4099;
constexpr int kSafeExpectedLeaf = 790;
constexpr std::array<int, 4> kSafeExpectedRoute{{0, 7, 78, 790}};
...
SafeArgs parse_safe_args(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (key == "--bundle") args.bundle = value();
      ...
      std::cout
          ...
          << "--final-snapshot FINAL.json --candidate-id 4099 --expected-leaf 790\n";
      std::exit(0);
    } else {
  ...
  if (args.candidate_id != kSafeCandidateStableId ||
    ...
    fail("this preregistered harness admits only candidate=4099 and leaf=790");
  }
  return args;
}
...
void write_atomic_no_overwrite(const std::string& path, const std::string& body) {
  ...
  if (fd < 0) fail("cannot create output staging file " + temp + ": " + std::strerror(errno));
  bool staged = false;
  try {
    std::size_t written = 0;
    while (written < body.size()) {
      const ssize_t n = ::write(fd, body.data() + written, body.size() - written);
      if (n <= 0) fail("cannot write output staging file " + temp);
      written += static_cast<std::size_t>(n);
    }
    if (::fsync(fd) != 0) fail("cannot fsync output staging file " + temp);
    if (::close(fd) != 0) fail("cannot close output staging file " + temp);
    staged = true;
    // link() publishes only if the destination is absent, unlike rename().
    if (::link(temp.c_str(), path.c_str()) != 0) {
      fail("cannot atomically publish output (destination may exist) " + path);
    }
    if (::unlink(temp.c_str()) != 0) fail("cannot remove output staging file " + temp);
    const std::string parent = dirname_of(path);
    const int dfd = ::open(parent.c_str(), O_RDONLY | O_DIRECTORY);
    if (dfd < 0) fail("cannot open output parent for fsync " + parent);
    if (::fsync(dfd) != 0) {
      ::close(dfd);
      fail("cannot fsync output parent " + parent);
    }
    ::close(dfd);
  } catch (...) {
    if (!staged) ::close(fd);
...
C3Snapshot capture_c3_snapshot(const Runtime& runtime, std::size_t pool_values) {
  ...
  void* base = nullptr;
  std::size_t bytes = 0;
  C3_CUDA(cudaMemGetAddressRange(&base, &bytes, runtime.id_list));
  if (base != runtime.id_list || bytes == 0 || bytes % sizeof(int) != 0) {
...
SafeC3Plan prepare_safe_c3_plan(const C3Snapshot& initial,
  ...
  if (plan.leaf_id != expected_leaf ||
    ...
    fail("frozen route differs from preregistered 0->7->78->790 contract");
  }
  ...
  if (plan.max_nodes.size() != 1 || plan.max_nodes[0] != 78 ||
      float_bits(plan.max_before[0]) != 1194013565U ||
      float_bits(plan.max_after[0]) != 1194016574U ||
      plan.write_slot != 46230 || plan.physical_span_end != 46294 ||
      plan.expected_sizes != std::vector<int>({4096, 409, 40, 4})) {
    fail("precomputed Safe-C3 4099 mutation contract drifted");
  }
  return plan;
}
...
D2HAudit audit_postcommit(const C3Snapshot& before, const C3Snapshot& after,
  ...
  audit.updated_fence_margin =
      after.logical.nodes[79].min_dis - after.logical.max_distance[78];
...
void emit_native_knn(std::ostream& out, int candidate_id, int active_count_value,
  ...
  out << "{\"record\":\"native_knn_4099\",\"query_stable_id\":" << candidate_id
...
std::string pass_summary_json(const TraceHeader& header, const SafeC3Plan& plan,
  ...
  std::ostringstream out;
  out << "{\"schema\":\"safe-c3-ancestor-fence-run-v1\","
      << "\"status\":\"PASS\","
...
std::string fail_summary_json(const std::string& detail) {
  ...
  (void)detail;
  return "{\"schema\":\"safe-c3-ancestor-fence-run-v1\","
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    // exactly the frozen base.  Candidate 4099 is therefore an inactive
    ...
    engine << "{\"record\":\"meta\","
           << "\"schema\":\"safe-c3-ancestor-fence-engine-v1\","
    ...
    std::cerr << error.what() << '\n';
    if (have_args && !args.summary.empty() && ::access(args.summary.c_str(), F_OK) != 0) {
      try {
