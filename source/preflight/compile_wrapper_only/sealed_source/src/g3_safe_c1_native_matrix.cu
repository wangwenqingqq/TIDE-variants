...
struct QueryExport {
  ...
  // This describes only candidate/result materialization: no full immutable
  // vector payload is scanned by the answer path.  The separate control-plane
  // state partition audit may iterate stable-ID metadata.
...
inline std::string query_receipt_sha256(const QueryExport& output) {
  std::ostringstream bytes;
  bytes << "safe-c1-g3-post-rebuild-receipt-v1\n"
        << output.kind << '\n' << output.base_path << '\n'
...
inline std::string post_rebuild_binding_sha256(const QueryExport& output,
  ...
  std::ostringstream bytes;
  bytes << "safe-c1-g3-post-rebuild-binding-v1\n"
        << "kind=" << output.kind << '\n'
 ...
 public:
  ...
  IssuedQuery query_range(int query_id, const std::int16_t* query_values, DistanceSq radius_sq) {
    ...
    try {
      ...
      // A range predicate mirror failure is also fail-stop.  This v2 path is
