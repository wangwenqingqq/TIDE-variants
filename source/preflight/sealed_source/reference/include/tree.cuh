...
// Every CUDA operation in the isolated builder is fail-stop.  A failed build
// cannot fall through to snapshot capture, receipt creation, or output publish.
inline void g3_tree_require_cuda_success(cudaError_t status, const char* phase)
{
	if (status == cudaSuccess) return;
	throw std::runtime_error(std::string("Safe-C1 GTS tree builder CUDA failure at ") +
	                         phase + ": " + cudaGetErrorString(status));
}

inline void g3_tree_require_host_allocation(const void* pointer, const char* phase)
{
	if (pointer != nullptr) return;
	throw std::runtime_error(std::string("Safe-C1 GTS tree builder host allocation failure at ") + phase);
}

inline void g3_tree_synchronize_or_fail_stop(const char* phase)
{
	g3_tree_require_cuda_success(cudaDeviceSynchronize(), phase);
	g3_tree_require_cuda_success(cudaGetLastError(), phase);
}

#define CHECK(call) do { g3_tree_require_cuda_success((call), #call); } while (0)
				  ...
				  int &tree_h, int *&empty_list)
{
	...
	initIndexData<<<(data_info[1] - 1) / THREAD_NUM + 1, THREAD_NUM>>>(data_info, node_list, split_list,
																	   empty_list, id_list);
	g3_tree_synchronize_or_fail_stop("initIndexData");
	...
	while ((cur_level < MAX_H - 1) && (split_num[0] > 0))
	{
		...
		g3_tree_synchronize_or_fail_stop("getPivotDis");

		thrust::sort_by_key(thrust::device, dis_list, dis_list + data_info[1], id_list);
		g3_tree_synchronize_or_fail_stop("thrust::sort_by_key");
	...
	// Mark remaining non-empty childless nodes as leaf
	{
		...
		g3_tree_require_host_allocation(h_nl, "leaf-mark node copy");
		g3_tree_require_host_allocation(h_el, "leaf-mark empty copy");
		CHECK(cudaMemcpy(h_nl, node_list, max_node_num[0] * sizeof(TN), cudaMemcpyDeviceToHost));
		CHECK(cudaMemcpy(h_el, empty_list, max_node_num[0] * sizeof(int), cudaMemcpyDeviceToHost));
		int fixed = 0;
		...
		if (fixed > 0) {
			CHECK(cudaMemcpy(node_list, h_nl, max_node_num[0] * sizeof(TN), cudaMemcpyHostToDevice));
		}
		free(h_nl); free(h_el);
	}
	...
	// This enables direct insertion without post-hoc rearrangement.
	{
		...
		g3_tree_require_host_allocation(h_nodes, "leaf-padding node copy");
		g3_tree_require_host_allocation(h_empty, "leaf-padding empty copy");
		g3_tree_require_host_allocation(h_ids, "leaf-padding id copy");
		...
		if (num_leaves > 0) {
			...
			g3_tree_require_host_allocation(leaf_ids, "leaf-padding leaf IDs");
			int li = 0;
			for (int i = 0; i < max_node_num[0]; i++)
			...
			g3_tree_require_host_allocation(h_padded, "leaf-padding id buffer");
	...
	CHECK(cudaFree(dis_list));
	CHECK(cudaFree(split_list));
	CHECK(cudaFree(split_num));
	CHECK(cudaFree(pid_list));
	dis_list = nullptr;
	split_list = nullptr;
	split_num = nullptr;
	pid_list = nullptr;
