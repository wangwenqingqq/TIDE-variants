...
PINS="$ROOT/hardened_static_pins_v4.json"
EXPECTED_PINS_SHA='e7d49a8675c946157226c0f03fe70de3b9cfee40ee9778001a1cb54840b15042'
C1_GPU0_IDLE_POLL_ATTEMPTS=15
C1_GPU0_IDLE_POLL_SECONDS=2
...
assert p['schema']=='gtspp-c1-hardened-static-pins-v4'
...
check(p['idle_poll_safety_supersession'],'GPU idle-poll safety supersession')
check(p['idle_poll_logdir_fix'],'GPU idle-poll logdir safety fix')
fix_path=pathlib.Path(p['idle_poll_logdir_fix']['path']).resolve()
fix=json.loads(fix_path.read_text())
assert fix['schema']=='gtspp-c1-gpu-idle-poll-logdir-fix-v1'
assert fix['status']=='POST_BLOCKED_PREFLIGHT_FIX_NO_GPU_EXECUTION'
assert fix['minimal_fix'].startswith('Create C1_RUN_OUT/gpu_snapshots')
idle_path=pathlib.Path(p['idle_poll_safety_supersession']['path']).resolve()
idle=json.loads(idle_path.read_text())
assert idle['schema']=='gtspp-c1-gpu-idle-poll-safety-supersession-v1'
assert idle['status']=='POST_INCOMPLETE_RUN_SAFETY_SUPERSESSION_NO_GPU_EXECUTION'
assert idle['replacement_safety_binding']['prelaunch_policy'].startswith('Before outer launch and before each C1Microbench binary, poll at most 15 times')
assert idle['replacement_safety_binding']['fail_closed'].startswith('If strict idle does not return')
assert idle['explicit_non_changes']['protocol'].startswith('Unchanged frozen C1')
assert p['semantic_constraints']['gpu_idle_poll_attempts']==15
assert p['semantic_constraints']['gpu_idle_poll_seconds']==2
guard_text=(root/'run_c1_workspace_microbenchmark_guard.sh').read_text()
assert 'C1_GPU0_IDLE_POLL_ATTEMPTS=15' in guard_text
assert 'C1_GPU0_IDLE_POLL_SECONDS=2' in guard_text
assert 'wait_for_gpu0_strict_idle' in guard_text
assert 'wait_for_gpu0_strict_idle "pre_$label" "$expected"' in guard_text
assert "wait_for_gpu0_strict_idle 'outer_prelaunch' ''" in guard_text
wait_start=guard_text.index('\nwait_for_gpu0_strict_idle() {')
wait_end=guard_text.index('\nsnapshot_post() {', wait_start)
wait_body=guard_text[wait_start:wait_end]
assert wait_body.index('mkdir -p -- \"$dir\"') < wait_body.index('poll_log=')
...
snapshot_idle() {
  ...
  GPU_UUID="$uuid"
  export GPU_UUID
  (( used <= 256 && util == 0 )) || return 1
  printf '%s\n' "$uuid"
}

# A post-phase telemetry sample may report a transient nonzero utilization even
# after the binary has released its CUDA context.  Poll boundedly before the
# *next* launch; never bypass identity/no-compute checks and never touch another
# process.  This safety-only helper is outside all C1 timing boundaries.
wait_for_gpu0_strict_idle() {
  local label="$1" expected_uuid="$2" dir="$C1_RUN_OUT/gpu_snapshots"
  local attempt state poll_log
  mkdir -p -- "$dir" || die "cannot create GPU snapshot directory for strict-idle polling: $dir"
  poll_log="$dir/$(safe_label "${label}")_strict_idle_poll.log"
  : > "$poll_log" || die "cannot create strict-idle poll log: $poll_log"
  for ((attempt=1; attempt<=C1_GPU0_IDLE_POLL_ATTEMPTS; ++attempt)); do
    if snapshot_idle "${label}_attempt_$(printf '%02d' "$attempt")" "$expected_uuid" >/dev/null; then
      printf 'strict_idle=PASS attempt=%s max_attempts=%s poll_seconds=%s physical_gpu_uuid=%s\n' \
        "$attempt" "$C1_GPU0_IDLE_POLL_ATTEMPTS" "$C1_GPU0_IDLE_POLL_SECONDS" "$GPU_UUID" >> "$poll_log"
      printf '%s\n' "$GPU_UUID"
      return 0
    fi
    printf 'strict_idle=NOT_YET attempt=%s max_attempts=%s poll_seconds=%s\n' \
      "$attempt" "$C1_GPU0_IDLE_POLL_ATTEMPTS" "$C1_GPU0_IDLE_POLL_SECONDS" >> "$poll_log"
    if (( attempt < C1_GPU0_IDLE_POLL_ATTEMPTS )); then sleep "$C1_GPU0_IDLE_POLL_SECONDS"; fi
  done
  die "GPU0 did not reach strict idle within ${C1_GPU0_IDLE_POLL_ATTEMPTS}x${C1_GPU0_IDLE_POLL_SECONDS}s before $label; no process was touched"
}
...
PY
  ...
  wait_for_gpu0_strict_idle "pre_$label" "$expected" >/dev/null
...
GPU_UUID="$(wait_for_gpu0_strict_idle 'outer_prelaunch' '')" || die 'outer GPU0 idle polling failed'
