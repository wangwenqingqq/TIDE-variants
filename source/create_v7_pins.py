#!/usr/bin/env python3
"""Create the complete CPU-only C1 v7 integrity pin set after a successful build."""
...
HELPER_PATHS={
    ...
    "pin_verifier":"verify_v7_pins.py",
    "pin_generator":"create_v7_pins.py",
    ...
    "profile_manifest_fixture":"tools/test_c1_v7_profile_manifest_fixtures.py",
    "profile_terminal_fixture":"tools/test_c1_v7_profile_terminal_fixture.py",
    "guard_session_fixture":"tools/test_c1_v7_guard_session_fixtures.py",
    "profile_guard_cli_fixture":"tools/test_c1_v7_profile_guard_cli_contract.py",
...
def validate_v7_trace_provenance(inputs:dict,protocols:dict)->None:
...
def validate_profile_execution_plan(protocols:dict)->None:
    ...
    if plan.get("schema")!="gtspp-c1-v7-profile-execution-plan-v1": raise SystemExit("profile plan schema")
...
def main()->None:
    ...
    build=root/"worktree_build_manifest_v7.json"
    ...
    validate_v7_trace_provenance(inputs,protocols)
    ...
    out={
        "schema":"gtspp-c1-v7-pins-v1",
        "created_utc":datetime.now(timezone.utc).isoformat(),
        "root":str(root),
        "scope":"All v7 C1 artifacts pinned after CPU/NVCC compilation only; no CUDA binary, nsys, nvidia-smi, or GPU workload executed.",
    ...
    target=a.out or root/"hardened_static_pins_v7.json"
