...
void LES3::preprocess(string path_to_sets, string path_to_groups, string path_to_ids) {
    ...
    vector<unsigned char> assigned(database.size(), 0);
    for(int i = 0; i < groups.size(); i++) {
        ...
        for(int set_id : groups[i]) {
            ...
            if(assigned[set_id] != 0) {
                throw runtime_error("LES3 group membership contains a duplicate set ID");
            }
            assigned[set_id] = 1;
            this_group.push_back(std::move(database[set_id]));
            this_group_ids.push_back(database_ids[set_id]);
        }
        grouped_database.push_back(std::move(this_group));
        grouped_ids.push_back(std::move(this_group_ids));
    }
    if(find(assigned.begin(), assigned.end(), 0) != assigned.end()) {
        throw runtime_error("LES3 group membership does not cover every set");
    }
    in.close();
    database.clear();
    database.shrink_to_fit();
    database_ids.clear();
    database_ids.shrink_to_fit();
