...
#include "c1_alloc_counter.cuh"

#ifndef C1_PERSISTENT_WORKSPACE
#define C1_PERSISTENT_WORKSPACE 1
#endif
#ifndef C1_ONE_QUERY_FASTPATH
#define C1_ONE_QUERY_FASTPATH 1
#endif
...
// C1 microbenchmark safety gate: result-slot storage is sized by the maximum
// possible selected leaves. Reject an invalid/overflowing observed count
// before any kernel can index result buffers; never truncate or continue.
[[noreturn]] static inline void c1FailClosedWorkspace(const char *message)
{
	fprintf(stderr, "C1 workspace safety violation: %s\n", message);
	fflush(stderr);
	abort();
}

static inline void validateC1SearchNumForWorkspace(int observed_search_num)
{
	if (observed_search_num < 0)
		c1FailClosedWorkspace("negative selected-leaf count");
	if (observed_search_num > update_ws_max_search_num_cap)
		c1FailClosedWorkspace("selected-leaf count exceeds allocated search workspace");
	if (MAX_SIZE <= 0 || observed_search_num > INT_MAX / MAX_SIZE)
		c1FailClosedWorkspace("selected-leaf count overflows result-slot arithmetic");
	const int required_slots = observed_search_num * MAX_SIZE;
	if (required_slots > update_result_ws_cap)
		c1FailClosedWorkspace("selected-leaf result slots exceed compact-result workspace");
}
...
{
	const int batch_queries = min(qnum_leaf_batch, qnum);
	if (qnum <= 0 || qnum_leaf_batch <= 0 || max_node_num_0 <= 0 || batch_queries <= 0)
		c1FailClosedWorkspace("invalid workspace dimensions");
	if (max_node_num_0 > INT_MAX / batch_queries)
		c1FailClosedWorkspace("search-workspace arithmetic overflow");
	int max_search_num_needed = max_node_num_0 * batch_queries;
	if (MAX_SIZE <= 0 || max_search_num_needed > INT_MAX / MAX_SIZE)
		c1FailClosedWorkspace("result-workspace arithmetic overflow");
	int max_result_slots_needed = max_search_num_needed * MAX_SIZE;

	if (search_num == nullptr)
	{
		CHECK(C1_CUDA_MALLOC_MANAGED(&search_num, sizeof(int)));
		CHECK(C1_CUDA_MALLOC_MANAGED(&result_num, sizeof(int)));
	}

	if (qnode_count == nullptr || qnode_count_prefix == nullptr || update_ws_qnum_leaf_cap < qnum_leaf_batch)
	{
		if (qnode_count != nullptr)
			CHECK(C1_CUDA_FREE(qnode_count));
		if (qnode_count_prefix != nullptr)
			CHECK(C1_CUDA_FREE(qnode_count_prefix));

		CHECK(C1_CUDA_MALLOC(&qnode_count, qnum_leaf_batch * sizeof(int)));
		CHECK(C1_CUDA_MALLOC(&qnode_count_prefix, qnum_leaf_batch * sizeof(int)));
	...
	if (query_node_list == nullptr || update_ws_qnum_cap < qnum || update_ws_max_node_cap < max_node_num_0)
	{
		if (query_node_list != nullptr)
			CHECK(C1_CUDA_FREE(query_node_list));
		if (allinclude_flags_ws != nullptr)
			CHECK(C1_CUDA_FREE(allinclude_flags_ws));
		CHECK(C1_CUDA_MALLOC(&query_node_list, max_node_num_0 * qnum * sizeof(int)));
		CHECK(C1_CUDA_MALLOC(&allinclude_flags_ws, max_node_num_0 * qnum * sizeof(int)));
	...
	if (qnode_idx == nullptr || update_ws_qnum_leaf_cap < qnum_leaf_batch || update_ws_max_node_cap < max_node_num_0)
	{
		if (qnode_idx != nullptr)
			CHECK(C1_CUDA_FREE(qnode_idx));
		CHECK(C1_CUDA_MALLOC(&qnode_idx, max_node_num_0 * qnum_leaf_batch * sizeof(int)));
	}

	if (update_ws_max_search_num_cap < max_search_num_needed)
	{
		if (init_result_id != nullptr)
			CHECK(C1_CUDA_FREE(init_result_id));
		if (init_result_dis != nullptr)
			CHECK(C1_CUDA_FREE(init_result_dis));
		if (qresult_idx != nullptr)
			CHECK(C1_CUDA_FREE(qresult_idx));
		if (query_lnode != nullptr)
			CHECK(C1_CUDA_FREE(query_lnode));
		if (query_qid != nullptr)
			CHECK(C1_CUDA_FREE(query_qid));

		CHECK(C1_CUDA_MALLOC(&init_result_id, max_search_num_needed * MAX_SIZE * sizeof(int)));
		CHECK(C1_CUDA_MALLOC(&init_result_dis, max_search_num_needed * MAX_SIZE * sizeof(float)));
		CHECK(C1_CUDA_MALLOC(&qresult_idx, max_search_num_needed * MAX_SIZE * sizeof(int)));
		CHECK(C1_CUDA_MALLOC(&query_lnode, max_search_num_needed * sizeof(int)));
		CHECK(C1_CUDA_MALLOC(&query_qid, max_search_num_needed * sizeof(int)));
	...
	if (update_result_ws_cap < max_result_slots_needed)
	{
		if (update_result_id_ws != nullptr)
			CHECK(C1_CUDA_FREE(update_result_id_ws));
		if (update_result_dis_ws != nullptr)
			CHECK(C1_CUDA_FREE(update_result_dis_ws));

		CHECK(C1_CUDA_MALLOC_MANAGED(&update_result_id_ws, max_result_slots_needed * sizeof(int)));
		CHECK(C1_CUDA_MALLOC_MANAGED(&update_result_dis_ws, max_result_slots_needed * sizeof(float)));
...
static inline void ensureTotalResultWorkspace(int needed)
{
	...
	if (total_result_id_ws != nullptr)
		CHECK(C1_CUDA_FREE(total_result_id_ws));
	if (total_result_dis_ws != nullptr)
		CHECK(C1_CUDA_FREE(total_result_dis_ws));

	int alloc_size = needed * 2;
	CHECK(C1_CUDA_MALLOC_MANAGED(&total_result_id_ws, alloc_size * sizeof(int)));
	CHECK(C1_CUDA_MALLOC_MANAGED(&total_result_dis_ws, alloc_size * sizeof(float)));
	total_result_ws_cap = alloc_size;
}

// Releases only buffers whose lifetime is the C1 query workspace.  It deliberately
// leaves C3/session state (delete/insert bookkeeping and the static tree) intact.
// The C1 microbenchmark calls this after a query only when
// C1_PERSISTENT_WORKSPACE==0; legacy update teardown may also call it safely.
static inline void releaseC1QueryWorkspace(int *&qresult_count, int *&qresult_count_prefix,
                                           int *&result_id, float *&result_dis)
{
	if (init_result_id != nullptr) CHECK(C1_CUDA_FREE(init_result_id));
	if (init_result_dis != nullptr) CHECK(C1_CUDA_FREE(init_result_dis));
	if (qresult_idx != nullptr) CHECK(C1_CUDA_FREE(qresult_idx));
	if (query_lnode != nullptr) CHECK(C1_CUDA_FREE(query_lnode));
	if (query_qid != nullptr) CHECK(C1_CUDA_FREE(query_qid));
	if (query_node_list != nullptr) CHECK(C1_CUDA_FREE(query_node_list));
	if (allinclude_flags_ws != nullptr) CHECK(C1_CUDA_FREE(allinclude_flags_ws));
	if (search_num != nullptr) CHECK(C1_CUDA_FREE(search_num));
	if (result_num != nullptr) CHECK(C1_CUDA_FREE(result_num));
	if (qnode_count != nullptr) CHECK(C1_CUDA_FREE(qnode_count));
	if (qnode_count_prefix != nullptr) CHECK(C1_CUDA_FREE(qnode_count_prefix));
	if (qnode_idx != nullptr) CHECK(C1_CUDA_FREE(qnode_idx));
	if (qresult_count != nullptr) CHECK(C1_CUDA_FREE(qresult_count));
	if (qresult_count_prefix != nullptr) CHECK(C1_CUDA_FREE(qresult_count_prefix));
	if (update_result_id_ws != nullptr) CHECK(C1_CUDA_FREE(update_result_id_ws));
	if (update_result_dis_ws != nullptr) CHECK(C1_CUDA_FREE(update_result_dis_ws));
	// Ephemeral C1 releases these buffers after every query.  Clear both
	// ownership pointers before the next ensureUpdateSearchWorkspace call so
	// the capacity reset below allocates fresh buffers instead of cudaFree-ing
	// a dangling pointer on the second query.
	update_result_id_ws = nullptr;
	update_result_dis_ws = nullptr;
	if (total_result_id_ws != nullptr) CHECK(C1_CUDA_FREE(total_result_id_ws));
	if (total_result_dis_ws != nullptr) CHECK(C1_CUDA_FREE(total_result_dis_ws));
	...
	total_result_id = nullptr;
	total_result_dis = nullptr;
						  ...
						  int *&qresult_count_prefix, int *&result_id, float *&result_dis, char *data_s, int *size_s)
{
	...
	if (qresult_count == nullptr)
		CHECK(C1_CUDA_MALLOC_MANAGED(&qresult_count, qnum * sizeof(int)));
	if (qresult_count_prefix == nullptr)
		CHECK(C1_CUDA_MALLOC_MANAGED(&qresult_count_prefix, qnum * sizeof(int)));
	cur_level = 1;
	start_idx = 1;
	search_num[0] = qnum;
	...
	if (C1_ONE_QUERY_FASTPATH && qnum == 1)
	{
		...
		validateC1SearchNumForWorkspace(search_num[0]);
		if (search_num[0] == 0)
		{
			qresult_count[0] = 0;
	...
	// Generic C1 path must surface deferred kernel/thrust errors before returning
	// to the measured runner; event timing alone is not the fail-closed boundary.
	CHECK(cudaDeviceSynchronize());
	CHECK(cudaGetLastError());
}
