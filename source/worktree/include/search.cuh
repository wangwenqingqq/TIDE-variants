...
__global__ void findNextRnn(int *query_node_list, int start_idx, TN *node_list, float r, short *data_d,
							int *qid_list, int node_num, int *max_node_num, int *data_info, int *empty_list, char *data_s, int *size_s, int cur_level, float *max_dis_d, int *allinclude_flags)
{
	int bid = blockIdx.x;
	int tid = threadIdx.x;
	int query_id = bid;
	...
	for (int i = tid; i < node_num; i += blockDim.x)
	{
		...
		{
			// All-include propagation: parent is all-include -> child is all-include (skip distance computation)
			if (allinclude_flags != nullptr && allinclude_flags[query_id * max_node_num[0] + nid_parent] == 1)
			{
				query_node_list[query_id * max_node_num[0] + nid] = 1;
				allinclude_flags[query_id * max_node_num[0] + nid] = 1;
			...
			{
				query_node_list[query_id * max_node_num[0] + nid] = 1;
				// All-include: if query ball fully contains this partition, mark it
				if (max_dis_d != nullptr && dis_q + max_dis_d[nid] <= r)
					allinclude_flags[query_id * max_node_num[0] + nid] = 1;
			}
		}
	}
							   ...
							   int *search_num, char *data_s, int *size_s)
{
	...
	if (bid < search_num[0])
	{
		...
		// All-include optimization: skip distance computation, include all points
		if (is_allinclude[0] == 1)
		{
			for (int i = tid; i < node[0].size && node[0].is_leaf == 1; i += blockDim.x)
			{
				...
				if (data_id != qid)
				{
					...
					init_result_dis[bid * MAX_SIZE + i] = -1.0f;  // Distance not computed
				}
			}
...
__global__ void mergeLeafNode(int *query_node_list, int *qnode_idx, int *query_lnode, int *max_node_num,
							  int qnum, int *query_qid, int start_q, int *allinclude_flags)
{
	int id = blockDim.x * blockIdx.x + threadIdx.x;
	int total_num = gridDim.x * blockDim.x;
	...
	for (int idx = id; idx < qnum * max_node_num[0]; idx += total_num)
	{
		if (query_node_list[idx + start_q * max_node_num[0]] == 1)
		{
			...
			// Encode all-include flag in nid sign: negative nid means all-include
			if (allinclude_flags != nullptr && allinclude_flags[idx + start_q * max_node_num[0]] == 1)
				nid = -(nid + 1);  // +1 to handle nid=0 case
					...
					float &time_search, FILE *&fcost)
{
	...
	int *allinclude_flags;
	CHECK(cudaMalloc((void **)&allinclude_flags, max_node_num[0] * qnum * sizeof(int)));
	CHECK(cudaMemset(allinclude_flags, 0, max_node_num[0] * qnum * sizeof(int)));
	...
	while ((cur_level < tree_h))
	{
		...
		findNextRnn<<<qnum, THREAD_NUM>>>(query_node_list, start_idx, node_list, r, data_d, qid_list, node_num, max_node_num,
										  data_info, empty_list, data_s, size_s, cur_level, max_dis_d, allinclude_flags);
		cudaDeviceSynchronize();
		cudaError_t cudaStatus = cudaGetLastError();
		if (cudaStatus != cudaSuccess)
	...
	for (int i = 0; i < qnum; i = i + qnum_leaf)
	{
		...
		mergeLeafNode<<<(qnum_leaf * max_node_num[0] - 1) / THREAD_NUM + 1, THREAD_NUM>>>(query_node_list, qnode_idx, query_lnode,
																						  max_node_num, qnum_leaf, query_qid, start_q, allinclude_flags);
		cudaDeviceSynchronize();
		cudaStatus = cudaGetLastError();
		if (cudaStatus != cudaSuccess)
					...
					int *&qresult_count, int *&qresult_count_prefix, char *data_s, int *size_s, float &time_search, FILE *fcost)
{
	...
	for (int i = 0; i < qnum; i = i + qnum_leaf)
	{
		...
		thrust::exclusive_scan(thrust::device, query_node_list + start_q * max_node_num[0],
							   query_node_list + start_q * max_node_num[0] + max_node_num[0] * qnum_leaf, qnode_idx);
		mergeLeafNode<<<(qnum_leaf * max_node_num[0] - 1) / THREAD_NUM + 1, THREAD_NUM>>>(query_node_list, qnode_idx, query_lnode,
																						  max_node_num, qnum_leaf, query_qid, start_q, nullptr);
		cudaDeviceSynchronize();
		cudaStatus = cudaGetLastError();
		if (cudaStatus != cudaSuccess)
			fprintf(stderr, "mergeLeafNode error: %s\n", cudaGetErrorString(cudaStatus));
