...
void upload_rp_constants(
    ...
    int mode)
{
    CHECK(cudaMemcpyToSymbol(c_rp_alpha, h_alpha, RP_MAX_LEVELS * sizeof(float)));
    CHECK(cudaMemcpyToSymbol(c_rp_beta, h_beta, RP_MAX_LEVELS * sizeof(float)));
    CHECK(cudaMemcpyToSymbol(c_rp_gamma, h_gamma, RP_MAX_LEVELS * sizeof(float)));
    if (h_lut_breaks && h_lut_slopes && h_lut_intercepts) {
        CHECK(cudaMemcpyToSymbol(c_lut_breaks, h_lut_breaks, RP_LUT_SIZE * sizeof(float)));
        CHECK(cudaMemcpyToSymbol(c_lut_slopes, h_lut_slopes, RP_LUT_SIZE * sizeof(float)));
        CHECK(cudaMemcpyToSymbol(c_lut_intercepts, h_lut_intercepts, RP_LUT_SIZE * sizeof(float)));
    }
    CHECK(cudaMemcpyToSymbol(c_lut_num_segments, &lut_size, sizeof(int)));
    CHECK(cudaMemcpyToSymbol(c_rp_mode, &mode, sizeof(int)));
