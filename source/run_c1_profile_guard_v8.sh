#!/usr/bin/env bash
# V8 profile outer guard. Only this process may launch nsys/C1Microbench for the
...
PLAN="$ROOT/run_c1_profile_primary_v8.sh"
PLAN_JSON="$ROOT/profile_execution_plan_v8.json"
PINS="$ROOT/hardened_static_pins_v8.json"
PIN_VERIFY="$ROOT/verify_v8_pins.py"
MANIFEST="$ROOT/c1_v8_profile_manifest.py"
VERIFIER="$ROOT/verify_c1_profile_artifacts_v8.py"
...
die() { mark_failed_if_manifest "blocked: $*"; printf 'C1-V8-PROFILE-GUARD BLOCKED: %s\n' "$*" >&2; exit 69; }
...
manifest_exists() {
  [[ -n "$PROFILE_OUT" && -d "$PROFILE_OUT" && -f "$PROFILE_OUT/profile_run_manifest_v8.json" && ! -L "$PROFILE_OUT/profile_run_manifest_v8.json" ]]
}
...
write_child_session_record() {
  ...
  record="$record_dir/owned_session_cleanup_v8.json"
...
payload={
 "schema":"gtspp-c1-v8-profile-owned-child-cleanup-v1",
...
verify_own_child_session() {
  ...
  if [[ "$phase" == bootstrap ]]; then
    ...
    [[ "$cmd" == *C1-V8-PROFILE-OWNED-BOOTSTRAP* && "$cmd" == *"$CHILD_READY_FILE"* && "$cmd" == *"$CHILD_GO_FILE"* && "$cmd" == *"$CHILD_ABORT_FILE"* && "$cmd" == *"$CHILD_EXPECTED_EXECUTABLE"* && "$cmd" == *"$CHILD_EXPECTED_COMMAND"* ]] || return 1
    return 0
  fi
...
own_wrapper_is_verified() {
  ...
  [[ ( "$exe" == /usr/bin/bash || "$exe" == /bin/bash || "$exe" == /usr/bin/setsid ) && "$cmd" == *C1-V8-PROFILE-OWNED-BOOTSTRAP* && "$cmd" == *"$CHILD_READY_FILE"* && "$cmd" == *"$CHILD_GO_FILE"* && "$cmd" == *"$CHILD_ABORT_FILE"* && "$cmd" == *"$CHILD_EXPECTED_EXECUTABLE"* && "$cmd" == *"$CHILD_EXPECTED_COMMAND"* ]]
}
...
[[ ! -e "$abort" ]] || exit 125
exec "$@"
' C1-V8-PROFILE-OWNED-BOOTSTRAP "$CHILD_READY_FILE" "$CHILD_GO_FILE" "$CHILD_ABORT_FILE" "$expected_exe" "$expected_command" "$@" >"$stdout_log" 2>"$stderr_log" &
  CHILD_WRAPPER_PID=$!
  adopt_own_child_session_from_ready
}
...
write_guard_card() {
  ...
  status="$status" reason="$reason" GPU_UUID="$SESSION_UUID" "$PYTHON" -B -I - "$PROFILE_OUT/profile_guard_card_v8.json" <<'PY'
import datetime,json,os,pathlib,sys
p=pathlib.Path(sys.argv[1])
old={}
if p.exists():
...
old.update({"schema":"gtspp-c1-v8-profile-guard-card-v1","updated_utc":datetime.datetime.now(datetime.timezone.utc).isoformat(),
 ...
 "snapshots_dir":str(p.parent/"gpu_snapshots"),"manifest":str(p.parent/"profile_run_manifest_v8.json")})
...
verify_source_only() {
  require_regular_raw "$PINS" && require_regular_raw "$PIN_VERIFY" && require_regular_raw "$MANIFEST" && require_regular_raw "$VERIFIER" && require_regular_raw "$PLAN" && require_regular_raw "$PLAN_JSON" && require_regular_raw "$PROFILE_MANIFEST_FIXTURE" && require_regular_raw "$PROFILE_TERMINAL_FIXTURE" && require_regular_raw "$PROFILE_CHILD_SESSIONS_FIXTURE" || return 1
  "$PYTHON" -B -I "$PIN_VERIFY" --root "$ROOT" --pins "$PINS" --static-source || return 1
  # All executable fixtures are CPU-only and run before any nvidia-smi/nsys path.
  "$PYTHON" -B -I "$PROFILE_MANIFEST_FIXTURE" --root "$ROOT" || return 1
  "$PYTHON" -B -I "$PROFILE_TERMINAL_FIXTURE" --root "$ROOT" || return 1
  "$PYTHON" -B -I "$PROFILE_CHILD_SESSIONS_FIXTURE" --root "$ROOT" || return 1
...
need(plan.get("schema")=="gtspp-c1-v8-profile-execution-plan-v1","schema")
...
need(card.get('schema')=='gtspp-c1-microbench-run-card-v8' and done.get('schema')=='gtspp-c1-microbench-completion-v8','schema')
...
cleanup() {
  ...
  if ensure_manifest_ready_from_disk; then
    ...
    if [[ "$SESSION_READY" == 1 ]]; then
      ...
      if ! rm -f -- "$PROFILE_OUT/.c1_profile_guard_session_v8.json"; then rc=69; FINAL_STATUS='FAILED'; FINAL_REASON='profile session removal failed'; fi
    fi
...
run_cpu_only_fixture_mode() {
  ...
  [[ "${C1_V8_CPU_ONLY_FIXTURE:-}" == YES ]] || { echo 'C1-V8 profile fixture mode requires explicit CPU-only marker' >&2; exit 64; }
  [[ "$fixture_out" == /tmp/* && -d "$fixture_out" && -f "$fixture_out/profile_run_manifest_v8.json" && ! -L "$fixture_out/profile_run_manifest_v8.json" ]] || {
    echo 'C1-V8 profile fixture requires a prepared regular manifest under /tmp' >&2; exit 64; }
...
if [[ "$#" -gt 0 && "$1" == --fixture-terminal-failure ]];then
  shift;[[ "$#" -eq 1 ]] || { echo 'Usage: run_c1_profile_guard_v8.sh --fixture-terminal-failure ABS_PROFILE_OUT' >&2; exit 64; }
  ...
  [[ "$PROFILE_OUT" == /tmp/* && -d "$PROFILE_OUT" && -f "$PROFILE_OUT/profile_run_manifest_v8.json" && ! -L "$PROFILE_OUT/profile_run_manifest_v8.json" ]] || { echo 'C1-V8-PROFILE-GUARD fixture requires prepared regular manifest under /tmp' >&2; exit 64; }
...
C1_V8_PROFILE_TRUST_ROOT="$(read_env C1_V8_PROFILE_TRUST_ROOT)";C1_V8_PROFILE_LAUNCHER="$(read_env C1_V8_PROFILE_LAUNCHER)";C1_V8_PROFILE_TRUST_GUARD_SHA="$(read_env C1_V8_PROFILE_TRUST_GUARD_SHA)";C1_V8_PROFILE_TRUST_PINS_SHA="$(read_env C1_V8_PROFILE_TRUST_PINS_SHA)";C1_V8_PROFILE_TRUST_PIN_VERIFY_SHA="$(read_env C1_V8_PROFILE_TRUST_PIN_VERIFY_SHA)";C1_ALLOW_GPU0="$(read_env C1_ALLOW_GPU0)";RUN_OUT="$(read_env C1_PROFILE_OUT)"
[[ "$0" == "$GUARD" && "$C1_V8_PROFILE_TRUST_ROOT" == YES && "$C1_V8_PROFILE_LAUNCHER" == "$LAUNCHER" ]] || die 'reviewed profile trust-root required'
...
[[ "$(sha "$GUARD")" == "$C1_V8_PROFILE_TRUST_GUARD_SHA" && "$(sha "$PINS")" == "$C1_V8_PROFILE_TRUST_PINS_SHA" && "$(sha "$PIN_VERIFY")" == "$C1_V8_PROFILE_TRUST_PIN_VERIFY_SHA" ]] || die 'profile trust-root bootstrap hash mismatch'
...
BASE="$ROOT/profiles";OUT="$("$REALPATH" -m -- "$RUN_OUT")" || die 'bad profile output';is_under "$OUT" "$BASE" && [[ "$(basename "$OUT")" == c1_v8_profile_* && ! -e "$OUT" && ! -L "$OUT" ]] || die 'unsafe profile output'
PROFILE_OUT="$OUT";export PROFILE_OUT
exec 9>>"$LOCK";"$FLOCK" -n 9 || die 'another v8 GPU0 session holds lock'
trap cleanup EXIT;trap signal_handler INT TERM HUP
...
"$PYTHON" -B -I - "$PROFILE_OUT/.c1_profile_guard_session_v8.json" "$SESSION_UUID" <<'PY'
import json,pathlib,sys
p=pathlib.Path(sys.argv[1]);p.write_text(json.dumps({'schema':'gtspp-c1-v8-profile-session-v1','physical_gpu_index':0,'physical_gpu_uuid':sys.argv[2]})+'\n')
PY
chmod 0600 "$PROFILE_OUT/.c1_profile_guard_session_v8.json" || die 'profile session chmod'
SESSION_READY=1
