#!/usr/bin/env bash
# V8 outer guard: only this process launches C1Microbench.  There are no
...
PLAN="$ROOT/run_c1_four_variants_v8.sh"
PLAN_JSON="$ROOT/measured_execution_plan_v8.json"
PINS="$ROOT/hardened_static_pins_v8.json"
PIN_VERIFY="$ROOT/verify_v8_pins.py"
MANIFEST="$ROOT/c1_v8_manifest.py"
SEMANTIC="$ROOT/verify_c1_semantics_v8.py"
ANALYZER="$ROOT/analyze_c1_microbenchmark_v8.py"
FINALIZER="$ROOT/finalize_c1_evidence_v8.py"
...
die() { printf 'C1-V8-GUARD BLOCKED: %s\n' "$*" >&2; exit 69; }
...
manifest_exists() {
  [[ -n "$C1_RUN_OUT" && -d "$C1_RUN_OUT" && -f "$C1_RUN_OUT/run_manifest_v8.json" && ! -L "$C1_RUN_OUT/run_manifest_v8.json" ]]
}
...
payload={
 "schema":"gtspp-c1-v8-owned-child-cleanup-v1",
...
verify_own_child_session() {
  ...
  if [[ "$phase" == bootstrap ]]; then
    ...
    [[ "$cmd" == *C1-V8-MEASURED-OWNED-BOOTSTRAP* && "$cmd" == *"$CHILD_READY_FILE"* && "$cmd" == *"$CHILD_GO_FILE"* && "$cmd" == *"$CHILD_ABORT_FILE"* && "$cmd" == *"$CHILD_EXPECTED_EXECUTABLE"* && "$cmd" == *"$CHILD_EXPECTED_COMMAND"* ]] || return 1
    return 0
  fi
...
own_wrapper_is_verified() {
  ...
  [[ ( "$exe" == /usr/bin/bash || "$exe" == /bin/bash || "$exe" == /usr/bin/setsid ) && "$cmd" == *C1-V8-MEASURED-OWNED-BOOTSTRAP* && "$cmd" == *"$CHILD_READY_FILE"* && "$cmd" == *"$CHILD_GO_FILE"* && "$cmd" == *"$CHILD_ABORT_FILE"* && "$cmd" == *"$CHILD_EXPECTED_EXECUTABLE"* && "$cmd" == *"$CHILD_EXPECTED_COMMAND"* ]]
}
...
[[ ! -e "$abort" ]] || exit 125
exec "$@"
' C1-V8-MEASURED-OWNED-BOOTSTRAP "$CHILD_READY_FILE" "$CHILD_GO_FILE" "$CHILD_ABORT_FILE" "$expected_exe" "$expected_command" "$@" >"$stdout_log" 2>"$stderr_log" &
  CHILD_WRAPPER_PID=$!
  adopt_own_child_session_from_ready
}
...
write_guard_card() {
  ...
  status="$status" reason="$reason" GPU_UUID="$SESSION_UUID" "$PYTHON" -B -I - "$C1_RUN_OUT/guard_run_card_v8.json" <<'PY'
import datetime,json,os,pathlib,sys
p=pathlib.Path(sys.argv[1])
old={}
if p.exists():
...
old.update({"schema":"gtspp-c1-v8-guard-card-v2",
 "updated_utc":datetime.datetime.now(datetime.timezone.utc).isoformat(),
 ...
 "manifest":str(p.parent/"run_manifest_v8.json")})
...
need(card.get("schema")=="gtspp-c1-microbench-run-card-v8","run-card schema")
need(done.get("schema")=="gtspp-c1-microbench-completion-v8","completion schema")
...
cleanup() {
  ...
  if ensure_manifest_ready_from_disk; then
    ...
    if [[ "$SESSION_READY" == 1 ]]; then
      ...
      if ! rm -f -- "$C1_RUN_OUT/.c1_guard_session_v8.json"; then rc=69; FINAL_STATUS='FAILED'; FINAL_REASON='session removal failed'; fi
    fi
...
run_cpu_only_fixture_mode() {
  ...
  [[ "${C1_V8_CPU_ONLY_FIXTURE:-}" == YES ]] || { echo 'C1-V8 fixture mode requires explicit CPU-only marker' >&2; exit 64; }
  [[ "$fixture_out" == /tmp/* && -d "$fixture_out" && -f "$fixture_out/run_manifest_v8.json" && ! -L "$fixture_out/run_manifest_v8.json" ]] || {
    echo 'C1-V8 fixture requires a prepared regular manifest under /tmp' >&2; exit 64; }
...
C1_V8_TRUST_ROOT="$(read_env C1_V8_TRUST_ROOT)"
C1_V8_LAUNCHER="$(read_env C1_V8_LAUNCHER)"
C1_V8_TRUST_GUARD_SHA="$(read_env C1_V8_TRUST_GUARD_SHA)"
C1_V8_TRUST_PINS_SHA="$(read_env C1_V8_TRUST_PINS_SHA)"
C1_V8_TRUST_PIN_VERIFY_SHA="$(read_env C1_V8_TRUST_PIN_VERIFY_SHA)"
C1_ALLOW_GPU0="$(read_env C1_ALLOW_GPU0)"
RUN_OUT="$(read_env C1_RUN_OUT)"
[[ "$0" == "$GUARD" && "$C1_V8_TRUST_ROOT" == YES && "$C1_V8_LAUNCHER" == "$LAUNCHER" ]] || die 'reviewed trust-root required'
...
[[ "$(sha "$GUARD")" == "$C1_V8_TRUST_GUARD_SHA" && "$(sha "$PINS")" == "$C1_V8_TRUST_PINS_SHA" && "$(sha "$PIN_VERIFY")" == "$C1_V8_TRUST_PIN_VERIFY_SHA" ]] || die 'trust-root bootstrap hash mismatch'
...
is_under "$OUT" "$BASE" && [[ "$(basename "$OUT")" == c1_v8_measured_* && ! -e "$OUT" && ! -L "$OUT" ]] || die 'unsafe output'
...
"$PYTHON" -B -I - "$C1_RUN_OUT/.c1_guard_session_v8.json" "$SESSION_UUID" <<'PY'
import json,pathlib,sys
p=pathlib.Path(sys.argv[1]); p.write_text(json.dumps({"schema":"gtspp-c1-v8-session-v2","physical_gpu_index":0,"physical_gpu_uuid":sys.argv[2]})+"\n")
PY
chmod 0600 "$C1_RUN_OUT/.c1_guard_session_v8.json" || die 'session chmod'
SESSION_READY=1
